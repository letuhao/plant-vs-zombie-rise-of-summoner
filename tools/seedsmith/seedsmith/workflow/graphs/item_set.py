...
def answer_declares_content(draft: dict, context: "dict[str, Any]") -> "list[str]":
    """`required: []` at the top level is deliberate — a `blocked` answer is legal and carries none
    of the content fields. What is NOT legal is an answer that declares neither."""
    if isinstance(draft.get("blocked"), str) and draft["blocked"].strip():
        return []
    kind = context.get("kind")
    wanted = (("name", "nameKey", "flavor", "capability", "members", "thresholds")
              if kind == "set" else ("name", "nameKey", "flavor", "charmClass", "axis", "families"))
    missing = [field for field in wanted if field not in draft]
    if missing:
...
def _resolved_stats(draft: dict, vocabulary) -> "tuple[dict[int, tuple], list[str]]":
    ...
    defects: "list[str]" = []
    for threshold in draft.get("thresholds") or ():
        ...
        for token in threshold.get("families") or ():
            ...
            if pick is None:
                ...
                picks.append(pick)
        if picks:
            by_threshold[int(pieces)] = tuple(picks)
