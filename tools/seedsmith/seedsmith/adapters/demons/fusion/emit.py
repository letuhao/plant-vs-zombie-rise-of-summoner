...
(demon-seed module 17, spec-fusion-recipe-generator.md §4) — mirrors `anchor/emit.py`'s own
`json.dumps(..., indent=2, sort_keys=True, ensure_ascii=False)` convention and `DemonSpeciesGen
...
#: `data/generated/demons/_fusion-recipes.json` — underscore-prefixed, ONE committed file (spec §4:
...
DEFAULT_OUTPUT_RELATIVE = ("data", "generated", "demons", "_fusion-recipes.json")
...
def check(recipes: "Mapping[str, dict]", path: Path) -> "list[str]":
    ...
    clean. Mirrors `DemonSpeciesGen --check`'s own discipline (byte-for-byte comparison, not a
