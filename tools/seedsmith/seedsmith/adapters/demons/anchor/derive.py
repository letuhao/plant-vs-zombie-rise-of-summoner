...
def load_rarity_power_fallback(version: "int | str" = 1) -> "dict[str, str]":
    """`threatBand id -> rarity id`, from `demon-rarity-power-fallback.v{version}.json`. A table,
    ...
    hot path)."""
    path = VARIANT_COUNT_TUNING_DIR / f"demon-rarity-power-fallback.v{int(version)}.json"
...
def resolve_unresolved_rarity(rarity: str, threat_band: str, *, mapping: "dict[str, str]") -> "tuple[str, bool]":
    """demon-corpus-self-heal Phase H (2026-09-07, owner-directed): rarity is OUR game's own
    ...
    (a species' parsed toughness/damage, bucketed by `demon-threat.v1.json`'s real p10..p90
    deciles) via a rank-preserving correspondence in `demon-rarity-power-fallback.v1.json` —
...
def load_aptitude_fallback(version: "int | str" = 1) -> str:
    """`inferredDefaultAptitude` from `demon-aptitude-fallback.v{version}.json`."""
    path = VARIANT_COUNT_TUNING_DIR / f"demon-aptitude-fallback.v{int(version)}.json"
    raw = json.loads(path.read_text(encoding="utf-8"))
    return raw["inferredDefaultAptitude"]


def resolve_unresolved_aptitude(aptitude: str, *, default: str) -> "tuple[str, bool]":
    """demon-corpus-self-heal Phase I (2026-09-07, owner-directed): unlike `rarity`, no real
    ...
    `threatBand` — there is nothing to derive from, so `demon-aptitude-fallback.v1.json`'s own
...
def resolve_secondary_element_from_fusion_lineage(
    ...
    """demon-corpus-self-heal follow-up (2026-09-07, owner-directed): a DIFFERENT signal source
    ...
    already-resolved `"none"` rather than only an `"unresolved"` value — a fusion demon literally
    made partly from a Fire-element parent plausibly carries some of that nature, and DemonRecipe
    ...
    Owner direction (2026-09-07): "not every demon has a secondary element and that is normal" —
    ...
    "should match" assumption failed for this recipe (confirmed live: `DemonRecipeCatalog.
