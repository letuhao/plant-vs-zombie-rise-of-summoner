"""Load the live creature seed roster used by action seeding.
...
CATALOG_PATH = REPO_ROOT / "data" / "seed" / "creatures" / "species"
LEGACY_CATALOG_PATH = REPO_ROOT / "src" / "FusionRpg.Core" / "Creatures" / "CreatureSpeciesCatalog.Generated.cs"
...
# file's own `<auto-generated>` header: "Regenerate: dotnet run --project tools/CreatureCatalogGen").
_ROW_RE = re.compile(
    ...
    r'BaseRarity\s*=\s*CreatureRarity\.(?P<rarity>\w+).*?'
...
class SpeciesRow:
    ...
    rarity_ordinal: int                 # CreatureRarity.cs declaration order, 0..9
...
def _live_row(record: dict[str, object]) -> SpeciesRow:
    ...
    # this species — `CreatureTraitPoolCuration.PickFor`'s own bridge, mirrored in `curation.py`. See
