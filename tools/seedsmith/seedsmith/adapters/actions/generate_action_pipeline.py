...
def _load_plan(path: Path) -> dict:
    ...
    if not isinstance(doc.get("_meta", {}).get("corpusHash"), str):
...
def _run_partition(*, name: str, generate: Callable[..., dict], briefs_path: Path,
    ...
    batches: list[dict] = []
    for attempt in range(max(1, max_passes)):
        summary = generate(
            ...
            write=not dry_run, resume=resume or attempt > 0,
        )
        ...
        raise RuntimeError(
            f"{name}: stopped after {max_passes} passes with "
            f"{batches[-1].get('remaining')} terminal briefs remaining"
        )
...
def run_pipeline(*, round_no: int = 1, batch_size: int = 25, max_passes: int = 8,
                 ...
                 resume: bool = True, plan_path: Path = PLAN_PATH) -> dict:
...
def run(argv=None) -> int:
    ...
    ap.add_argument("--max-passes", type=int, default=8,
                    help="maximum retry passes for unresolved model votes")
