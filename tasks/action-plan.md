...
| **`equip-atom-source-not-wired`** | **FIXED 2026-09-07 — re-investigated, not left as "adjacent scope."** That framing was this session's own, unverified characterization, not audit text — challenged, then checked directly: `EquipAtomSource`/`BattleStatComposer` live entirely in `Core/Battle/`, this program's own territory, and the production resolver shape was already documented on `EquipAtomSource.FromResolver`'s own doc comment. Fixed in `Program.cs` (wired once at boot, matching `RungPolicy.Configure`'s established pattern): `BattleStatComposer.UseEquipment(EquipAtomSource.FromResolver(instanceId => store.ResolveBindings(new OwnerScope(OwnerKind.UniqueActor, instanceId), ...).AtomsByBinding` flattened`))`. **Proven end-to-end** (`BuildSquadEquippedActionsTests.cs`, `A_real_equipped_items_atom_reaches_BattleStatComposer_through_the_real_production_resolver`) via `RpgStore.ProduceAndBind` — the SAME primitive `ReconcileUniqueEquipmentAtomBindingsUnlocked` calls internally for every real item equip, confirmed by reading that method directly (`RpgStore.UniqueActors.cs:1432`) — binding a well-formed `stat.derived` atom to a real `UniqueActor` owner scope and asserting exact arithmetic (`bare.Get(channel) + amount == geared.Get(channel)`), not merely "changed." **A real, separate, honestly-named remaining gap (content, not wiring):** every real catalogued item today (`data/seed/containers/unique-equip.json`) wraps a `stat.modify` atom, never `stat.derived` — confirmed directly (`atom.fx-passive-atk-flat.t1`'s `KindId` is `stat.modify`) — so no shipped item exercises this seam through the full `UpsertUniqueEquipment` round trip yet; the test binds through `ProduceAndBind` directly for that reason, exactly as A25 authors a synthetic triggered atom because no real seed atom is triggered yet. Full regression run, every anomaly traced to a specific non-self cause, none touching `EquipAtomSource`/`BattleStatComposer`/`Program.cs`'s equip wiring or `BuildSquadEquippedActionsTests.cs` (which is 7/7 green): `Data.Tests` 1117/1118 (the one failure is `ItemUniqueStoreTests`'s already-documented pre-existing rung-seeding gap). `Core.Tests` 12688/12709 — 21 failures, sampled and traced across every distinct signature: 14 share one cause, a real `UnknownKind — kind ''` row already committed in `data/seed/atoms/vocabulary.json` (confirmed by reading the file's own import error, not inferred) breaking any full-corpus scan; 1 is a golden-hash drift matching this program's own established concurrent-content-session pattern; 1 (`NoSpecClaimsAnUnregisteredChannel`) names `status.v1.json` inside four doc files a live concurrent session is actively editing right now (confirmed by exact filename match against `git status`); 3 (`ProveAptitudeJsonEmitTests`) fail in an external subprocess on `BattleStatComposer.Configure` — a different static than the one this fix touches (`UseEquipment`), in a separate OS process this fix cannot reach; 2 more share the vocabulary.json cause. `Server.Tests` 287/312 — 25 failures, spot-checked across every distinct signature: 2 (`ContentBootStartupWiringTests`) reproduce the SAME `vocabulary.json` error verbatim (confirmed with a temporary diagnostic assertion, reverted after reading it); 1 (`AptitudeChannelModsTests`) fails on `BattleTuningRejection: missing 'speciesTempo'` — the same live concurrent class-system/tuning session's in-progress schema change, not this fix; the remaining `World*ProjectionTests`/`DistrictAssaultCommandWireTests` fail via a cascading HTTP 500 from the same real-content-import boot path `ContentBootStartupWiringTests` already pins down. Zero of the 47 anomalies across any suite trace to this fix. |
