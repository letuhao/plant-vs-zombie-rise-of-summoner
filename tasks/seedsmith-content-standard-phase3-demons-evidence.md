# Phase 3 evidence — `content-completeness-demons` (Tasks 9-10)
...
Spec: [docs/architecture/seedsmith-content-standard/spec-content-completeness-demons.md](../docs/architecture/seedsmith-content-standard/spec-content-completeness-demons.md).

---

## Task 9 — spec written and self-reviewed

**File:** `docs/architecture/seedsmith-content-standard/spec-content-completeness-demons.md`.

**Acceptance: "Names exactly which demon-species field(s) count as 'description/flavor' for this
...
import json, glob
sample_keys = set()
for f in glob.glob('data/seed/demons/species/**/*.json', recursive=True):
...
classification model's own audit trail (`tools/seedsmith/seedsmith/adapters/demons/anchor/prompts.
...
This is distinct from `demon_coverage.py`/`demon_roster.py`'s own checked fields (`elementPrimary`,
...
- `tools/seedsmith/seedsmith/adapters/demons/completeness.py` (new) — `load_species_corpus`,
...
- `tools/seedsmith/seedsmith/report/cli.py` (edited) — one additive `elif args.adapter ==
  "demons":` branch in `cmd_check`, mirroring the existing `dungeon` branch's shape exactly (found
...
- `tools/seedsmith/tests/test_demons_completeness.py` (new) — 9 tests, all against the real corpus.

**No file outside `tools/seedsmith/`, `docs/architecture/seedsmith-content-standard/`, and this
evidence file was touched.** No demon seed data was regenerated, deleted, or modified — this task
...
```powershell
python -c "from seedsmith.report.cli import main; import sys; sys.exit(main(['check','../../data/seed/demons','--adapter','demons','--metric','Content/FieldMissing']))"
```
Output:
```
[GAP] Content/FieldMissing — demons:species: 904 of 904 'species' entries in domain 'demons' have no 'flavor' (BloverUmbrella, SquashBlover, CactusBlover, +901 more)
...
Also proven in `tests/test_demons_completeness.py::ContentFieldMissingAgainstRealCorpusTests::
test_reports_904_of_904_missing_flavor`: asserts `subject == "demons:species"`,
`missingCount == 904`, `totalCount == 904`, `field == "flavor"` — against `load_species_corpus`
reading the real, on-disk `data/seed/demons/species` tree, not a fixture.
...
correctly for the sibling `demon`/`commander-effect` kinds (confirmed: `demon` kind count is
unaffected by adding species — `test_species_entries_add_onto_an_existing_corpus_without_
disturbing_it`).

### Acceptance 2 — "A resumed demon-species generation run backfills only genuinely
missing/stale entries"

Demons' own real, live, already-shipped resumability (`adapters/demons/run/runner.py`'s `start()`)
...
`Content/FieldStale` correctly reports nothing for demons today: no `flavor` field exists yet to
...
python -m pytest tools/seedsmith/tests/pipeline/test_staleness.py tools/seedsmith/tests/test_content_completeness.py \
  ...
  tools/seedsmith/tests/test_demons_completeness.py tools/seedsmith/tests/test_anchor_emit.py \
  tools/seedsmith/tests/test_adapter_demons.py -q
```
```
65 passed in 2.98s
```
(Confirms Phase 0's own Core suite plus this phase's own new tests plus the pre-existing demons
...
1. `git status` at the time of this run shows active, uncommitted, concurrent modification of
   ...
   demon-theme/action-pairing counts) in exactly these domains — the same class of drift Checkpoint
...
2. Grep-confirmed: none of the 15 failing test files import `content_completeness`,
   `pipeline.staleness`, `pipeline.backfill`, `adapters.demons.completeness`, or
   `language_consistency` — and this task's only code edit (`report/cli.py`'s `cmd_check`) is
   gated behind `args.adapter == "demons"`, an additive branch that cannot affect the
...
`test_demon_themes.py` (×2), `test_distribution_planner.py` (×3), `test_dungeon_registries.py`,
...
- [x] Task 9: names exactly which demon-species field counts as description/flavor — answer:
...
- [x] Task 10: a resumed demon-species generation run backfills only genuinely missing entries —
...
- [x] Demon-adapter test suite green (65/65 across Core + demons-specific suites); full suite run
