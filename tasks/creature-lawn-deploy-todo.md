# Task list: creature lawn deploy

Plan: [creature-lawn-deploy-plan.md](creature-lawn-deploy-plan.md). Source prerequisites:
[creature-progression-todo.md](creature-progression-todo.md). Specs:
[docs/architecture/creature-lawn-deploy-map.md](../docs/architecture/creature-lawn-deploy-map.md),
[docs/architecture/creature-lawn-deploy/](../docs/architecture/creature-lawn-deploy/) (`spec-lawn-deploy-core.md`,
`spec-lawn-deploy-progression.md`, `spec-lawn-deploy-events.md`, `spec-zomboss-deploy-ai.md`), plus
the source/isolation specs listed in [creature-progression-todo.md](creature-progression-todo.md).

Dependency order: source prerequisites D0-D2 (`creature-progression-todo.md`) → Phase 1
...
- **Commander refusal NOT built — a real, investigated finding, not a skip.** `CommanderId`
  (`Core/Commanders/CommanderId.cs`) is a fixed Dave/Zomboss enum with no creature-instance binding
...
- Acceptance:
  ...
  - [x] Deploying any other owned creature specimen is unaffected — no false-positive refusal.
  ...
  - [ ] ~~Deploying the current default Commander's own `instanceId` refuses~~ — inapplicable today, see
        above. Re-open this line once a real creature-to-Commander binding exists.
- Verify: `dotnet test tests/FusionRpg.Data.Tests --filter CreatureLawnDeployCommanderRefusal` — **39/39
  passing**, confirmed live (3 new tests + the full existing `PatronStoreTests`/`UniqueActorStoreTests`
  suites, zero regressions).
- Files: `RpgStore.UniqueActors.cs` (edit), `tests/FusionRpg.Data.Tests/CreatureLawnDeployCommanderRefusalTests.cs` (new).

### T1.2 — Reconciled trait-binding on every deploy · **M** · 3-4 files — **DONE 2026-09-06**

- `src/FusionRpg.Data/Sqlite/RpgStore.UniqueActors.cs` — new `ReconcileCreatureTraitBindingsUnlocked`,
...
- Acceptance:
  ...
  - [x] `AtomPushService`'s output for that owner includes exactly the specimen's current trait
        containers — **DONE 2026-09-07**: `tests/FusionRpg.Server.Tests/CreatureLawnDeployAtomPushTests.cs`
        ...
        **A real bug in this test's own expectation was then found and fixed by a full-suite regression
        ...
        `dotnet test tests/FusionRpg.Server.Tests --filter CreatureLawnDeployAtomPush` AND a full-suite
        ...
        already-documented `vocabulary.json` defect, none touching creatures/species/this module's diff).
- Verify: `dotnet test tests/FusionRpg.Data.Tests --filter CreatureLawnDeploy` — **4/4 passing**. Full
  ...
  ladder seeding), and `CreatureSpeciesImportCliTests.A_stale_committed_file_refuses_the_whole_import_and_
  writes_nothing` failed only in the full run with a subprocess "build failed" message (it shells out to
  `dotnet run --project tools/CreatureSpeciesImport`, racing a concurrent session's own in-progress edit) —
  re-run in isolation immediately after: 2/2 passing.
- Files: `RpgStore.UniqueActors.cs` (edit), `tests/FusionRpg.Data.Tests/CreatureLawnDeployTests.cs` (new).
...
- Acceptance:
  - [x] **Confirmed no creature-scale magnitude reaches `AtomCompiler`'s risk-relevant surface via T1.2's
        own path.** Trait grants use `ProduceAndBind`'s fixed `CreatureTraitAtomPinTheta` (=20, the same
...
- **A second, real finding emerged from the same investigation**: this codebase already tried to solve
  ...
  now refuses `HypnoAlly`-mode creatures outright (`"deploy.hypno-ally-not-implemented"`) rather than
  attempting an unverified native hypnotize call. `PlantAvatar`-mode creatures (confirmed the common case)
  deploy normally, side/typeId unchanged.
- **A real regression found and fixed while verifying this**: `CreatureLawnDeployCommanderRefusalTests.cs`
  and `CreatureLawnDeployTests.cs` (T1.1/T1.2's own tests) both picked their test species via
  ...
  their own tests. Fixed by excluding `DeployMode != CreatureDeployMode.HypnoAlly` from both files' own
...
- Acceptance:
  - [x] A `PlantAvatar`-mode creature deploys normally — side/typeId exactly match the species' own values,
        confirmed via a real committed species (`abyssswordstar`).
  - [x] A `HypnoAlly`-mode creature refuses deploy with `"deploy.hypno-ally-not-implemented"`, leaving the
...
- Verify: `dotnet test tests/FusionRpg.Data.Tests --filter CreatureLawnDeploy` — **10/10 passing** (3 new
...
- **Full `FusionRpg.Data.Tests` regression run (2026-09-06/07) found the SAME species-selection defect
  ...
  `PvZ_deploy_accepts_a_bound_creature` the same way. All three fixed identically — added
  `&& s.DeployMode != CreatureDeployMode.HypnoAlly` to each file's own species selection, same narrow
...
- Files: `RpgStore.UniqueActors.cs` (edit — the refusal, in `TryBeginUniqueDeploy`),
  `tests/FusionRpg.Data.Tests/CreatureLawnDeployHypnoRefusalTests.cs` (new, 3 tests),
  `CreatureLawnDeployCommanderRefusalTests.cs` + `CreatureLawnDeployTests.cs` + `ExpeditionStoreTests.cs` +
...
- A 6-question investigation (species-magnitude storage, equipment's `Absolutes`, the expedition
  ...
  `ConcreteSpeciesSeedReader.ToCreatureSpeciesDef` mapper seam (same seam `TraitPool` needed curation at);
...
- **Built**: `CreatureSpeciesDef.Magnitudes` (new field, `CreatureSpeciesCatalog.cs`), threaded through
  `ConcreteSpeciesSeedReader.ToCreatureSpeciesDef` (one line — magnitudes are already the closed,
  channel-shaped vocabulary, so unlike `TraitPool` this is a straight pass-through, no curation layer
  needed). New `RpgStore.UniqueActors.cs` method `ReconcileCreatureMagnitudeBindingsUnlocked`, mirroring
  `ReconcileCreatureTraitBindingsUnlocked`'s exact diff shape (singleton wanted-set: a specimen's species
...
- Acceptance:
  - [x] A deployed creature's live combat stats (not just its trait effects) measurably differ from a
        vanilla `type_id` unit's defaults, traceable to the specimen's own magnitudes — proven at the
        `effect_binding`/mechanism level (`CreatureLawnDeployMagnitudeTests.cs`); the live-board
...
- Verify: `dotnet test tests/FusionRpg.Data.Tests --filter CreatureLawnDeployMagnitude` — **3/3 passing**
  ...
  including a new direct `ToCreatureSpeciesDef` magnitude-passthrough test AND the pre-existing real
  829-species diff test (`BuildCreatureSpeciesSnapshot` vs. the injector's own mapper path) unaffected.
  ...
  `BattleStatComposer.Configure` precondition — none touch `CreatureSpeciesDef`/`ConcreteSpeciesMapper`/
...
- Files: `CreatureSpeciesCatalog.cs` (edit, new field), `ConcreteSpeciesSeedReader.cs` (edit, 1 line),
  `RpgStore.UniqueActors.cs` (edit, new reconcile method + wiring),
  `tests/FusionRpg.Data.Tests/CreatureLawnDeployMagnitudeTests.cs` (new, 3 tests),
  `tests/FusionRpg.Core.Tests/Creatures/ConcreteSpeciesSeedReaderTests.cs` (edit, +1 test).

### T1.6 — Live E2E proof · **S**, grew to **M** · 3 files — **CI + prior live-lawn proof DONE; current provenance re-probe pending**

- Extended `StorageE2ETests.cs`'s own `deploy → ack → assert ActiveBound` pattern to a real creature
  specimen minted through the real `/api/test/mint-creature` seam (`FusionEndpoints.cs`'s own existing
  test-only helper), picking a species by property (`GET /api/creatures/catalog`'s own `deployMode`/
  `captureOnly` fields) rather than a hardcoded id — same lesson `CreatureLawnDeployHypnoRefusalTests.cs`
...
- **Major pre-existing, suite-wide regression found and fixed along the way, unrelated to this
  ...
  (`catalog-runtime`'s 2026-09-05 flip to `CreatureSpeciesCatalog.Configure(store.
  BuildCreatureSpeciesSnapshot())`) throws on an empty roster, and a fresh E2E `DataDir` had NEVER had
  ...
  marker `Directory.Exists(data/generated/creatures)` false-positived on an EMPTY copy of that folder
...
- Acceptance:
  - [x] E2E test passes in CI — **2/2 passing** for this program's own new file
        (`CreatureLawnDeployE2ETests.cs`), and the pre-existing `StorageE2ETests.cs` baseline (7/7) that
  ...
  - [x] A real, owner-observable live-lawn check confirms the same — **DONE 2026-09-07**. Rebuilt +
        ...
        this instance — `slot=critical-hunter, source=creature-trait` — proving T1.2's reconcile fired for
...
- Verify: `dotnet test tests/FusionRpg.E2E.Tests --filter CreatureLawnDeployE2E` — 2/2;
...
- Files: `tests/FusionRpg.E2E.Tests/CreatureLawnDeployE2ETests.cs` (new, 2 tests),
  `tests/FusionRpg.E2E.Tests/RpgApiFactory.cs` (edit — the species-seeding fix, affects every E2E test).

### ✅ Checkpoint 1 — a creature can really deploy and really fight as itself — **CLOSED 2026-09-07**
...
- [x] `FusionRpg.Data.Tests`/`FusionRpg.Core.Tests`/`FusionRpg.E2E.Tests` full suites — **not literally
      ...
      already-memory-documented expedition-golden drift — none touch creatures/species), `E2E.Tests`
...
- [x] A real live-lawn check: deploy a real creature, observe it on the board with its own stats/traits —
...
- Mirrored `commander-surface-map.md`'s own `MatchCommanderSnapshot`/`MatchCommanderSnapshotHolder`/
  ...
  by background REST refresh (`GET /api/creatures/{playerId}` + `GET /api/patron/{playerId}`, Patron
  ...
  (`CreaturesUpdated`/`patron.aura` — see below), `MatchHost.Apply`'s own `isStart`/`isEnd` branches wired
  to `BeginMatch`/`EndMatch` at the exact same board.start/board.end moment the commander snapshot uses.
- **Real gap found and fixed along the way**: `CreaturesUpdated`/`PatronUpdated` (the server's own
  ...
  that actually change ROSTER MEMBERSHIP or PATRON status (`CreatureEndpoints.cs`'s `/summon`,
...
- Files: `src/FusionRpg.Core/Match/LawnDeployRosterSnapshot.cs` (new),
  ...
  `src/FusionRpg.Server/CreatureEndpoints.cs` + `FusionEndpoints.cs` (edit, InjectorGroup broadcasts),
...
- Acceptance:
  - [x] Scenario table, one fact per named case × {fires, does-not-fire-below-threshold,
        ...
        cross-cutting gates (no eligible creature ever refuses regardless of board state; a per-run budget
...
- **FE half DONE 2026-09-07**, built after a dedicated investigation (an Explore agent — SignalR
  ...
  - `LawnPage.tsx`: a new inline `Banner` (mirroring the T22 banner's own exact JSX shape/`data-testid`
    convention) offering one button per eligible creature (resolved via the two-hook join the investigation
    found — `useCreatureRoster(playerId)` → `profile.speciesId` → `useSpeciesIndex()` → `TypeIcon`/name,
    the same recipe `CreaturesPage.tsx` already uses) plus a Dismiss button. Accept calls the ALREADY-
...
- Acceptance:
  ...
  - [x] Accepting the prompt calls the T1.x deploy path for the player's own chosen eligible creature —
...
- [x] A real live-lawn check: play to a triggered case, see the prompt, deploy an owned creature through it
...
1. **Root cause of the original silent failure — a real cache-miss race, found and fixed.** The
   ...
   - **Fix**: `LawnDeployRosterSessionCache.LastBuildWasCacheMiss` (new, mirrors
     ...
     landed (distinct from a real player genuinely owning zero eligible creatures, which also returns an
...
4. **A real, separate frontend infra issue found, diagnosed, and worked around transparently — not
   ...
   question, not a `creature-lawn-deploy` defect (every OTHER event kind on this page would be equally
...
5. **The click-through found a THIRD real bug — this one squarely in this task's own scope, fixed.**
   Replaying `8dceda95`'s real fire (11 real eligible creatures) rendered the banner correctly (11 real
   ...
   - **Fix**: the eligible-roster `.Where(...)` now also excludes any specimen whose species resolves
     to `CreatureDeployMode.HypnoAlly`, via an in-process `CreatureSpeciesCatalog.Get(...)` lookup — zero new
     REST calls, since `CreatureSpeciesCatalog.Configure(...)` already runs on this same injector process
...
- **Design, resolved by investigation, not guessed** (an Explore agent read `IWorldView`, `SpecimenOwnershipOracle`,
  ...
  - `IOwnSideOracle.RelationOf` returning null (an unregistered ptr — a raw vanilla PvZ unit, never a
    unique creature) resolves to `RelationKind.Enemy`, not silently dropped or defaulted to Self/Ally —
...
### T3.2 — Zomboss's own creature roster/pool · **M** · 2 files — **DONE 2026-09-07**
...
- **Design, resolved by investigation** (an Explore agent read the real summon roller, `CreatureAcquisition`,
  ...
  - **"The same pool the player draws from" is the EXACT predicate, not the nearest-sounding one.**
    ...
    `s.Acquisition.HasFlag(CreatureAcquisition.Summonable)` — a materially different filter from the
  ...
  - **A real "threat band" exists but is unreachable — named explicitly, not silently worked around.**
    `data/tuning/creature-threat.v1.json`'s own per-species threatBand is consumed into a numeric/derived
    field and discarded during species generation (`SlotFilter.cs`'s own doc comment, confirmed) —
    `CreatureSpeciesDef` (the live catalog the summon roller and this task both read) has no `ThreatBand`
    field at all. Rather than inventing a second, parallel threat scale reachable from this catalog, or
    quietly filtering by nothing, `BaseRarity` (already on `CreatureSpeciesDef`, already the summon-gacha
...
- A deterministic scorer over `ILawnBoardView` (T3.1) + the T3.2 roster, choosing whether and which
  creature to deploy. Weights come from the T3.2 tuning file. Derives its own randomness via
...
- **Design**: two-stage, not one blended score — matches Assumption 2's own "deterministic scorer, not a
  ...
  broken by `SpeciesId` ordinal — matching `CreatureRecipeCatalog`'s own established tie-break. Named
...
- Acceptance:
  - [x] Scenario table: no eligible creature declines
        (`No_eligible_creature_declines_regardless_of_board_state_or_roll`); exactly one is picked
...
- **A second real bug found proactively, before it could ever fire live**: T3.2's own roster never
  excluded `CreatureDeployMode.HypnoAlly` — the exact same class of gap Checkpoint 2 found and fixed for
...
- **Server-side** (`RpgStore.ZombossDeploy.cs`, new): `EnsureZombossPlayer()` (idempotent find-or-create)
  ...
  (`MintCreature`, `SummonRoller.RollTraits` — the SAME shared trait-roll summons and wild joins already
...
**Phases 1–3 establish the deploy/trigger/AI path**: a player-owned unique creature can deploy onto the live
...
4 and the companion `creature-progression-todo.md` checkpoint pass.
...
- **Acceptance:**
  ...
  - [x] `creature.progression.v1` source claims parse through the closed source contract and are retained on
...
- **Acceptance:**
  ...
  - [x] A unique and general creature sharing a type/species never cross-credit XP or allocation.
