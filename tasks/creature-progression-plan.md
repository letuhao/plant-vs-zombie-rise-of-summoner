# Implementation plan: creature progression sources
...
Covers the three approved cross-cutting progression specs under `docs/architecture/creatures/`:
...
The lawn-specific consumer is planned in [creature-lawn-deploy-plan.md](creature-lawn-deploy-plan.md), which
depends on this initiative.

## Overview

Make progression source selection explicit at every creature spawn and composition boundary. A gameplay
...
- `pvz_activity_facts.source_kind/source_id` carry the typed claim using the `creature.progression.v1`
...
D0 source contract + provenance parser
  ...
  └── D2 dedicated progression isolation
          └── lawn-deploy-progression (creature-lawn-deploy plan Phase 4)
```
...
Change general-creature species XP and allocation consumers to require `EmpireGeneral`. Preserve the
...
|---|---|---|
| A same-type unique is classified as a general creature | High | Persist and parse a typed source claim; no type/side inference; add same-type cross-credit tests |
