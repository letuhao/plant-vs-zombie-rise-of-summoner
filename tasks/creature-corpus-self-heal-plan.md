# creature-corpus-self-heal: implementation plan

## Context

The full creature-species classification run completed (833 species indexed). A new reusable audit
tool, `tools/CreatureQualityReport`, scanned the whole corpus across four dimensions — classification
...
3. **68 species (81‰) unresolved rarity, 29 (34‰) unresolved aptitude, 4 unresolved element, 11
   unresolved threatBand** — genuine 3-way vote splits, per `creatures metrics`'s own signal.
...
- **Phase C spends real model calls and runs in the background** (matching this session's own
  established pattern for `creatures run start/resume` — a harness-tracked background process, driven
...
- [ ] **A1: fix the stale-duplicate write bug** · **S** · 1-2 files
  ...
  - Files: `tools/seedsmith/seedsmith/adapters/creatures/run/runner.py`, its test file.
...
- [ ] **A2: one-time cleanup of the 217 existing duplicates** · **S** · 0 new files (a driver script, not committed)
  ...
  - Acceptance:
    - [ ] `CreatureQualityReport`'s duplicate count reads 0 afterward.
    - [ ] No species is lost — same total distinct species count before and after.
  - Verify: re-run `dotnet run --project tools/CreatureQualityReport`, read Section 1.

### ✅ Checkpoint A
- [ ] `python -m pytest` (seedsmith) full suite green.
- [ ] `CreatureQualityReport` Section 1 shows 0 duplicates, 0 on-disk-not-indexed (or a named, real
...
- [ ] **B1: `run-control` gains a pipeline-scoped rerun mode** · **M** · 3-4 files
  ...
  - Acceptance:
    - [ ] `creatures run rerun --pipeline kit-shape --all` re-executes ONLY kit-shape for every
...
- [ ] **C2: redeploy kit-shape corpus-wide** · owner-adjacent, real model cost
  - `creatures run rerun --pipeline kit-shape --all` (Phase B's mechanism) — ~833 calls.
  - Acceptance: `CreatureQualityReport`'s attackTempo entropy rises meaningfully above 0.00.

- [ ] **C3: self-heal every currently-unresolved field** · real model cost
  - `creatures run rerun --unresolved` (selector already exists, no new code) — full 8-pipeline
  ...
  - Acceptance: unresolved rates in `CreatureQualityReport` Section 1 drop; species that resolve
    become generatable (Section 3 failure count drops correspondingly).

### ✅ Checkpoint C
- [ ] Both reruns completed (state=completed, not failed) — `creatures run status` checked, not
  assumed from "the command returned."

### Phase D — verification

- [ ] **D1: before/after report** · **S**
  - Re-run `CreatureQualityReport --json <path>` after Phases A-C; diff against the pre-heal snapshot
...
| Phase C's real model calls hang or the process dies mid-run | Low (checkpointed per species already) | Same background-run + `creatures run status`/`resume` discipline used for the original full run |
...
cd tools\seedsmith
python -m pytest -q
python -m seedsmith creatures run rerun --pipeline kit-shape --all
python -m seedsmith creatures run rerun --unresolved

cd ..\..
dotnet run --project tools/CreatureQualityReport -- --json data/generated/creatures-quality-after.json
```
