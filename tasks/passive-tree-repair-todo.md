...
## Phase 1 — corpus self-consistency (drift + orphan + provenance)
...
### P1.2: Resolve the `wither` orphan node
**Spec:** repair skill §6. **Description:** `skill.wither-def-t9-n1` exists in
`data/generated/passive-tree/wither.json` but in no seed document (seed 39, generated 40). Find
whether the seed lost a node, the generated file predates a seed edit, or a re-generation was
partial — then make seed and generated agree through the pipeline.
**Acceptance:**
- [ ] Root cause named; no unrecorded divergence remains
- [ ] A check exists that fails when a generated node has no seed document node (envelope test, no count)
...
| G1 | P1 | `FamilyExpandGen --check` 0; no orphan; uniform current vintage |
