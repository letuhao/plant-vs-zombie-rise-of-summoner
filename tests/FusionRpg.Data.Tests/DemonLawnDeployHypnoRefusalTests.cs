...
/// <summary>demon-lawn-deploy T1.4: owner-confirmed 2026-09-06 — a demon's own species side/typeId
...
public class DemonLawnDeployHypnoRefusalTests : IDisposable
{
    readonly string _dir;
    readonly RpgStore _store;

    public DemonLawnDeployHypnoRefusalTests()
    {
        _dir = Path.Combine(Path.GetTempPath(), "fusionrpg-hypno-refusal-" + Guid.NewGuid().ToString("N"));
    ...
    // DemonSpeciesCatalog from the compiled default (not the anchor-pipeline corpus), and a hardcoded
    ...
    static readonly DemonSpeciesDef HypnoSpecies = DemonSpeciesCatalog.All
        .First(s => s.DeployMode == DemonDeployMode.HypnoAlly && s.Acquisition != DemonAcquisition.CaptureOnly);
    static readonly DemonSpeciesDef AvatarSpecies = DemonSpeciesCatalog.All
        .First(s => s.DeployMode == DemonDeployMode.PlantAvatar && s.Acquisition != DemonAcquisition.CaptureOnly);
    ...
    {
        Assert.Equal(DemonDeployMode.HypnoAlly, HypnoSpecies.DeployMode);
        Assert.Equal(DemonDeployMode.PlantAvatar, AvatarSpecies.DeployMode);
    }
