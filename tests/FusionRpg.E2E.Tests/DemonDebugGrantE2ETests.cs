...
/// `/api/demons/debug/grant` reuses the same `RpgStore.MintDemon` atomic path every other mint source
/// (summon, fusion, capture, delve) already goes through — this is a test-coverage seam, not a second
/// mint implementation.</summary>
[Collection("e2e")]
public class DemonDebugGrantE2ETests : IAsyncLifetime
{
    readonly HttpClient _http;

    public DemonDebugGrantE2ETests(RpgApiFactory factory)
    {
    ...
    {
        var before = await _http.GetFromJsonAsync<JsonElement>("/api/demons/1/summon-state");
        var balanceBefore = before.GetProperty("balance").GetProperty("balance").GetInt64();

        var grant = await _http.PostAsJsonAsync("/api/demons/debug/grant",
        ...
        var after = await _http.GetFromJsonAsync<JsonElement>("/api/demons/1/summon-state");
    ...
    public async Task Atoms_preview_compiles_for_a_real_instance_id()
    {
        ...
        var preview = await _http.GetFromJsonAsync<JsonElement>($"/api/demons/debug/atoms-preview/{instanceId}");
    ...
    {
        var preview = await _http.GetAsync("/api/demons/debug/atoms-preview/not-a-real-instance");
    ...
    {
        // gameTypeId 4 == potatomine (earth/fire) in the real catalog. A bare UniqueActor (no demon
        // profile) sidesteps the demon-contracts deploy gate entirely, matching
        // AtomPushServiceInstanceOwnerRewriteTests's own CreateUniqueActor + deploy + ack shape.
        var spawn = await _http.PostAsJsonAsync("/api/demons/debug/spawn-unique-actor",
        ...
        var bind = await _http.PostAsJsonAsync($"/api/demons/debug/grant-test-atom/{instanceId}", new { });
