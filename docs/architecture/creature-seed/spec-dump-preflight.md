...
**Module id:** `dump-preflight` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 5 of 16
...
|---|---|---|---|
| 1 | dump exists | `data/seed/creatures/_dump/_manifest.json` present and parses | ask: run `CreatureCorpusDump`? |
...
| 4 | contract audits clean | `creatures contract --audit` exits 0 | refuse — a numeric field in the schema poisons every downstream row |
...
| 8 | tuning present | `creature-threat.v1.json` loads and validates | refuse |
...
On pass it writes `data/seed/creatures/_dump/_preflight.json`: the dump hash it validated, the model id
...
```powershell
python -m seedsmith creatures preflight                  # human-readable, exit 1 on any refusal
python -m seedsmith creatures preflight --json           # machine-readable, for run-control
python -m seedsmith creatures preflight --skip-model     # checks 1-4, 7-9 only; never allowed before a real run
```
...
```text
tools/seedsmith/seedsmith/adapters/creatures/preflight.py   the checks - committed, CI-visible
