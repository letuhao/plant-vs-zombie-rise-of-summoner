...
**Module id:** `species-generator` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 12 of 16
...
interval, written to `data/generated/creatures/` as committed, diffable rows.
...
```text
data/seed/creatures/species/**.json  ->  species-generator  ->  data/generated/creatures/**.json
```
...
item generator at creatures" — there is no item generator to point.

**Corrected 2026-09-05 — this module has shipped.** `data/generated/creatures/` holds **830 committed JSON
files**, one per concrete species, all tracked (counted this session; first committed 2026-09-04 in
`52713f4`), and the generator is `src/FusionRpg.Core/Creatures/Generation/` —
`CreatureSpeciesGenerator`, `SpeciesExpander`, `ConcreteSpeciesSerializer`, `CreatureCorpusBuilder` — with the
...
```powershell
dotnet run --project tools/CreatureSpeciesGen -- --seed data/seed/creatures/species --out data/generated/creatures
dotnet run --project tools/CreatureSpeciesGen -- --check
dotnet run --project tools/CreatureSpeciesGen -- --explain <speciesId>   # every derivation step, shown
...
```text
tools/CreatureSpeciesGen/Program.cs                            arguments and report
src/FusionRpg.Core/Creatures/Generation/SpeciesExpander.cs     the derivation, testable
src/FusionRpg.Core/Creatures/Generation/ConcreteSpecies.cs     the row shape
data/tuning/creature-shape.v1.json                             tempo/reach tables
data/generated/creatures/**                                    committed output
tests/FusionRpg.Core.Tests/Creatures/SpeciesGeneratorTests.cs
```
