...
**Module id:** `rarity-migration` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 11 of 16
**Model calls:** none. **This is the largest and riskiest non-runtime module in the program, and it
carries a data migration.**

## Objective

Migrate `CreatureRarity` from four values to the ten-rung ladder, update every consumer, migrate persisted
shard material ids, and reverse `ssot-rarity.md` §4.3.

Owner, Q4: *"Adopt the full 10-rung item ladder for creatures."*
Owner, Q24: *"Migrate CreatureRarity to 10 values now."*
...
> *"**Creatures keep their own ladder.** `CreatureRarity` stays a four-value code enum, and a one-way band
> map lets the two be compared without being fused … The map exists so a legendary creature's drop table
...
rewrite to record that creatures adopted the ladder on 2026-09-01, why, and that the four-row band map
...
| `Creatures/CreatureMaterialCatalog.cs:18-19` | enumerates all four; composes `shard.{rarity.ToId()}` |
| `Creatures/Fusion/FusionRoller.cs:77` | `SlotsFor` switch, four arms |
| `Creatures/Fusion/StarPolicy.cs` | `StarCap`, and promotion gated on `Legendary` being the top |
| `Creatures/Fusion/FusionTuning.cs` | `StarCap` / `RecipeCost` dictionaries keyed by rarity; `ParseRarity` from strings |
| `Creatures/Contracts/ContractTuning.cs` · `ContractPolicy.cs` | `BaseUpkeepPerDay`, `RitualPriceSouls` keyed by rarity |
| `Creatures/SoulEarnPolicy.cs:43` · `SoulEarnTuning.cs:16` | `DiscoveryDelta` keyed by rarity |
| `Creatures/SummonRoller.cs:21` · `SummoningTuning.cs:8` | rates 74/20/5/1 and both pity thresholds |
| `Creatures/Fusion/CreatureRecipeCatalog.cs:50,60` | **ordinal arithmetic and a relational comparison — see §3** |
...
`CreatureRecipeCatalog.cs:60`:

```csharp
(CreatureRarity)((int)output.BaseRarity - 1)
```
...
`CreatureRecipeCatalog.cs:50` — ten lines earlier in the same method — is the other half:

```csharp
.Where(s => s.BaseRarity >= CreatureRarity.Rare && s.Acquisition != CreatureAcquisition.CaptureOnly)
```
...
They are found by grepping for two shapes — casts to or from `CreatureRarity`, and comparisons against a
...
`CreatureMaterialCatalog.cs:19` composes `shard.{rarity.ToId()}`, and those ids are **stored in player
...
These do not mention `CreatureRarity` and are invisible to every grep for it. They would survive the
...
recorded first, and this locks the creature economy's units.
...
```text
src/FusionRpg.Core/Creatures/CreatureRarity.cs              ten values, ToId() -> ladder ids
src/FusionRpg.Core/Creatures/CreatureRarityLadder.cs        OneRungBelow / RungsBelow, the named helpers
...
// four. CreatureRecipeCatalog used a bare (int)r - 1 and meant "a quarter of the ladder";
// after the widening the same expression silently means a tenth. Name the intent.
public static CreatureRarity RungsBelow(CreatureRarity r, int rungs)
```
...
| Test | Asserts |
|---|---|
| `no_bare_cast_between_int_and_CreatureRarity_outside_the_ladder_helper` | a guard test; the §3 landmine cannot come back |
| `no_relational_comparison_against_a_named_CreatureRarity_member` | the second half of §3 — `>= CreatureRarity.Rare` cannot return |
...
- [ ] A guard test forbids bare int↔`CreatureRarity` casts outside the ladder helper.
