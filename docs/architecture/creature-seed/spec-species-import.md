...
**Module id:** `species-import` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 13 of 16
**Model calls:** none.

## Objective

Load `data/generated/creatures/**.json` into SQLite in **one all-or-nothing transaction** — or write
...
**The same trap is now set for creatures.** `data/seed/creatures/species/**` is a third format under the same
root. Two consequences, both required:

1. `SeedScanner`'s owned-folder list is **explicit**, and creature anchors are **not** added to it — the
   atom importer must keep ignoring them.
2. This module reads `data/generated/creatures/`, not `data/seed/`, which sidesteps the collision
...
| uniqueness | `speciesId` and `creatureTypeId` are unique; `creatureTypeId >= CreatureTypeIdFloor` | refuse |

The `creatureTypeId` floor is `CreatureSpeciesCatalog.CreatureTypeIdFloor = 10_000`
(`CreatureSpeciesCatalog.cs:29`), and its stated reason is that *"web-battle events must never collide
...
So: the transaction and every statement live in `FusionRpg.Data`; `tools/CreatureSpeciesImport` holds
...
```powershell
dotnet run --project tools/CreatureSpeciesImport -- --db <server data dir>
dotnet run --project tools/CreatureSpeciesImport -- --db <dir> --check     # validate + roll back
...
```text
tools/CreatureSpeciesImport/Program.cs             arguments, report, exit codes
src/FusionRpg.Data/Sqlite/RpgStore.Species.cs   the transaction and every statement
src/FusionRpg.Core/Creatures/ConcreteSpeciesRow.cs the shape both sides agree on
...
| `duplicate_creatureTypeId_refuses` | the id space is busy now |
| `creatureTypeId_below_floor_refuses` | the PvZ collision guard |
| `reimport_is_row_identical` | idempotency by hash |
| `species_removed_upstream_is_deleted` | no accumulating union |
| `atom_importer_still_ignores_creature_seed_folders` | pins `SeedScanner`'s root list |
...
**Never:** partial import; SQL in `tools/`; add creature folders to `SeedScanner`'s roots; import a stale
...
- [ ] The atom importer still refuses to sweep creature seed folders.
