...
**Module id:** `fusion-recipe-generator` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 17 of 18 (new)
**Model calls:** yes, bounded to gap species only. **Depends on:** `species-generator` (11).

## Objective

Move fusion-recipe assignment out of `CreatureRecipeCatalog.Build()`'s in-process, no-backtracking
algorithm and into the same seed → concrete discipline every other creature-seed artifact already
...
fusion with deterministic engine read the whole creature species seed and make distribution index... fill
...
`spec-creature-fusion.md` (shipped 2026-08-21) locked recipe generation as **"code-generated from the
species catalog"** — a single deterministic pass, `CreatureRecipeCatalog.Build()`, assigning each
...
catalog for the first time, `CreatureRecipeCatalog.Build()` threw `InvalidOperationException: No unused
...
`tools/CreatureRecipeDistributionIndex`, §1 below — a first pass here misread the live database's raw
...
Reads every concrete species (`data/generated/creatures/*.json`, `species-generator`'s own committed
output — the same input `species-import` reads, never the live database). For every rarity rung at or
above `CreatureRecipeCatalog.OutputEligibilityFloor` (`Cultivated`):

- the output count at this rung (species with `Acquisition != CaptureOnly`)
- the nearest populated rung below (mirrors `CreatureRecipeCatalog.InputPoolBelow`'s own walk-down —
...
capacity allows (`C(n,2)` recipes, filled by the existing `CreatureRecipeCatalog` search order — reused
...
4. **Assign `inputA`/`inputB` from the resolved, unordered pair the same way `CreatureRecipeCatalog`
   ...
   way (`spec-creature-fusion.md` lock 5's "pick one guaranteed trait from any input" is symmetric), so
...
1. Runs the EXISTING `CreatureRecipeCatalog.Build()` algorithm exactly as shipped (`TryFindPair`'s
   ...
   `tools/CreatureRecipeDistributionIndex`, land inside this same deterministic pass, since `Build()` fills
...
2. For each deficit output, takes §2's per-member-voted proposal (if the vote resolved to a pair — an
   ...
   - both `inputA`/`inputB` exist in the real roster (`CreatureSpeciesCatalog.IsKnown`)
   - neither is `Acquisition == CaptureOnly` (spec-creature-fusion.md's own owner lock 6, unconditional —
   ...
   - **the cross-rung relaxation is recorded, not hidden**: a reconciled recipe whose two inputs are
     NOT both at the single nearest-populated rung below the output (`CreatureRecipeCatalog`'s own default
...
output" property this whole program is built on (`creature-seed-map.md` §1's own law: *"the seed is
...
```text
data/generated/creatures/_fusion-recipes.json
```
...
this module's own reconciler can resolve on its own authority — `spec-creature-fusion.md`'s own Boundaries
...
rung-distance cost multiplier mirroring the promotion cost bump `spec-creature-fusion.md`'s own "Costs"
table already uses when a band jumps. Not this module's call; recorded so the call gets made.

## Commands

```powershell
dotnet run --project tools/CreatureRecipeDistributionIndex           # §1, deterministic, no model calls
python -m seedsmith creatures fusion propose                          # §2, model calls, only if §1 found a shortfall
python -m seedsmith creatures fusion reconcile                        # §3+§4, deterministic, writes the seed
python -m seedsmith creatures fusion reconcile --check                # compare against committed, exit 1 if stale
...
```text
tools/CreatureRecipeDistributionIndex/Program.cs         §1 — thin CLI over the real CreatureRecipeCatalog
                                                       search logic, reused not reimplemented
tools/seedsmith/seedsmith/adapters/creatures/fusion/
...
data/generated/creatures/_fusion-recipes.json             the committed seed
tools/seedsmith/tests/test_fusion_recipe.py
tests/FusionRpg.Core.Tests/Creatures/Fusion/FusionRecipeReconcileTests.cs
```
...
| `deterministic_pass_alone_matches_todays_CreatureRecipeCatalog_for_every_non_shortfall_output` | reconciliation does not change a single recipe that already worked |
...
direction was set — recorded here, and in `creature-seed-map.md` §5, so a future reader does not need to
reconstruct the authorization from chat history:

- `spec-creature-fusion.md`'s own Boundaries name *"Ask first: … recipe-graph shape changes"* —
  ...
  already on record.
- `spec-creature-fusion.md`'s design text states *"every summonable rare/epic/legendary species gets
...
- [ ] Every non-shortfall recipe is byte-identical to what today's `CreatureRecipeCatalog.Build()` already
