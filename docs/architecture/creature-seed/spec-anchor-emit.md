...
**Module id:** `anchor-emit` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 8 of 16
**Model calls:** none.

## Objective

Write the classified anchors to `data/seed/creatures/species/**.json` as **seed files** under
...
| Phase | Behaviour |
|---|---|
| **The one re-derivation** | every species, including the existing 84, is classified fresh. The old `CreatureCorpusEmit` corpus is superseded and its tool deleted |
...
`tools/CreatureCorpusEmit` is removed in this module, not earlier. It is the current producer of
`data/seed/creatures/creature/**`, and deleting it before its replacement emits is how a corpus goes missing
for a wave. Its 84-entry output is superseded by `species/**` and the old tree is removed in the same
change.

**`CreatureSpeciesCatalog.Generated.cs` is NOT touched here.** The runtime keeps reading it until
...
```powershell
python -m seedsmith creatures emit --dump data/seed/creatures/_dump
python -m seedsmith creatures emit --check              # exit 1 if the tree would change
python -m seedsmith creatures emit --stale              # list entries whose inputs moved
python -m seedsmith creatures emit --diff-legacy        # field-by-field vs the shipped 84
...
```text
tools/seedsmith/seedsmith/adapters/creatures/anchor/emit.py        writer + staleness
tools/seedsmith/seedsmith/adapters/creatures/anchor/provenance.py  the record
data/seed/creatures/species/**                                     committed output
...
**Ask first:** deleting `tools/CreatureCorpusEmit` (it is the current corpus producer); changing the file
...
corpus before the new one emits; touch `CreatureSpeciesCatalog.Generated.cs` in this module.
