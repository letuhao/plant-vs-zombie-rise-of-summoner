...
**Module id:** `anchor-contract` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 2 of 16
...
```powershell
python -m seedsmith creatures contract --print          # the resolved JSON Schema
python -m seedsmith creatures contract --audit          # the numeric audit, exit 1 on a finding
...
```text
tools/seedsmith/seedsmith/adapters/creatures/anchor/schema.py       the schema, built in code
tools/seedsmith/seedsmith/adapters/creatures/anchor/descriptions.py the prose, one constant per attribute
tools/seedsmith/seedsmith/adapters/creatures/anchor/audit.py        the numeric audit
...
Match the existing creature adapter: frozen sets for vocabularies, module-level constants in
