...
**Module id:** `classify-pipelines` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 7 of 16
...
in one score range is a signal to retune `creature-threat.v1.json`, which is a balance edit a human makes
...
|---|---|---|
| `posture-resource` | `posture` is derived from `aptitudePrimary`; a Bastion creature whose `resourceProfile` omits `poise` is incoherent | re-prompt pipeline 7 naming the conflict |
...
| `acquisition-nonzero` | `CreatureAcquisition.None` is a catalog error (`CreatureRarity.cs:11`) | re-prompt pipeline 6 |
...
```powershell
python -m seedsmith creatures generate --pipeline element-primary --species <id>   # one species, one pipeline
python -m seedsmith creatures generate --all                                        # the full run, via run-control
python -m seedsmith creatures generate --dry-run                                    # render prompts, call nothing
...
```text
tools/seedsmith/seedsmith/workflow/graphs/creature_anchor.py     the eight graphs
tools/seedsmith/seedsmith/adapters/creatures/anchor/prompts.py   prompt bodies, one per pipeline
