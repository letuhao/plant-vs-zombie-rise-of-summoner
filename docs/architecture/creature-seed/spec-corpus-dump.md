...
**Module id:** `corpus-dump` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 1 of 16
**Model calls:** none.
...
`tools/CreatureCorpusEmit/Program.cs:45-52`:

```csharp
var species = CreatureSpeciesCatalog.All;          // the 84 the C# generator emitted
...
**This module does not modify `CreatureCorpusEmit`.** That tool keeps serving the existing 84-species
...
- `JavaScriptEncoder` configured for CJK ranges so Chinese names are not `\uXXXX`-escaped — the same
  choice `CreatureCorpusEmit/Program.cs:1-5` already makes with `UnicodeRanges`
- `null` written explicitly, never omitted — an absent key and a null key must not hash differently

### 4. Layout

```text
data/seed/creatures/_dump/
...
```powershell
dotnet run --project tools/CreatureCorpusDump -- <server data dir> [output root]
dotnet run --project tools/CreatureCorpusDump -- <server data dir> --check   # exit 1 if the tree would change
...
```text
tools/CreatureCorpusDump/Program.cs        arguments, report, exit codes only
tools/CreatureCorpusDump/DumpWriter.cs     canonical serialisation + hash
src/FusionRpg.Core/Creatures/Generation/DumpEnvelope.cs   the record shapes, so tests can hold them
tests/FusionRpg.Core.Tests/Creatures/CorpusDumpTests.cs
data/seed/creatures/_dump/**               committed output
```
...
```csharp
// The whole table, not the catalog's opinion of it. CreatureCorpusEmit walked
// CreatureSpeciesCatalog.All and could therefore never see a species the C# generator
// had not already picked - the defect this tool exists to fix.
var rows = store.ListAlmanacSeed();
```

Match `CreatureCorpusEmit/Program.cs`: a comment says *why*, arguments and reporting stay in `Program.cs`,
...
| `Dump_covers_every_almanac_row` | exported count equals `ListAlmanacSeed().Count`, **not** `CreatureSpeciesCatalog.All.Count` — the regression test for the circularity defect |
...
- [ ] No SQL appears anywhere under `tools/CreatureCorpusDump/`.
