...
**Module id:** `roster-metrics` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 10 of 16
**Model calls:** none.
...
That is a `creature-threat.v1.json` retune, and this module is the instrument that says by how much. It
...
Every target above lives in `data/tuning/creature-roster-targets.v1.json`, not in code, so a finding is
...
```powershell
python -m seedsmith creatures metrics                  # the report
python -m seedsmith creatures metrics --gate           # exit 1 on any closed-loop finding
python -m seedsmith creatures metrics --grid           # the 21 x 12 occupancy matrix
python -m seedsmith creatures metrics --queue          # the open-loop review queue
...
```text
tools/seedsmith/seedsmith/metrics/creature_roster.py           the checks
data/tuning/creature-roster-targets.v1.json                    the declared targets
