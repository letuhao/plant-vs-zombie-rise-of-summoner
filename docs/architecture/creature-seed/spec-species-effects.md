...
**Module id:** `species-effects` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 15 of 16
**Model calls:** yes. **This is the module whose absence made every generated creature do nothing.**

## Objective

Turn each species anchor into a **`species-passive.{speciesId}` container seed** — the fixed core, the
affix pool, and the eligibility tags — so a generated creature has effects and not just a stat block.

Owner, 2026-09-01: *"we really miss these pipeline on our specie generator — we just generate creature
...
**`creature-seed` cannot finish on its own**, and this is the module where that becomes true. Stated here
...
```text
data/seed/creatures/species-effects/
...
```powershell
python -m seedsmith creatures effects --species <id>          # one species
python -m seedsmith creatures effects --all                   # via run-control
python -m seedsmith creatures effects --dry-run               # render prompts, call nothing
python -m seedsmith creatures effects --check                 # exit 1 if the tree would change
...
tools/seedsmith/seedsmith/adapters/creatures/effects/prompts.py   prompt bodies
tools/seedsmith/seedsmith/adapters/creatures/effects/schema.py    the container-seed schema
data/tuning/creature-species-effects.v1.json                      affinity->weight, core band, roll bands
data/seed/creatures/species-effects/**                            committed output
```
