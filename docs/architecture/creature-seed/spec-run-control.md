...
**Module id:** `run-control` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 9 of 16
...
`data/seed/creatures/_runs/<runId>.json`, committed **only for completed runs**; in-progress records live
...
```powershell
python -m seedsmith creatures run start --all
python -m seedsmith creatures run pause
python -m seedsmith creatures run resume
python -m seedsmith creatures run cancel
python -m seedsmith creatures run rerun --basis inferred
python -m seedsmith creatures run overwrite-all --confirm <token>
python -m seedsmith creatures run status          # state, progress, ETA from callsMade
```

## Project structure

```text
tools/seedsmith/seedsmith/adapters/creatures/run/machine.py    states + transitions, pure
tools/seedsmith/seedsmith/adapters/creatures/run/record.py     persistence
tools/seedsmith/seedsmith/adapters/creatures/run/selectors.py  the eight selectors
tools/seedsmith/tests/test_run_control.py
data/seed/creatures/_runs/**                                   completed records, committed
```
