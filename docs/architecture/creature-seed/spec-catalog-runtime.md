...
**Module id:** `catalog-runtime` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 14 of 16
**Model calls:** none. **The riskiest module in the program** — it changes how nine shipped call sites
get their data.

## Objective

Move `CreatureSpeciesCatalog` from a compiled static array to store-backed reads, so the roster the game
...
Verified by grep, 2026-09-01. Every one reads `CreatureSpeciesCatalog`:

| Site | Reads | Shape of the read |
|---|---|---|
| `Battle/WaveCatalog.cs:56` | `All.Where(BaseRarity == r)` | full scan, filtered, ordered |
| `Creatures/Fusion/CreatureRecipeCatalog.cs:49,59` | `All` twice | full scan |
| `Creatures/SummonRoller.cs:104` | `All` | full scan, pool build |
...
| `Data/Sqlite/RpgStore.Creatures.cs:34,39` | `IsKnown`, `KnownVariants` | validation |
| `Data/Sqlite/RpgStore.Fusion.cs:160,208` | `Get` | point lookup |
| `Data/Sqlite/RpgStore.Summons.cs:103,136` | `Get`, `All.Count` | point + count |
| `Server/CreatureEndpoints.cs:19` | `All.Select(...)` | full projection |
...
`CreatureSpeciesCatalog`'s surface — `All`, `Get`, `IsKnown`, `KnownVariants`, `CreatureTypeIdFloor` — stays
exactly as it is. What changes is where `_all` comes from (`CreatureSpeciesCatalog.cs:36`):
...
`CreatureSpeciesCatalog` is a **static class** whose roster is a compiled field:

```csharp
public static IReadOnlyList<CreatureSpeciesDef> All => _all ??= Validate(GeneratedSpecies);
```
...
`CreatureSpeciesCatalog` gains the same pair. `CreatureCorpusEmit/Program.cs` already documents what happens
...
Three catalogs build themselves from `CreatureSpeciesCatalog.All` in **inline static field initialisers**:

| Site | Field |
|---|---|
| `Battle/WaveCatalog.cs:29` | `public static readonly IReadOnlyList<WaveDef> All = Build();` |
| `Creatures/Fusion/CreatureRecipeCatalog.cs:19,21,24` | `All`, `ById`, `ByPair` |
| `Creatures/CreatureMaterialCatalog.cs:11,23` | `All`, `Known` |

`static readonly` initialised inline runs **once per process, at first touch of the type**. So:

1. **A reload after import cannot propagate.** `CreatureSpeciesCatalog` would hold the new roster while
   `WaveCatalog` and `CreatureRecipeCatalog` still hold recipes derived from the old one — a split-brain
...
2. **There is a static initialisation order hazard.** Whichever of the three is touched first triggers
   `CreatureSpeciesCatalog.All`. If `Configure` has not run, that is a throw at an arbitrary call site
...
(`CreatureSpeciesCatalog.cs:39`); it gains an empty-roster refusal that names the data directory and the
...
|---|---|
| `Creatures/Generation/CreatureSpeciesGenerator.cs` | **no production callers** — referenced only from `CreatureCatalogTests` |
| `Creatures/CreatureSpeciesCatalog.Generated.cs` | superseded by the imported roster |
| `tools/CreatureCatalogGen` | its output is the file above |
| `Creatures/Generation/CreatureCorpusBuilder.cs` | consumed only by `CreatureCorpusEmit`, deleted in `anchor-emit` |
| `tests/.../CreatureCatalogTests.cs` generator tests | they test a deleted generator |

**The catalog tests are not deleted wholesale.** The ones asserting *properties of the roster* —
element distribution, id uniqueness, `CreatureTypeId` floor — are retargeted at the loaded catalog. They
...
Before deleting `CreatureSpeciesCatalog.Generated.cs`, both sources exist. So:
...
2. Add `Configure` / `UseScoped` to `CreatureSpeciesCatalog`; every host gains its call.
...
```powershell
dotnet test tests/FusionRpg.Core.Tests --filter CreatureCatalog
...
```text
src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.cs        API unchanged; source swapped
src/FusionRpg.Core/Creatures/SpeciesSnapshot.cs            immutable snapshot + reload point
src/FusionRpg.Data/Sqlite/RpgStore.Species.cs           the read side (shared with species-import)
tests/FusionRpg.Core.Tests/Creatures/CreatureCatalogTests.cs  retargeted property tests + the diff test
```
...
static IReadOnlyList<CreatureSpeciesDef>? _all;
```
...
- [ ] `CreatureSpeciesGenerator`, `CreatureSpeciesCatalog.Generated.cs` and `tools/CreatureCatalogGen` are gone.
