...
**Module id:** `fusion-recipe-runtime` · **Program:** [creature-seed](../creature-seed-map.md) · **Build order:** 18 of 18 (new)
**Model calls:** none. **Depends on:** `fusion-recipe-generator` (17).

## Objective

Give `CreatureRecipeCatalog` the same store/file-backed seam `catalog-runtime` (module 13) already built
for `CreatureSpeciesCatalog`, so the committed `_fusion-recipes.json` seed — not an in-process
recomputation — is what the running game reads. Retires `CreatureRecipeCatalog.Build()`'s live algorithm
...
`CreatureSpeciesCatalog` reads a **database table** (`species-import` writes ~829 rows with real per-field
...
precedent, not `CreatureSpeciesCatalog`'s heavier DB-backed one. Reuse that pattern:

```csharp
public static class CreatureRecipeCatalog
{
    static IReadOnlyDictionary<string, CreatureRecipeDef>? _configured;
    public static void Configure(IReadOnlyDictionary<string, CreatureRecipeDef> recipes) => ...
...
1. Add `Configure`/`IsConfigured` behind the existing `All`/`Get`/`IsKnown`/`TryMatch` API — no call site
   changes (`Server/CreatureEndpoints.cs`, `RpgStore.Fusion.cs`, and every other real consumer of
   `CreatureRecipeCatalog` keep their existing code, per `spec-catalog-runtime.md`'s own "nine call sites"
...
3. A diff test: the store-backed recipe set, for every output NOT flagged `crossRungGapFill`, matches
   `CreatureRecipeCatalog.BuildDeterministicOnly()`'s own live computation exactly. This is
   ...
   **⛔ This proof is only valid if both sides read the identical species content — pin that
   ...
   `BuildDeterministicOnly()`'s comparison basis from the COMMITTED `data/generated/creatures/*.json`
   tree (its own §1). `CreatureSpeciesCatalog.All` at diff-test time, per `catalog-runtime` (module 13),
   ...
   against whatever `CreatureSpeciesCatalog.All` happens to hold at THAT moment, a non-gap-fill recipe can
   ...
   configures `CreatureSpeciesCatalog` (via `UseScoped`, the same test-isolation seam `catalog-runtime`
   already built) from the SAME committed `data/generated/creatures/*.json` tree the seed being tested was
   generated against, never from the live database, for the duration of the comparison.
4. Flip `Program.cs` from computing `CreatureRecipeCatalog.All` live to `Configure`-ing it from the
...
```powershell
dotnet test tests/FusionRpg.Core.Tests --filter CreatureRecipeCatalog
...
```text
src/FusionRpg.Core/Creatures/Fusion/CreatureRecipeCatalog.cs   API unchanged; source swapped; Build renamed
src/FusionRpg.Server/Program.cs                          gains the Configure call, beside the species one
tests/FusionRpg.Core.Tests/Creatures/Fusion/CreatureRecipeCatalogTests.cs   retargeted + the diff test
```

## Code style

```csharp
// Same discipline CreatureSpeciesCatalog.cs already states for itself: the API is unchanged on
...
| `the_diff_test_scopes_CreatureSpeciesCatalog_to_the_committed_tree_not_the_live_database` | pins §3 step 3's own precondition — a real, distinct check from the diff itself passing |
...
| `every_real_consumer_of_CreatureRecipeCatalog_works_unchanged` | one test per real call site, matching `spec-catalog-runtime.md`'s own per-consumer discipline |

## Boundaries

**Always:** keep the public API; fail loudly on a missing/stale seed; scope the diff test's
`CreatureSpeciesCatalog` to the exact committed tree the seed was generated against, never the live
database; prove the diff before flipping; run the live check before calling this done.

**Ask first:** changing any real consumer of `CreatureRecipeCatalog` (the design says none should need to).
...
- [ ] `CreatureRecipeCatalog.Build()`'s public surface is gone; only the generator's own CLI calls the
