# Ideal: creature mechanism gaps — hybrid typing, fusion inheritance, threat-driven difficulty

**Program:** facet of `creature-system` / `creature-seed`. **Phase:** idea — **all four open questions
...
**Predecessor:** [creature-seed-ideal.md](creature-seed-ideal.md) Q9 named two programs the full aspect
...
**2. Seed → concrete → per-player is binding for every creature generator.** Species *stats* are
...
`creature-seed-map.md` §3a closed for species generally, and it is the seam fusion inheritance (§3 below)
...
runtime by themselves (`creature-seed-ideal.md` §7.1, quoting `effect-atom/definitions.md` §0). "What
does this creature do" always decomposes to "which atoms does its container carry" — never to the
...
it has no permission to exist. This is the direct constraint on §4 below: a creature's own threat cannot
...
(`creature-seed-ideal.md` principle 4, §4.5) and it is why fusion's own existing `slotsByRarity`/
...
The owner's own framing: of the leftover creature-species mechanisms, exclude `creature-capture` and
...
creature distinctly itself, and both build on the identical atom-container substrate. The third,
**a creature's own threat contributing to world/encounter difficulty**, is a narrower, structurally
separate power-ladder question — bundled in on explicit request, not because it shares a mechanism
with the other two.

Two candidates were deliberately left off even though they touch creatures: the species-build content
...
Q9 (`creature-seed-ideal.md`) reverted `aspect-scope` — the design that would have let **one species
...
> *"revert aspect feature, original creature need original aspect, no element/status — that is better
...
multiplies the balance surface by N before a single creature is playable"* — the class-system's own
...
- **A creature species already carries `ElementSecondary` as real data** — nullable, on
  `CreatureSpeciesDef` (cited directly in `spec-aspect-scope.md`'s own code excerpt; confirmed live: **21
  of 841** real generated species carry a non-`none` secondary element today, measured this session).
- **Web-battle already reads and uses both elements for the creature's own stats.**
...
- **The lawn side already resolves both elements too.**
  `src/FusionRpg.Core/Creatures/LawnElementResolver.cs:78-89` builds a real `ActorElementTypes` from a
...
- **The atom/container layer gets dual-element effects for free.** Pool grouping defaults to
  ...
  (`creature-seed-ideal.md:1164-1166`, §7.2).
...
- **`hybrid.secondaryWeightMilli` defaults to 0.** `HybridPayload.cs:14-19`'s own docstring: *"Inert
  ...
  ask-first, and it moves the expedition goldens: wave creatures carry a real `ElementSecondary`
  (`WaveCatalog.cs:115`) even though the hand-built battle goldens do not."* This is the entire
  remaining distance between "a creature's secondary element already affects its own stats" (built,
  unconditional, §2.2) and "a creature's secondary element ever appears in its own attack" (inert until
...
- **Verified 2026-09-07 (was an open question — now resolved by tracing the code, not asked):** the
  ...
  *bigger* gap on lawn than on web-battle, not the same one. `HybridPayload.Build` is called from
  ...
  lawn path derives an attack's element weighting from an attacking creature's own
  ...
  an incoming hit's typed payload* — a genuinely different consumer from "this attacking creature's own
  ...
  creature's own lawn attack constructs its overlay), not a tunable flip — see the corrected Path 1 in
  §2.6.
- **`stat.derived` atoms are quarantined everywhere** — *"no opcode, no bag branch, no sink arm; battle
  ...
  wiring lands. A wiring gap on a scheduled path, not a wall"* (`creature-seed-ideal.md:1192-1195`,
  §7.4). Relevant here because it is the one atom kind a dual-typed creature's own passive would most
...
- **The aspect tier itself — one species fanning out into N differently-typed variants — remains
  ...
  typing doesn't exist": a single creature already having two fixed elements is (near-)fully wired
...
**SMT's alternative is worth naming for if the roster ever needs more than two slots.** Creatures carry
independently-authored per-element ranks — Weak / Resist / Null / Drain / Repel — never multiplied
together; a creature can be simultaneously Weak-Fire, Null-Ice, and Drain-Elec, and conflicts on one
...
shared `HybridPayload.Build(primary, secondary, weight)` into whatever constructs a creature's own lawn
...
- **A "pick one, roll rest" mechanism already shipped** (`spec-creature-fusion.md`, locked decision 5,
...
- **The real per-player roll mechanism already exists at the right layer.**
  `SpeciesMaterialiser.Materialise` (`src/FusionRpg.Core/Creatures/Materialise/SpeciesMaterialiser.cs:35-70`)
...
- **`InstanceProducer.Compose` (`src/FusionRpg.Core/Effects/Atoms/InstanceProducer.cs:28-82`) has no
  ...
  flip.
- **Fusion does not read a specimen's own materialised content at all today.** `spec-creature-fusion.md`'s
...
- **Three separable gates, all data-driven.** *How many transfer*: Persona 5's step function on
  ...
  whole system... they moved it exactly once"*). *Eligibility*: Nocturne's per-creature 9-category mask
  (median 4/9 allowed) versus Persona 5's shared 14×12 inheritance-type grid (129/168 = 77% allowed,
  a creature's *own* element always blocked) — the shared-grid shape is the cheaper-to-maintain lesson a
...
- **The series' own determinism arc never settled, which is itself the finding.** Nocturne shipped
  ...
  parent skill count. Vengeance's fix (2024) is Essence Fusion: a single-use captured per-creature
...
## 4. Mechanism C — a creature's own threat feeding world/encounter difficulty
...
- **`Θ`'s composition is closed, documented, and read in full for this document** —
  ...
  Zero species or creature term, confirmed by reading the formula, not assumed.**
- **A real, tuned, per-rung offset column already exists and is simply unconsumed.**
  `data/tuning/creature-threat.v1.json`'s `thetaOffset`: `nuisance=0 … calamity=40`, an evenly-stepped
...
- **Confirmed zero consumers anywhere in `src/`, by direct grep this session** — `threatBand` itself
  is discarded during species generation and unreachable from the live `CreatureSpeciesDef` catalog. This
  is not a small wiring flip like §2.3's tunable: nothing today has an opinion about *when* a creature's
  own threat should matter for difficulty. A creature becomes "content" in at least three structurally
  different contexts — Zomboss's own lawn deploy (shipped, `creature-lawn-deploy`, already has its own
  independent wave-gated rarity-ceiling roster, unrelated to `Θ`), a captured wild encounter
  (`creature-capture`, unbuilt), a Delve party's opposition (`party-dungeon`, shipped) — and each might
...
**If the real need turns out to be "many creatures → one encounter/zone difficulty number" rather than
"this one creature's own danger," two shipped genres solve that differently, and both are a materially
...
- **`party-dungeon`** (the Delve) — already shipped/partial; a creature's own threat rung would be the
...
Notably **not** named as a target: `creature-lawn-deploy` (which correctly keeps its own independent,
already-tuned difficulty curve, unrelated to `Θ`) and `creature-capture` (also out of this document's
scope, per §1). Which of the two shapes named in §4.3 — a direct per-creature `Θ_content` read, or an
...
| 4 | Species Θ offset — which consumer first? | **Owner decision** | Deferred and tracked; named future consumers are `party-dungeon` and world map events, not `creature-lawn-deploy` or `creature-capture` (§4.4). |
