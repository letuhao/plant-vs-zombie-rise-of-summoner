# Spec: `adapter-creatures`

Module `adapter-creatures` in the [seedsmith map](../seedsmith-map.md) §3b. Wave **D1**.
Depends on `corpus`, `creature-corpus-emit`.
...
Teach seedsmith what a creature is — and nothing else. Every module downstream (planner, briefkit,
...
and the full `items` suite stayed green throughout, re-run as part of `test_adapter_creatures.py`
itself so a future regression here is attributed correctly. Full reasoning for why this option was
chosen over a creatures-local dict or abusing `registries()`:
...
| Method | Creatures answer |
|---|---|
| `kinds()` | `creature`, `aspect`, `commander-effect`, `environment` — §2.2 |
...
|---|---|---|
| `creature` | — | The record `creature-corpus-emit` writes. References nothing; everything references it |
| `aspect` | `creatureId` | One species yields N aspects. Needs `aspect-scope`, **approved 2026-08-31** |
| `commander-effect` | `creatureId` | |
| `environment` | `creatureId`, `sectorId` | ⚠️ **Ships as a kind; nothing generates into it in v1** — A7 |

**Items and actions are deliberately absent.** Audit A3: `Corpus.load` is single-root, so a creature
"item" in this corpus would be a *different thing* from a real item — unequippable, and outside the
item corpus's own role/frame/affix rules. A creature is a **theme**; items and actions stay in their own
corpora and reference it (`creature-themes`, wave D4).

`reference_fields` is what `planner.ordering` reads to derive generation order (Kahn + Tarjan), so
declaring `creatureId` here is what makes creatures generate before everything that references them —
...
(`creature-corpus-emit` §9 Q1) until the taxonomy lands, rather than the adapter pretending to a
grouping it cannot yet supply.

**`family` is multi-valued** — a creature belongs to zero, one or several (owner, 2026-08-31). Whatever
...
([`numerics/model.py:68`](../../tools/seedsmith/seedsmith/numerics/model.py)). Creatures have no
magnitudes to resolve: rarity is a band, `Θ` comes from the power ladder, and generated creature content
...
```powershell
cd tools\seedsmith
python -m pytest tests/test_adapter_creatures.py -q
...
tools/seedsmith/seedsmith/adapters/creatures/
    __init__.py      → CreaturesAdapter (the five methods)
    kinds.py         → the four KindSpecs + motif expression rules
    registries.py    → closed vocabularies, read from the emitted corpus + catalog
tools/seedsmith/tests/test_adapter_creatures.py
```

Mirrors `adapters/items/` exactly. **No file outside `adapters/creatures/` should need to change** — if
...
full account and why the other two options (a creatures-local dict, or abusing `registries()`) were
rejected.
...
| **The seam itself** | `_stub`'s tests still pass — the core did not learn a creature concept |
...
- **Ask first:** adding a kind; changing a frozen vocabulary; generating into `environment`; any
  edit to a file outside `adapters/creatures/`.
...
1. The creatures corpus loads and every existing metric runs against it — **zero model calls**.
...
1. ~~Does `creature` need `id_pattern` / `runtime_id_fields`?~~ **DECIDED: leave both unset**, matching
...
2. ~~Should `aspect` ship in D1 or wait?~~ **DECIDED by the owner: the creature program builds
   ...
   tier exists in code. `CreatureSpeciesDef` still carries `ElementPrimary`/`ElementSecondary`/
   ...
   [`seedsmith-map.md`](../seedsmith-map.md) §3b with the creature program as its owner, because a
