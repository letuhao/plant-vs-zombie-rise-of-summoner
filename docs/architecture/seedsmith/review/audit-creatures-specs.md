# Seedsmith — creatures feature, spec audit (D1–D4)

**Lens:** handed the seven creatures specs and told to build them — what stops that, what is
self-contradictory, and what is assumed rather than checked?

**Method:** read all seven specs plus the code and data they cite —
`CreatureSpeciesCatalog.Generated.cs`, `RpgStore.AlmanacSeed.cs`, `RpgStore.cs`'s `recipes` DDL,
...
**The worry.** `creature-corpus-emit` depends on `almanac_seed`, which the ideal cites as a *spec*
...
`spec-adapter-creatures` §2.2 declares an `aspect` kind, and `spec-aspect-scope.md` was approved
2026-08-31. **Approved is not built.** The tier those aspects live on does not exist in code —
`CreatureSpeciesDef` still carries `ElementPrimary`/`ElementSecondary`/`TraitPool` on the *species*.

**Why this is sharper than the open question I filed.** `adapter-creatures` §8 Q2 asks whether `aspect`
should ship in D1 and answers "declaring the kind early is harmless". That is true and insufficient:
the specs never say **who builds `aspect-scope`, or when**. It belongs to the creature program, which
...
weighted toward the text-rich side, which is a consequence of `CreatureSpeciesGenerator` preferring
zombies (*"creatures wear zombie bodies"*), not luck.
...
seedsmith's distribution machinery was built against **1,438 item entries**. The creature roster is
**24** (`CreatureSpeciesGenerator.DefaultMaxSpecies`). With perhaps 5–8 families, that is 3–5 creatures
each.

`Distribution/MotifSharing` (D3) computes creatures-per-motif over that. On n=24, minus whatever S3's
...
signal. Raising `DefaultMaxSpecies` is a creature-program decision, not this feature's.

---

## S5 — ⛔ CONTRADICTION inside `spec-creature-themes`: two success criteria cannot both hold
...
`theme.thorned-chassis`, `theme.verdant-graft` — across 31 sets. **None is a creature.** They are an
authored item vocabulary, ~6 sets each, and clearly deliberate.

So if the registry is creature-published only, all five legacy themes fail validation and 39 entries
...
and its fix) is unaffected; only the number was wrong. `spec-creature-themes.md` is corrected in
...
prefixes: legacy themes are `theme.*`. Give creature themes their own prefix (`creature.*`), and the
...
`spec-creature-themes` §9 Q1 ("migrate or coexist?") is therefore **answered**, and its success criteria
...
`CreatureSpeciesGenerator` selects the top **24 by observed HP** and assigns rarity by rank
...
- A creature can leave the roster. Its families, motifs and **published theme** dangle. Items themed to
  it are now themed to nothing.
- A creature's rarity can change, altering anything derived from it.
...
departed creature's theme is **retired, never deleted**, so existing items keep resolving. That is the
...
`CreatureSpeciesCatalog.Generated.cs`). Head-noun extraction over those tokens is not merely inaccurate;
it is undefined.
...
`spec-adapter-creatures` §1 sets a good success criterion: *"not one line of core code changed."* True
for D1.

But `creature-metrics` (D3) adds two files under `metrics/`, and `creature-themes` (D4) edits
...
| S2 | blocker (D2) | map §3b | add `aspect-scope` as an explicit cross-program dependency with an owner | ✅ **owner decided 2026-08-31**: creature program builds it first, D2's aspect generation waits |
| S3 | risk, unquantified | scheduling | measure flavour coverage before scheduling D2 | ✅ **measured 2026-08-31 — 100%**, see below |
| S4 | risk | `spec-creature-metrics` | note that n bounds what the sharing metric can resolve | ✅ **premise removed** — see below |
| S5 | **contradiction** | `spec-creature-themes` | prefix split (`theme.*` / `creature.*`) | ✅ applied |
...
**Owner decision 2026-08-31 — the species cap is removed entirely.** `CreatureSpeciesGenerator.Generate`
...
`DefaultMaxSpecies` is a creature-program decision, not this feature's."* Two things were missed:
...
and rising. The metric now reports `creatureCount` beside every figure, because `n` moves between runs.
...
(`2 + max(2,150) = 152`, `20 < 152`). Same creature, same rank, different tier — purely because capture
...
other tiers were proportional, so a 900-creature roster would still have had exactly two legendaries. It
...
**Tooling defect found and fixed in passing:** `tools/CreatureCatalogGen` never called
