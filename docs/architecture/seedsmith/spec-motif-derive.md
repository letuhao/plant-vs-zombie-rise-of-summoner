...
Derive each creature's **motifs** and **anti-motifs** — the shared vocabulary every generator draws from.

This is the feature's coherence mechanism. Because the aspect, commander-effect, environment and
theme generators all read the same 3–5 motifs, their output sounds like **one creature** *without any
...
**Done means:** every creature has motifs, anti-motifs and a `basis`, derived with **no human pass**
...
The expression rules live on each `KindSpec` (`adapter-creatures` §2.7) — a motif is a *material* to an
...
For each creature, in order:

1. **Family motifs** — every family the creature belongs to contributes its motifs. This is the
   inheritance channel that makes a family mean something rather than being a label.
2. **Own text** — `flavorInfo` / `flavorIntroduce` contribute creature-specific motifs.
3. **Trim to 3–5**, family-first, deterministically ordered.

A creature in two families inherits from both. A creature in none derives from its own text alone, and if
...
Anti-motifs say what the creature is **not**, and they exist because a generator told only what a thing
*is* will happily add what it isn't. They are derived from the **nearest contrasting family** — the
family sharing the fewest motifs with this creature's own.

This is deliberately mechanical. Authoring anti-motifs would reintroduce the human pass Q1 removed,
and asking a model for "what is this creature not" invites answers that are true of everything.
...
> creature then gets nut-ish motifs *and* the nut family, and inheritance appears to work beautifully.
> It is a tautology: one token read twice. **And every metric reports success.**

So `basis` is not audit trail here, it is the input to a correctness check. `creature-metrics` excludes
creatures whose motifs **and** families are both `basis = name` from the sharing metric, because their
...
Nothing here is a magnitude. `adapter-creatures` returns an empty `channels()` list, so `numerics` is
...
python -m seedsmith creatures motifs           # derive and write
python -m pytest -q
```
...
data/seed/creatures/_registry/motifs.v1.json     → the append-only motif vocabulary
data/seed/creatures/_generated/motif-assignments.json  → speciesId -> {motifs, antiMotifs, basis}
```
...
| Creature in two families | inherits from **both** |
| Creature in no family, with text | motifs from own text, `basis = "text"` |
| Creature in no family, no text | **no motifs**, `basis = "blocked"` — not padded, not an error |
...
| **A2's tautology case** | a creature whose family and motifs are both `basis = "name"` is **flagged** in the output so `creature-metrics` can exclude it — asserted here, not left to the consumer |
...
- **Never:** emit a number; pad a `blocked` creature with invented motifs; rename or re-position a
  published motif id; treat the expression rules (`adapter-creatures` §2.7) as optional.
...
4. Tautological creatures (A2) are flagged in the output.
...
1. ~~How many motifs from the family versus own text?~~ **DEFAULT: 2 inherited + 1 own**, inside the
   ...
   answers it.
2. ~~Anti-motifs when a creature has only one family.~~ **DEFAULT: contrast against the nearest *other*
