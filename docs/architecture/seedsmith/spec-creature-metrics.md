# Spec: `creature-metrics`

Module `creature-metrics` in the [seedsmith map](../seedsmith-map.md) §3b. Wave **D3**.
...
This module gates D4 for that reason. Without it, `creature-themes` would generate content from a
family/motif graph nobody has verified — and both failure modes below are invisible to every check
seedsmith already has.

**Done means:** per-creature coverage exists alongside per-family coverage, and motif sharing is measured
...
### 2.1 `Coverage/CreatureUncovered` — per-creature, because per-family is not enough

Audit A5:

> `families[]` is multi-valued. If a creature belongs to three families, **a handful of multi-family
> creatures can satisfy every partition** while most of the roster gets no content — and coverage
> reports green.

Family coverage answers *"is every family represented"*. That is a real question, and a **different**
one from *"does every creature have content"* — which is the one a player notices.

So: a finding per creature that has no generated content, regardless of how well its families are
...
effect or theme. A creature is uncovered only when it has *nothing*, and coverage is otherwise binary:
a creature with SOME content produces no finding at all, the same "silence is healthy" convention
...
also names which kinds ARE present for a partly-covered creature. That would need a second finding
type (or a NOTE alongside every healthy creature), which is exactly the noise the "any artifact
counts" decision was chosen to avoid. What actually ships is narrower and still answers the useful
question: **the one GAP finding a zero-content creature produces carries `absentKinds` — every kind
...
for creatures with nothing present. Collapsing coverage to a bare pass/fail boolean (no kind list at
...
> creature gets nut-ish motifs *and* the nut family, and inheritance appears to work beautifully. It is
...
The property worth measuring is **creatures-per-motif**, not vocabulary size. Q7 settled that already,
...
**The exclusion is what makes the metric mean anything.** A creature whose motifs **and** families are
...
creatures from the numerator and denominator both.

Reported, always, in the finding's evidence:

| Field | Why it is reported rather than folded in |
|---|---|
| `creaturesPerMotif` | the measured property |
| `creatureCount` | the `n` the figure was measured over — it varies between runs now (§2.2a) |
| `excludedTautological` | how much of the corpus could not be measured |
| `singleUseMotifs` | a motif used by exactly one creature is a private adjective, not a shared vocabulary |
...
update that adds almanac entries adds creatures. `n` is now **whatever capture coverage yields** — 84
...
- **It does change how the output must be read.** `n` is no longer a fixed design point to reason
  against, so **the metric reports `creatureCount` alongside every figure**. A sharing number without
  its `n` is uninterpretable when `n` moves between runs — and it will move, every time capture
  coverage improves.
- **It removes the reason to distrust the figure.** At 84 and climbing, creatures-per-motif is a real
...
"the roster grew". Counts plus `creatureCount` let a reader tell those apart; a ratio cannot.
...
```powershell
cd tools\seedsmith
python -m pytest tests/test_creature_metrics.py -q
python -m seedsmith report --adapter creatures
python -m pytest -q
```
...
tools/seedsmith/seedsmith/metrics/
    creature_coverage.py      → Coverage/CreatureUncovered
    motif_sharing.py       → Distribution/MotifSharing
tools/seedsmith/tests/test_creature_metrics.py
```

**They live in `metrics/`, not in `adapters/creatures/`** — deliberately. Per-entity coverage and
vocabulary sharing are generic properties; items will want both. The adapter supplies the strata, the
metric does the counting. Putting them under the adapter would make a general check creature-shaped by
...
| Case | Expect |
|---|---|
| Every creature has content | `Coverage/CreatureUncovered` reports **nothing** |
| One creature uncovered, its families all covered | **one finding** — this is A5's exact case, and the test states it as such |
| A creature with a commander effect but no aspect | **covered**, and the evidence names `aspect` as absent (§2.1) |
| Two runs over rosters of different size | each carries its own `creatureCount`; the sharing figures are not comparable without it (§2.2a) |
| Motifs each used by one creature | `singleUseMotifs` equals the vocabulary size; sharing reported as absent |
| Motifs shared across a family | `creaturesPerMotif` > 1 |
| A creature with motifs and families both `basis = "name"` | **excluded** from the sharing calculation, and counted in `excludedTautological` |
...
- **Never:** give `MotifSharing` a pass/fail field; count a tautological creature as evidence of
  sharing; report per-family coverage as if it answered per-creature coverage.

---

## 8. Success criteria

1. A creature with no content is found even when all its families are covered (A5).
...
6. Both live in `metrics/` and work for a non-creature adapter that supplies the same strata.
