...
Make `motif-derive` read the **prose** in a creature's flavour text and ignore the **stat table**.
...
| creature | derived motifs | what they actually are |
|---|---|---|
...
**18 of 84** creatures carry `flavorIntroduce` — lore with zero stat content. It is preferred input…
**but preferring it naively makes some creatures worse**, measured:

| creature | before | after prose filter alone |
|---|---|---|
...
### 2.3 A creature can become `blocked` that was not before, and that is correct

Filtering removes input. A creature whose `flavorInfo` is **entirely** a stat table has no prose, and if
it also lacks `flavorIntroduce` it will now derive from its name only (`basis="name"`) or, with an
empty name, become `blocked`.

**This is the honest outcome, not a regression.** A creature whose "motifs" were `一类` and `击杀` had no
...
no discriminating power — a motif shared by nearly every creature is not a motif. Corpus-frequency-based
...
python -m seedsmith creatures motifs        # regenerates motifs.v1.json + motif-assignments.json
```
...
data/seed/creatures/_registry/motifs.v1.json          → REGENERATED (append-only rules apply, §7)
data/seed/creatures/_generated/motif-assignments.json → REGENERATED
```
...
| A creature with `flavorIntroduce` | derives from it in preference to `flavorInfo` |
| A creature whose `flavorInfo` is 100% stat lines, no `flavorIntroduce` | falls back to name, `basis="name"` — **not an error** (§2.3) |
...
- **Always:** keep classification pure and separately testable; prefer `flavorIntroduce`; treat a
  newly-`blocked` creature as a correct outcome; preserve determinism.
- **Ask first:** adding a new mechanical section header; corpus-frequency stopword exclusion (§2.4);
  changing the 3–5 motif bound.
- **Never:** call a model here; change what `basis` means; hand-author a motif to make a creature look
  covered.
...
`Coverage/CreatureUncovered` run reports 84/84 creatures with no content), so nothing downstream breaks.
...
1. Stat lines contribute **zero** tokens to any creature's motifs.
2. The three named regression cases (`一类`, `伤害`, `优先`) are pinned by test.
3. `flavorIntroduce` is preferred where present (18/84 creatures).
...
1. ~~Should a corpus-frequency floor exclude near-universal words like `僵尸`?~~
   ✅ **CLOSED — no, and the question's premise was wrong.** Measured across all 84 creatures, a
