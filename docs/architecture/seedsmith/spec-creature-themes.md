# Spec: `creature-themes`

Module `creature-themes` in the [seedsmith map](../seedsmith-map.md) §3b. Wave **D4**.
Depends on `motif-derive`. **Gated by `creature-metrics` (D3).**
...
Make a creature usable as a **theme** by the items and action corpora — which is how the owner's
"item specific for each creature" and "action specific for each creature" actually get built.

**Done means:** the items corpus can author a set or unique themed to a creature, using a legal
`themeKey` that carries that creature's motifs and expression rules — with no cross-corpus write and no
...
### 2.1 Why items are not a creatures kind — audit A3

The obvious design puts an `item` kind in the creatures adapter. **It does not survive contact with the
code.** `Corpus.load(root)` is single-root: one corpus, one directory. Items live in
`data/seed/items/`. So a creature "item" would be either:
...
**So a creature is a theme.** Items stay in the items corpus and reference the creature. Strictly better
than the original framing: no cross-corpus write, the single-root constraint respected rather than
fought, and a creature's signature gear is naturally a **set** — already theme-required, already
...
`theme.thorned-chassis`, `theme.verdant-graft`. None is a creature. They are deliberate, roughly six
sets each.

That draft asserted both *"the adapter rejects a `themeKey` absent from the registry"* and *"existing
themed content still validates"*. **Those cannot both hold** if the registry is creature-published only.

**The resolution the existing id grammar already implies:** legacy themes are `theme.*`; creature themes
are **`creature.*`**. The `themeKey` vocabulary is a **union of two append-only populations that cannot
collide**, each with its own provenance and rules:

| Population | Prefix | Authored by | Rules |
|---|---|---|---|
| Item themes | `theme.` | humans, pre-creatures | append-only; untouched by this feature |
| Creature themes | `creature.` | this module | append-only; carries motifs, expression rules, `basis` |
...
```
data/seed/creatures/_registry/themes.v1.json
  themeKey -> { speciesId, displayName, motifs[], antiMotifs[], expression{}, basis }
```

**The direction is one-way and it matters.** Creatures *publish* themes; items *consume* them. Nothing
in the creatures corpus reads an item, and nothing in the items corpus writes a creature. Two adapters,
...
`adapter-creatures` §2.7's rules, narrowed to items and actions:
...
the creature one: *Shell Blade*, *Shell Plate*, *Shell Ward* — every check passing, the corpus
...
A theme carries the `basis` of the motifs behind it. A creature whose motifs are `basis = "blocked"`
...
A creature with `basis = "name"` publishes a theme **marked as such**. Whether such a theme may be used
...
**Membership — better.** Under the old top-24 selection, a better capture could *evict* a creature: a
newly-seen species out-ranked one already published. With no cap, nothing is evicted by a rival.
A creature leaves only if the game removes its type or the capture loses its HP row — much rarer.
...
`count / 4` rare). As the roster grows, **every creature's rarity is recomputed against a larger pool**,
so a creature can change tier without moving rank at all:
...
Same creature, same observed HP, same rank — Common becomes **Legendary** purely because capture
coverage improved. Anything this feature derives from rarity must therefore treat it as **a property
of a particular roster snapshot**, never as a stable attribute of a creature.
...
**Why this still corrupts silently rather than failing loudly:** items themed to a departed creature
...
departed creature's theme is retired, not removed** — marked `retired: true`, still resolvable, no
longer offered for new generation. Additionally, **a theme records the `rarity` it was published
against**, so a later reader can see that the creature's tier has since moved rather than silently
...
`creature-metrics` gates this module. Generating themed content from a family/motif graph that has not
...
python -m seedsmith creatures themes            # emit the theme registry
python -m seedsmith report --adapter items   # items now validate themeKey against it
python -m pytest -q
```

`creatures themes` is the public refresh entrypoint; `creatures theme-refresh` is an equivalent alias.
...
tools/seedsmith/tests/test_creature_themes.py
data/seed/creatures/_registry/themes.v1.json    → emitted, committed, append-only
```

**One file outside `adapters/creatures/` changes** — the items registry gains a vocabulary. That is the
single exception to `adapter-creatures`'s "no file outside the adapter" rule, and it is a *vocabulary*
...
| Case | Expect |
|---|---|
| A creature with motifs | publishes a theme carrying motifs, anti-motifs and expression rules |
| A creature with `basis = "blocked"` | **publishes no theme** |
| A creature with `basis = "name"` | publishes a theme **marked** `basis = "name"` |
...
| A creature theme id | always `creature.*`-prefixed; a collision with `theme.*` is impossible by construction |
| Existing themed content (30 sets, 8 uniques, 38 total) | **still validates** — a test asserts no existing entry breaks |
| Theme registry | append-only; a re-run with a new creature leaves existing keys untouched |
| A creature that leaves the roster | its theme is **retired, still resolvable** — never deleted (§2.4a) |
| An item themed to a retired creature | still validates |
| The roster grows and a creature's rarity changes tier | the published theme keeps the `rarity` it was published against; the change is **visible, not silently inherited** (§2.4a) |
| Direction | nothing in `adapters/creatures/` reads the items corpus — asserted structurally |
...
- **Always:** publish one-way (creatures → themes → items); carry expression rules and `basis`; keep the
...
- **Ask first:** using a `basis = "name"` theme for generation; any second file outside
  `adapters/creatures/`; making `themeKey` required on `unique` (it is optional today).
- **Never:** write into the items corpus from the creatures feature; read an item from the creatures
  adapter; publish a theme for a `blocked` creature; break an existing themed entry.

---

## 8. Success criteria

1. Items can be authored themed to a creature, validated against the registry.
2. All 38 existing themed entries still validate.
3. `blocked` creatures publish nothing; `name`-based themes are marked.
4. Expression rules travel with every theme.
5. Exactly one file outside `adapters/creatures/` changes, and it adds a vocabulary rather than a
   concept.
...
1. ~~Do the existing themes become creature themes, or coexist?~~ **ANSWERED by audit S5**: they
