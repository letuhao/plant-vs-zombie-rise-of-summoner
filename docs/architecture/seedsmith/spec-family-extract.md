...
### 2.2a ⛔ Label language must be decided before this module is built — audit S7
...
So this module's prompt decides whether the next module can work at all. Two options, and the specs
must pick one **before either is built**:

| Option | Consequence |
|---|---|
| **Extraction emits English labels** | head-noun merging works; but a translation step now exists that nothing specs, and quality depends on a model rendering game-specific Chinese compounds |
| **Extraction emits native labels** | no translation; but `family-consolidate` §2.1's merge rule must be **replaced**, and the synonym map carries the whole load |

Filed as an open question in the first draft of both specs. **It is not a question — it is a
blocker**, because it changes the next module's core algorithm and cannot be deferred to between
them.

### 2.3 Output — candidates, not decisions

Per demon: zero or more `{ label, basis }`, where `basis` is the audit's honesty mechanism:
...
- **Always:** record `basis` on every candidate; batch deterministically; treat `blocked` as an
  answer; commit the recorded output.
- **Ask first:** changing batch size (it changes which demons see each other); overlapping batches;
  any schema field beyond `label`/`basis`.
...
2. Every candidate carries `basis`; `blocked` demons carry no label.
...
1. ⛔ **Label language — see §2.2a.** Promoted from open question to blocker by audit S7; it must be
   answered before `family-consolidate` is built, not after.
2. **Batch size.** Small batches lose sharing; large ones lose attention and cost context. It wants
   measuring on the real 24-species roster, not choosing now.
3. **Should a demon be allowed more than one candidate label from one batch?** Families are
   multi-valued (owner), so probably yes — but permitting it at extraction makes consolidation's
   merge problem larger, and it may be cleaner to let multi-membership emerge from consolidation
   instead.
