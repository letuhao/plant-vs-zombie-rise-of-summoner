...
Depends on `adapter-creatures`, `pipeline`.
...
Extract **candidate family labels** for each creature from its name and description.

Owner, 2026-08-31: *"family don't exist, we only crawling it from almanac natural language text, need
a classify llm pipeline… a creature maybe have some family."*
...
**Done means:** every creature has zero or more candidate labels, each recording **what it was derived
...
The brief carries, in full: the creature's own name and description, the **sibling creatures in the same
batch** (§2.2), and the per-kind expression rules. It cites nothing.

### 2.2 Batched extraction — the design decision that matters most

**Extracting one creature at a time produces a unique label per creature.** A model shown "Wall-nut" alone
...
- Batch size is a **structural constant with a comment**, not a tunable: it trades context against
  the number of calls, and changing it changes which creatures see each other — a balance pass would
  never touch it, but it is not arbitrary either.
- A creature appears in exactly one batch. Overlapping windows would let the same creature receive
...
`CreatureSpeciesCatalog.Generated.cs`. `family-consolidate`'s central merge rule is **head-noun
...
Per creature: zero or more `{ label, nativeLabel, basis }` (§2.2a covers the label pair), where `basis`
...
| `blocked` | neither was sufficient; this creature has no candidate |

**`blocked` is an answer, not a failure.** G1's rule already requires every schema to carry it, and
here it is load-bearing: a creature with no usable text **must** be allowed to have no family, because an
...
`basis` is not decoration. `creature-metrics` uses it to exclude tautological pairs (A2), and
...
data/seed/creatures/_generated/family-candidates.json   → recorded output, committed
```
...
| A creature with rich description | candidate with `basis = "text"` |
| A creature with only a name | candidate with `basis = "name"` — recorded, not silently promoted |
| A creature with neither | `basis = "blocked"`, **no label**, and it is not a failure |
| Three sibling creatures in one batch | able to receive **one shared label** — asserted against a scripted response, proving the batch is actually presented together |
| Single-creature batching (falsifier) | the same fixture produces three distinct labels — this is what §2.2 exists to prevent, and the test proves the prevention is real rather than assumed |
| A creature with a Chinese name | candidate carries an English `label` **and** the `nativeLabel` it was read from (§2.2a) |
| A `blocked` creature | carries **neither** `label` nor `nativeLabel` |
...
| A creature receiving two labels from one batch | **both** recorded — multi-membership starts here, not at consolidation |
| Schema | passes `audit_schema` (no numbers) and `audit_open_loop_schema` (no verdict) |
| Brief text | contains no citation-shaped string |
| Model returns a label for a creature not in the batch | rejected, not accepted into the record |
...
- **Ask first:** changing batch size (it changes which creatures see each other); overlapping batches;
...
2. Every candidate carries `basis`, `label` and `nativeLabel`; `blocked` creatures carry no label at all.
...
2. ~~Batch size.~~ **DECIDED: 8**, as a structural constant with a comment saying what it trades
   (context against call count) and that changing it changes which creatures see each other. It is not a
   tunable — a balance pass would never touch it. Revisited against the first real run, not guessed
   further now.
3. ~~More than one candidate label per creature per batch?~~ **DECIDED: allow it.** Families are
