# Spec: `creature-corpus-emit`

Module `creature-corpus-emit` in the [seedsmith map](../seedsmith-map.md) §3b (feature 2 — creatures).
Wave **D1**. Depends on nothing inside seedsmith.

Ideal: [seedsmith-creatures-ideal.md](../seedsmith-creatures-ideal.md). Audit findings referenced as `A#`
...
Produce the creature seed corpus on disk, so seedsmith can read it the way it reads every other corpus.
...
But every fact about a creature lives in SQLite (`almanac_seed`, `recipes`) or in committed C#
(`CreatureSpeciesCatalog.Generated.cs`). Something has to bridge that, once, at dev time.

**Done means:** `dotnet run --project tools/CreatureCorpusEmit -- <server data dir>` writes
`data/seed/creatures/*.json`, the output is committed, and a fresh install needs no game data to use it.
...
`CreatureCatalogGen` already establishes the pattern: a C# dev-time tool that reads captured data
...
|---|---|---|
| `CreatureSpeciesCatalog.Generated.cs` | `speciesId`, `name`, `side`, `gameTypeId`, `creatureTypeId`, element(s), rarity, deploy mode, acquisition, variants, trait pool | **Authoritative.** Never re-derived here |
...
**The catalog is never restated, only referenced.** A creature's element and rarity live in
`CreatureSpeciesCatalog`; copying them into the corpus creates two sources of truth, and the copy nobody
updates is the one that decides. The emitted entry carries `speciesId` and the fields the catalog
does **not** have.

### 2.2 Output shape

`data/seed/creatures/<kind>/<partition>.json`, in seedsmith's own seed-file format:
...
`motif-derive` uses it to set `basis`, `creature-metrics` uses it to exclude tautological pairs (A2), and
...
```powershell
dotnet run --project tools/CreatureCorpusEmit -- <server data dir>   # emit
dotnet test tests\FusionRpg.CreatureCorpusEmit.Tests
...
```
tools/CreatureCorpusEmit/                → Program.cs (arg parsing, file writing)
src/FusionRpg.Core/Creatures/Generation/ → CreatureCorpusBuilder.cs  (pure: rows -> entries)
src/FusionRpg.Data/Sqlite/            → existing reads only; no new SQL if avoidable
data/seed/creatures/                     → emitted, committed output
tests/FusionRpg.CreatureCorpusEmit.Tests/
```

**The builder is pure and lives in Core**, taking already-read rows and returning entries. Only
`Program.cs` touches the filesystem and the DAL. This is the same split `CreatureSpeciesGenerator` uses
...
1. `data/seed/creatures/` loads through `Corpus.load` unmodified.
...
1. ~~Partition key.~~ **DECIDED: `side/rarity` for D1; families are added as a *second* stratum in
   ...
   growing pool now that the species cap is removed (`spec-creature-themes` §2.4a shows a creature going
...
2. ~~`lineage` transitive or direct?~~ **DECIDED: direct parents only.** It is sufficient for
   `creature-fusion`'s current use, and the transitive closure is derivable from direct edges whenever a
