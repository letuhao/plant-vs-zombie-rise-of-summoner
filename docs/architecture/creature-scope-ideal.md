# Creature scope ideal: what keys what, general vs. unique, and where fusion inheritance actually sits

**Status: idea phase only — no spec, no plan, no code changes.**

> ⏸ **§5-9 (general-creature progression, troop-stack identity/upgrade containers, stack experience,
...
> creature scope... and complete creature species leftover to make gameplayable first."* §1-4 (the
> general/unique/Commander/Patron vocabulary and WAVE F2.4's own scoping question) are NOT deferred —
> they feed directly into WAVE F2 (`tasks/creature-standalone-todo.md`), which resumes now. §5-9 stay
> here, fully investigated and ready to pick up, until the owner returns to them — see
> `tasks/creature-standalone-todo.md`'s own "Deferred — future work" note for the pointer back to this
> doc.

Written in response to a real,
named confusion: *"creature have general depend creature species, unique depend fusion, gatcha, summon,
...
**Every RPG feature lives in the RPG layer. It is never built by changing what PvZ is.** A creature
...
atoms) and the RPG layer *writes back* signed deltas; it never depends on PvZ representing "creature,"
...
**`docs/architecture/creature-system-map.md`, "Vocabulary" section (its own §, added 2026-09-06)** is
...
### Axis 1 — what the creature IS (general vs. unique)
...
| Used for | Army-scale, disposable, or engine-spawned populations — a lawn wave, **siege garrisons, world-map legions** | An individually-meaningful, player-invested creature — your roster, a Commander/Patron, a Delve party member |
...
"a creature whose atoms are read from a shared per-player-per-species roll." It is "a creature that has no
persistent row *at all*" — a headcount, not an instance. **Every creature a player ever owns — from
gacha, summon, fusion, or capture — is a Unique creature** the instant it exists as a row. There is no
"general" tier of *player-owned* creature; general creatures are the OTHER, disposable population (siege
...
are not four different *kinds* of creature.

**2026-09-08 owner decision:** the distinction also selects progression. A gameplay-owned spawn
mechanism declares its progression source. General creatures associated with the player's empire use the
empire-wide species fallback only when that mechanism supplies no dedicated progression source.
Unique creatures and Commanders use their respective dedicated sources and never combine them with the
fallback. The binding statement is `decisions.md`'s *Creature progression source and spawn ownership*
...
**Both are a role you *assign* to an existing Unique creature — never a separate kind, never a
promotion that changes what the creature IS.** "Commander is a promotion of unique" (the user's own
framing) is directionally right in spirit (only a Unique creature is eligible) but the more precise
model is *designation*, not evolution: the creature's own row is unchanged, a separate pointer
(`commanders.md`/`spec-patron-creature.md`) names which `UniqueActor` currently holds the role, and it
can be reassigned. Neither Commander nor Patron ever fights.

**A third, unrelated concept:** a **Delve party member** — a Unique creature that actively fights inside
a dungeon run and can go `Downed`. This is not a role at all; it's a different LOOP (`party-dungeon-ideal.md`),
and conflating "the creature fighting for me" with "Commander" is the single most common version of this
...
different question: *given* a Unique creature (Axis 1 says what it is), **which of its own data is
...
|---|---|---|---|
| Species catalog (base stats, element, rarity floor) | `speciesId` only | Shared — by definition, a species fact | `CreatureSpeciesCatalog` (`src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.cs`) |
| The specimen row itself | `instanceId` | **Individual** — every specimen gets its own row | `rpg_unique_actors` / `UniqueActorDto` |
| Rarity, variant, element (rolled at mint) | `instanceId` | **Individual** — rolled once per mint, in `CreatureProfileDto` | `RpgStore.Creatures.cs:29` `MintCreatureUnlocked` |
| Level / XP | `instanceId` | **Individual** | `rpg_unique_actors.level`, `AwardUniqueActorXpUnlocked` |
| Star (promotion investment) | `instanceId` | **Individual** | `rpg_creature_profiles.star`, `StarPolicy` |
...
| TraitIds (old, flavour-only tags) | `instanceId` | **Individual** — but cosmetic, never resolves to real atoms today | `CreatureProfileDto.TraitIds`, `spec-creature-fusion.md` locked decision 5 |
...
**So the real picture is not "general (shared) vs. unique (individual)."** A Unique creature already
...
narrower, more specific fact than "unique creatures are individual" implied, and it is the actual reason
...
**There is no mechanism today for a creature specimen to carry its own individually-rolled gameplay
...
- **Persona/SMT's Demonic Compendium is the other precedent, and it's closer to this repo's own
  shape.** The Compendium stores registered creatures at the SPECIES/build level (an entry per creature
...
- **(b) Move toward Pokémon-shaped.** Give each specimen (or at least fusion-born specimens) its own
  ...
  own idea if the owner wants creatures to have Pokémon-style individual depth — not something to build
  as a side effect of WAVE F2.4.

## 4. Summary — a direct answer to the confusion, restated once

- **"Creature have general depend creature species"** — not quite: *general* creatures are the disposable,
  no-instance, army-scale population (siege/legion), and a species is just their type key. A
  player-owned creature of ANY acquisition method is never "general."
...
- **"Commander is promote of unique"** — close: it's a *role assignment* to an existing Unique creature
  (reassignable), not a promotion that changes the creature itself. Patron is the match-scoped sibling
...
- **"Fusion much key of player, specie, unique?"** — fusion's own transaction is keyed on
  `(playerId, sacrificeInstanceId × 1-2, baseInstanceId?)` — purely Unique-creature-scoped; species is
  read *from* the specimens, never a fusion input in its own right. The part that feels like it spans
  all three keys is real: fusion **mints** a new Unique creature (`instanceId`-scoped), whose real
...
## 5. Extending the idea: how do general creatures get bonuses and upgrades?

**A real, valid objection the owner raised after reading §1-4 above:** if general creatures (siege
...
### 5.1 Today: general-creature combat power is disconnected from player progression — a real gap
...
- **World-map legions (`WorldEntityMember`, `src/FusionRpg.Core/World/WorldState.cs:275-284`)**: `Hp`/
  ...
  `LoamPhases.cs:280`) — no code path reads `CreatureSpeciesCatalog`, `Θ`, or `P(Θ)` when building one.
...
- **Siege-board waves are the one exception, and only half of one.** `WaveCatalog` already prices
  general creatures through the real `P(Θ)` ladder (`base-defense-ideal.md` §3.5,
...
- **`WhoKind.Type` resolves by live board `typeId`, never by `UniqueActor` or ownership**
  ...
  A general creature spawned with a given `typeId` is reached exactly the same way any other board entity
  is — structurally, this already targets general creatures. **But it is completely unwired**:
...
- **The `CreatureType` allocation scope — the "general passive tree" `creature-system-map.md:51` already
  ...
  general creature on the board shares a `typeId` the player has put `CreatureType` points into, this chain
  ...
  tree state — Commander and each creature alike"), which reads as assuming per-instance state for every
  creature — structurally impossible for a general creature with no `instanceId` to key on. The species-keyed
  reality is the correct resolution: a general creature reads the shared, species-level tree, it simply
  isn't stated that way in that doc.
- **What's actually missing is three separate, smaller, real things — not a new mechanism:**
  1. *Write side, wiring gap.* Nobody has ever persisted a `CreatureType` allocation in production —
...
- **Total War (Warhammer/Three Kingdoms)** — named Lords get individual skill trees and equipped
  items (the Unique-creature shape); generic units instead get a general's proximity aura plus
...
**The recommendation this section supports, without deciding it:** general-creature progression does not
...
`world-map-scope`/`world-buff.*` carriers (§5.3), or wiring the already-species-keyed `CreatureType`
...
**A real refinement from the owner, and it corrects where §5 was aiming.** Restated: *creature species
is the root template and should stay non-upgradable — a general creature is a copy taken from it — and
the actual confusion is that general creatures have no owner-scoped upgrade container or management of
...
**The confusion is real, but it is not "general creatures have no owner at all" — it's that ownership and
...
§5's `CreatureType`/`WhoKind.Type` wiring is an **empire-wide, species-keyed** bonus — the same value
...
- **`WorldEntityMember` needs its own light identity** — a `MemberId`, not a full `UniqueActor` (a
  troop stack is not an individually-equipped Unique creature; giving it one would blur Axis 1 back
...
- **Creature species itself needs no upgrade concept, confirmed by this investigation, not just
  asserted** — nowhere in the real code does anything treat `CreatureSpeciesCatalog` as mutable or
...
- **WoG's own "Alternative Upgrade" mod** gives select creatures (Crusader, Silver Pegasus, Iron
  Golem, Horned Creature, Vampire Lord, Minotaur King, Magma Elemental) a second upgrade tier, unlocked
...
## 8. Enriching §7: a near-death survivor can be promoted to a Unique creature — one at a time, low chance, tunable

Owner's own proposal: a general-creature troop stack that fights, comes near death, **survives, and
levels up** gets a chance — tunable, meant to be low — for exactly **one** member to be promoted into
a real Unique creature. Investigated the same way as every section above.
...
- **A tunable, banded, deterministic "low chance" roll is not a new mechanism — this repo already
  ...
  becomes a player's own creature" — from an `HpBandOf` banding (low/half/high, driven by
...
- **The mint itself is already a solved, shared path.** `RpgStore.MintCreature` (used identically by
  summon, fusion, capture, and this session's own debug-grant seam) is the one real place a
  `UniqueActor` gets created from a `CreatureMintSpec` — promotion is a fifth caller of the same function,
  not a new one.
...
- **The promotion itself** is a fifth `RpgStore.MintCreature` caller (§8.2) — the mechanism that turns
...
container that buffs that one specific Unique creature forever — and titles more broadly should be
...
- **`Trait` has the RIGHT storage shape** — `CreatureProfileDto.TraitIds` is per-`instanceId`, permanent,
  ...
  applied to *every* creature carrying that trait id, never a per-specimen roll). This is the code-level
...
brief-assembler/deterministic-emitter/closed-vocabulary-validator shape `creatures/anchor` already uses
...
- This repo's own already-cited genre pass: `docs/research/genre-mechanics/06-summoner-minion-fusion-rpg.md` §1.5 (Persona/Nocturne/SMT IV/V fusion-inheritance determinism arc — already used directly in `creature-mechanism-gaps-ideal.md` §3.3, not re-derived here).
