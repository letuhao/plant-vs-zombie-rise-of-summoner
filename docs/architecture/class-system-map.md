...
| **2** | `BattleStatComposer` **runs no subsystems** — it never references `ActorHub`, `DerivedComposer` or `IActorStatSubsystem` | An aptitude subsystem is **invisible to battle**. **Decided 2026-08-26: the composers stay separate** — the battle-side seam is `ChannelMods`, the way `StarPolicy` already feeds progression stats in. See below. **HISTORICAL — overturned 2026-09-12** as a SOLID/DRY defect (`decisions.md` "ActorHub sole Hot compose gate"); `battle-hub-fuse` (`actor-hub-and-combat-power-solid-fixing` T6, 2026-09-13) deleted `BattleStatComposer` entirely — battle now composes through this SAME `ActorHub`/`DerivedComposer`, not a separate seam |
...
| **Omni combination is TUNABLE** — default `totalPower = omni + category`; the former ban was removed 2026-09-02 | [actor-hub-ssot.md](actor-hub-ssot.md) §3 ban |
