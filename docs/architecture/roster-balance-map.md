...
The original version of this map (2026-09-05) targeted the **creature species roster** (841 rows,
...
brief at all. **The creature-species roster and the atom-family diversity bug are two systems that
...
|---|---|
| RB1 `distribution-stats` | **Redirected.** Same technique (Shannon evenness, top-share), new corpus: real usage counts over the 98 affix families, not creature species axes. |
...
| RB6 `pipeline-direction` | **Kept, retargeted.** The actual fix: weight the shared 98-family pool by real usage history instead of creature-species cell need. The design principle ("bias, not a cage"; prove the C1 tier invariant; replay-provable at zero token cost) carries over unchanged — only the input signal changes. |

**Net effect: three modules, not six.** Documented in §2 below.

**The creature-species finding stands, separately, and is handed off, not built here.** Running
`creature_roster.py`'s existing, already-tuned metrics against the real corpus for the first time
...
share 973‰, posture imbalance, aptitude skew. **This is real, valuable evidence for `creature-seed`'s
...
| Creature-species roster metrics (a *different* corpus, unrelated to this bug) | **built, already tuned**, first real run done 2026-09-06 | `metrics/creature_roster.py`, `data/tuning/creature-roster-targets.v1.json` — handed to `creature-seed`, not owned here |
...
- **Never narrow `allowedAtomFamilies`.** The pool stays the full 98; direction is a weight applied
  ...
  at risk.
- **Never touch the creature-species roster.** That finding belongs to `creature-seed`; this program reads
