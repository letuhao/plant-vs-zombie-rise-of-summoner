# Seedsmith content-completeness — `creatures`
...
Creature species already write real `_provenance` (`data/seed/creatures/species/plant/aerial-flora.
...
what the task brief assumes going in: **which creature-species field(s) count as "description/flavor"
...
`data/seed/creatures/species/**/*.json` was enumerated directly (not sampled):
...
- Its real, committed values carry raw mechanical/stat vocabulary — the exact class of content this
  ...
  already enforces for the sibling `creature`/`commander-effect` kinds.

This distinguishes `reason` cleanly from the fields `creature_coverage.py`/`creature_roster.py` already
...
**Decision:** this module registers `CompletenessSpec(domain="creatures", kinds={"species"},
...
`creature`/`commander-effect` kind files under the same `data/seed/creatures/` root, which already are
real `{kind, entries}` documents and load correctly through the generic path today (confirmed:
`data/seed/creatures/creature/plant/common.json` and `data/seed/creatures/commander-effect/all.json` both
...
**The fix (`adapters/creatures/completeness.py`, mirroring the live precedent found while grounding
...
the SAME `Corpus` the generic loader already built, since `creatures` is a *mixed*-shape adapter
(`creature`/`commander-effect` already parse; only `species` needs the bespoke walk), unlike dungeon
(where every kind needed it). `report/cli.py`'s `cmd_check` gains one `elif args.adapter ==
"creatures":` branch calling this, matching the existing `dungeon` branch's own shape exactly.

## 3. The backfill-wiring half: proving the shared engine generalizes to creatures' own real resumability

Task 10's own acceptance requires "a resumed creature-species generation run backfills only genuinely
missing/stale entries." Creatures' species-anchor generator (`run/runner.py`'s `start()`) already does
...
`Content/FieldStale` (Task 3's registry) reports nothing for creatures today — correct, not a bug: no
...
```powershell
python -m seedsmith.report.cli check data/seed/creatures --adapter creatures
...
```
tools/seedsmith/seedsmith/adapters/creatures/completeness.py   (new) — §2, §3
tools/seedsmith/seedsmith/report/cli.py                     (edited) — one elif branch, §2
tools/seedsmith/tests/test_creatures_completeness.py            (new)
```
...
1. `load_species_corpus` loads all 904 real entries with `kind="species"` from the real
   `data/seed/creatures/species` tree.
...
- **Always:** cite real file:line evidence for the "no flavor field exists" finding, not an
  assumption; keep `Content/FieldMissing`'s report against creatures `gates=False`; add species
  entries into the corpus the generic loader already built rather than discarding it (creatures is
...
- **Never:** treat `reason` as a flavor/description field (§1); modify `run/runner.py`'s own live
  ...
  (Core, already built and tested) for a creatures-specific need.
