...
| D2 | **All four acquisition sources**: skill points · aptitude thresholds · items/affixes · creature aspect | The `skillPointsPerTheta: 1` grant, minted since 2026-08-26 with **zero consumers**, finally has a spender |
...
| D9 | **Tree roster**: 12 primary + all elemental + all status + each creature family (+ creature species, deferred) | `n ≈ 40–60`, which *simplifies* the math — see §3.1 |
...
| D21 | **Every actor carries its own tree state** — Commander and each creature alike (owner, 2026-09-04) | Maximum build expression: each creature is genuinely built, not a stat block. **This promotes sparse storage from a nicety to a hard requirement** — ~50 trees × ~29 skills is ~1,450 possible per-skill soul levels *per actor*, so only non-zero entries may ever be persisted (§7.9). A dense row per actor is not an option at this scope |
...
| D46 | **The balance verdict is scored against MIRROR SQUADS. The AI is another player** (owner, 2026-09-05: *"we will make our AI enemy system smarter then it will cheat and make counter build to destroy player… our design already pvp pve balance consider AI as another player"*) | Closes `squad-harness` OQ1. The spec's own reason was that §3.5's causal story — *"a corner maxes one axis and floors eleven, so every opponent finds an open one"* — **requires the opponent to be a build**, and an authored wave is not. The owner's reason is stronger: **the AI is a build by design.** `decisions.md:104` keeps classes alive *only* as Zomboss AI patterns, and `world-map-program.md:30` specs `ai-commander` as Zomboss and neutral factions *"committing through the same command interface"*. So a mirror squad is not a proxy for the production case — **it is the production case**, and an adversarial counter-building AI is the strictest form of it |
| D42 | **Tree power share ships as a flagged guess and is tunable** (owner, 2026-09-05: *"just ship, make it tunable then we rebalance if need"*) | `treeShareMilli` and `budget.treeTotalPoints` cannot be measured until trees carry power in the resolver, but every number a player reads scales from them. Both ship marked **UNMEASURED** in `data/tuning/passive-tree.v1.json` and are re-derived after `mechanism-wiring` and `squad-harness`. **Safe because the catalog stores COEFFICIENTS**: a re-measure is a tuning republish, not a regeneration — the numbers move, the node ids do not |
| D43 | **Existing saves are seeded from a proxy at first load** (owner, 2026-09-05) | D37's counters count forward and there is no event log to replay, so a long-running save would show 27 trees at tier 0 beside primary trees at tier 7 — which reads as broken, not as new. Each existing player gets a starting counter derived from something already persisted. **A one-time, auditable, stamped fiction that never runs again for a new player** |
| D44 | **The soul level rides `CurveInput.Level`** (owner, 2026-09-05) | `Θ_node = Θ_actor + Ws·soulLevel` is already an index offset, so it is dimensionally what `Level` carries. No reviewed E2 change to a shipped type, no other consumer touched. The cost is stated: a curve row cannot say on its own whether `Level` means actor level or a soul-shifted index, so **the caller must** |
| D45 | **Species trees ship as committed catalog data** (owner, 2026-09-05) | What is reviewed is what ships, and a version-to-version diff stays readable — which is exactly what `tree-review`'s `O(diff)` incremental re-review depends on. Deriving at import would make the deriver load-bearing: one change to it silently rewrites 33,600 nodes with no diff to read, and the reviewed artifact would stop being the shipped artifact, which is the property D24 exists to protect |
| D37 | **The two missing gate quantities are built INSIDE this program** — `status_applied.<id>` and `element_mastery` (owner, 2026-09-05) | §13.4 measured that 27 of 39 generic trees had no gate quantity, stranding **1,080 of 1,560 nodes (69%) permanently at tier 0**. Waiting on the creature program's `aspect-scope` module was not a plan — nothing scheduled it, and `status_applied` was owned by nobody at all. **New module `gate-counters`**, and the map's *"a tree's gate quantity must exist before that tree's content is generated"* rule now has a path to being satisfied for all 39 rather than 12 |
| D38 | **`g = 11` stands, sized against the MEASURED corner build, and is a tunable** (owner, 2026-09-05) | The 10.40-vs-19.2 dispute was never arithmetic — they differ by exactly the corner share (`19.2/10.40 = 1/0.54163`). 10.40 sizes the skill wallet against the build `HybridViability` actually constructs and every sweep in this program measured; 19.2 sizes it against a share-1.0 build no measurement has used. Keeping the measured basis keeps the calibration consistent with all existing evidence. **`grant.skillPointsPerThetaMilliByScope` is a tunable**, so `squad-harness` can move it without reopening a spec |
| D39 | **`H` reads the FINAL ALLOCATION, self-spent only** (owner, 2026-09-05) | Closes the order-dependence doc 10 A13 found: `H` becomes a function of the build you hold, not the sequence you bought it in, so two players following the same guide get the same `F`. Keeps D8's rule that a good off-build drop can never lower your multiplier. **The red team's F4 breadth exploit stays open by choice** — acceptable only while `F` is measured as doing little; it must be revisited if `F` ever gets teeth |
| D40 | **All three exclusion forms are kept; nullification is PRINTED LOUDLY** (owner, 2026-09-05) | Reroute → Precedence → Nullification all survive, so the generator is never forced to refuse a pair it cannot reroute or order. The risk — that an owned node doing nothing reads as a bug in a 35,160-node corpus — is answered by **presentation, not by removal**: both sides print the rule and name the same winner, and the surface renders it inert rather than unlocked. Target stays ~2% of nodes, and `tree-review` censuses every one. **`tree-review` may now enforce it** |
| D41 | **`speciesUniqueAffixMin = 8`, tunable** (owner, 2026-09-05) | A quarter of each species tree is provably its own — enough to read as the bloodline reward D23 describes, while staying a gate a machine can enforce across 840 trees. Marked nodes are planner-chosen deterministically (deepest-mechanism-first), so **raising the number later only adds marks at the shallow end and never re-owns an existing node** — which is what keeps a future ruling an `O(diff)` re-review rather than a migration. Owner: *"make it tunable, so we can add more if need"* |
...
| D34 | **`skillPointsPerTheta` becomes per-scope** | Required by D25, not optional: it is a single scalar (`AptitudeTuning.cs:13`) while aptitude points already ship a four-scope table. If every actor reads `Θ_player`, **50 creatures × 31 nodes at Θ=100 is 1,550 — the whole catalog — and D25's bound breaks outright** |
...
**Deferred by the owner to its own round:** creature **species** trees — *"consider as reward; building it
takes advantage but will block some builds… creature species tree will define how to block."* A tree that
...
| Per-category budgets and gate quantities | `AllocationScope { Commander, CreatureType, Aspect, UniqueCreature }` with a shipped per-scope rate table and **no caps** (`PointBudget.cs`) |
...
CreatureType (almanac XP) → creature family trees · UniqueCreature (specimen level) → creature species tree. Three of
...
element build, ignore others"* — **has already happened in the shipped creature corpus.** Species seeds
...
against this distribution means ~40% of all creatures push Onslaught and ~45% push earth, while Ferocity,
...
per-player objects*. **D24 does not break it — it lands on the same split the creature pipeline already
...
- **840 species are indexed, not 841.** The 841st is a stale duplicate of `SnorkleZombie` in
  ...
  (`CreatureQualityReport/Program.cs:77`) — the same defect class Phase A2 fixed 217 times, surviving
...
| Shared-deterministic generated content, on disk | `data/generated/creatures/` — 830 committed concrete files |
...
| Elemental | 6 | `element_mastery` | ⛔ comments only — `PointBudget.cs:15` says outright it *"is owned by the creature program's `aspect-scope` module and does not exist yet"* |
...
| `grant.skillPointsPerThetaMilliByScope` | points per Θ, per scope | 11 at Commander | `aptitudes.v{n}.json` | **Tunable.** A single scalar today; **per-scope is required, not optional** — at one scalar, 50 creatures unlock the whole catalog (D34) |
...
| target distribution | shares per aptitude / element | near-uniform + named theme allowance (D32) | `passive-tree-targets.v1.json` | **Tunable.** Shaped like `creature-roster-targets.v1.json` |
...
## 16. Open questions — cleared 2026-09-05

The spec round raised 19. Nine went to the owner (D37–D45); ten were cleared without one, because each
had a single defensible answer that fits in a line. **A recommendation nobody disputes is a decision,
not an open question** — leaving them listed is how a settled thing gets re-litigated.

| Question | Answer | Why it needed no owner |
|---|---|---|
| The real per-tree review rate | **Run a 20-tree pilot**; it gates the full run | Not a decision — a measurement, half an hour, and it also yields the defect correlation the sampling design needs |
| A two-reviewer agreement pass | No second reviewer initially | The sample is seeded and reproducible, so this can be added later at zero cost. Revisit only if the pilot shows high variance |
| Acceptable manual-correction rate | Above ~5%, regenerate rather than correct | Above that rate the prompt is wrong, and hand-correcting hides it. The creature corpus's 3-in-840 was a floor set by how little was reviewed, not a target |
| What "the tier below is unlocked" means | **One node of the same branch** | The spec's own recommendation, and the reading that keeps `firstPoints` where D38 calibrated it. "Tier complete" moves it ~4× (`g ∝ k²`) |
| Does a tree respec share the species respec counter | **Its own counter** | Different operations, and the price escalation should track how often you respec *that* thing. Settled before persistence, since changing it later is a migration |
| Do species trees gate on `UniqueCreature` | **Yes**, and `gate-counters` owns the binding | The quantity is live and persisted (`rpg_unique_actors.level`); only the caller was missing, and D37 already created the module that writes exactly this kind of binding |
| Stance groups for non-aptitude trees | `credit = 0` stays | It is the only reading every measurement in this program covers. Extending cross-unlock to elements/statuses is a new design round, not a default |
| May a node author a bake-time slot (L2) | **No**, for v1 | Smaller record, and neither choice breaks the freeze line — so take the one with less machinery until something needs it |
| Comparing two plans side by side | Not blocking | The interim (a stated reason in place of a diff state) holds until there is a build to compare against |
| Does `element_mastery` read the shared Aspect rate | **Its own key** | That rate is an explicitly unmeasured placeholder owned by the class system. Sharing it means a residual-fit republish there silently re-paces six elemental trees while 21 status trees stay put |

### 16.1 Still open, and none of them blocks planning

1. **Player-facing naming** — *paths / traits / Focus / Plan / bloodline / stance*, and especially
   **aptitude points vs skill points**, which the player must be able to tell apart. A task for when
   player text is authored, not a design question.
2. **The 17th atom kind (D16)** — conversion nodes stay refused until it lands. Blocked and tracked;
   it is a reviewed `decisions.md:112` change and a genuinely new capability.
3. **Three of four `skillPointsPerThetaMilliByScope` values** (`creatureType`, `aspect`, `uniqueCreature`).
   Each multiplies a different source unit, so none can be inferred from another. Measurement-gated.
