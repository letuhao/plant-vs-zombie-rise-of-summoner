...
| Acceptable manual-correction rate | Above ~5%, regenerate rather than correct | Above that rate the prompt is wrong, and hand-correcting hides it. The demon corpus's 3-in-840 was a floor set by how little was reviewed, not a target |
...
| Do species trees gate on `UniqueDemon` | **Yes**, and `gate-counters` owns the binding | The quantity is live and persisted (`rpg_unique_actors.level`); only the caller was missing, and D37 already created the module that writes exactly this kind of binding |
...
3. **Three of four `skillPointsPerThetaMilliByScope` values** (`demonType`, `aspect`, `uniqueDemon`).
