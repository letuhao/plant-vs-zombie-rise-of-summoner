...
WHO values (a target, a type, a unique creature, own/enemy side), and an event-driven delivery model that
...
**One soft dependency worth naming, not hiding:** `battlefield-scope`'s target/type/unique-creature
...
⚠ **`WhoKind` cannot express it either** — `{ Target, Type, UniqueCreature, Relation }`
