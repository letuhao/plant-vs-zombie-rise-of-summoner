...
program's history** (+14.17pp win rate, W=1 vs W=4; −0.671 rounds-to-win, EarlyBound vs LateBound),
through the real `BattleEngine.Resolve`, on synthetic profiles only — every shipped profile stays
byte-identical (`MeasProbe` diffed byte-for-byte). A real, previously-undiscovered defect surfaced along
the way: `BasicAttackEnvelope.Commitment` was hardcoded, making `DefaultCommitment` unreachable for the
basic attack regardless of dispatch completeness — fixed, confirmed inert for the atomic path.
`reaction-lane`'s own remaining gap (`RL2`/`RL3`) is narrower now: D14's root cause is resolved, but the
DEFENDER's own counter-intent interrupting an attacker's wind-up is a separate, not-yet-designed
integration `timeline-dispatch` does not provide.
...
| `timeline-dispatch` | ✅ **D14's fix — BUILT and MEASURED.** [spec-timeline-dispatch.md](battle-tempo/spec-timeline-dispatch.md). A local, per-round discrete-event dispatch (`RunTimelineActionPhase`) behind `BattleModeProfile.UsesTimelineDispatch` (default `false`, unset by every catalog row) proves `W` (+14.17pp win rate) and `Commitment` (−0.671 rounds-to-win) both non-zero through the real `BattleEngine.Resolve`, on synthetic profiles only. Every shipped profile byte-identical. | `action-timing`, `commitment-binding` |
