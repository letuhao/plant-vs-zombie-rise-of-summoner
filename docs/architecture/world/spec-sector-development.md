...
**One naming defect to fix while here:** every `*CostMilli` in `data/tuning/loam.v1.json`'s `structures` block (`wellCostMilli` 200, `waystationCostMilli` 300, `granaryCostMilli` 150) is a **whole loam unit**, not a per-mille — `StructureDef.CostMilli` is compared directly against `WorldEntity.CarriedLoam` at `BuildResolver.cs:101`, and that is a plain count. The maths is right and the name lies. Renaming a tuning key is a `publish.py` migration plus a loader edit, not a redesign.
