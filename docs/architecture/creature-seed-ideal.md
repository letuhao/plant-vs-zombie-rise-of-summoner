...
`tools/seedsmith/seedsmith/adapters/creatures/kinds.py` declares `creature`, `aspect`, `commander-effect`,
...
It also has direct precedent — `data/seed/creatures/**` is already exactly this shape: a committed dump
...
`FusionRoller.Roll` (`src/FusionRpg.Core/Creatures/Fusion/FusionRoller.cs:18-50`) is live, tested, and
