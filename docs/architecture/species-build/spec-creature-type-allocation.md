...
Make `AllocationScope.CreatureType` real. The scope, the value type, the persistence table and the read
...
```
baseline(player, species) = plan.shares[species] ⊗ PointBudget.PointsFor(CreatureType, speciesLevel, tuning)
```
...
`PointsFor(CreatureType, speciesLevel, tuning)`. **Scope-local**: overspending `CreatureType` can never be
...
src/FusionRpg.Data/Sqlite/RpgStore.Aptitudes.cs             creatureType scope_key encoding
...
4. **Budget refusal:** an override spending more than `PointsFor(CreatureType, level)` is refused, and the
   refusal is scope-local — a large `Commander` budget does not fund it.
5. **Scopes sum:** an actor with both a Commander and a CreatureType allocation reads the sum, and `share`
