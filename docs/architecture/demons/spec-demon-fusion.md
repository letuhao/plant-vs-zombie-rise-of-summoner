...
- `DemonRecipeCatalog` (Core): **amended 2026-09-06 (`demon-seed` modules 17-18, `fusion-recipe-generator`/`-runtime`, T8.3-T8.5)** — recipes are no longer computed live at startup. A deterministic pass (same same-rung pairing rule as originally shipped) covers the overwhelming majority of outputs; for the rungs that mathematically cannot support a unique pair per output (the real corpus's own Almanac/Sunwoven shortfall — 20 eligible outputs, only 4 rungs-below candidates), a bounded, per-member-voted LLM proposal fills the gap, validated and reconciled by a deterministic reconciler that is the SOLE write authority. The result is committed as a seed (`data/generated/demons/_fusion-recipes.json`, `Configure`d at startup like the species catalog) — never recomputed in the running game. **Best-effort, not exactly-one-per-species:** most summonable rare-and-above species get a recipe; an output whose gap-fill vote never converges ships with NO recipe, named as `unresolved` in the generator's own run report — never fabricated. Inputs: two species from the band below (same rung) whose elements relate to the output, OR — only for a flagged `crossRungGapFill` recipe — from the nearest 2-3 populated rungs below when the same rung cannot supply a unique pair. Capture-only species get no recipe and appear in no recipe (owner lock 6, unconditional, enforced by both the deterministic pass and the reconciler). Validated at import (unknown ids, capture-only inputs, duplicate pairs reject).
...
- **Ask first:** cost-table or star-bonus tuning (game balance); making stars affect PvZ deploy before `patron-demon`.
- **Cleared, not silently taken (2026-09-06, `demon-seed` T8.3):** "recipe-graph shape changes" above once meant no recipe could ever draw its two inputs from more than one rung below the output. The owner authorized `crossRungGapFill` — a bounded subset of recipes (the real corpus's own Almanac/Sunwoven shortfall, ≤15 outputs) may draw inputs from the nearest 2-3 populated rungs below when the same rung cannot supply a unique pair. Every other recipe's graph shape (same-rung inputs) is unchanged.
