...
Apply the empire's per-player, per-species progression only to an eligible general demon whose gameplay mechanism explicitly declared `EmpireGeneral`. The first consumer is the normal lawn run. This makes the level-3 introduction of general demon species progression understandable without leaking its fallback into unique or Commander paths.
...
It does not create a world map, empire-management screen, unique demon progression, Commander progression, manual allocation UI, or new balance values.

## Eligibility rule

An actor receives the empire species allocation only when all of the following are true:

1. Its spawn mechanism supplied `DemonProgressionSource.EmpireGeneral`.
...
The lawn spawn adapter creates an `EmpireGeneral(playerId, speciesId)` claim when the normal PvZ lawn mechanism registers an eligible general demon. It persists that provenance with subsequent activity facts using the source-contract serializer.
...
- A normal registered lawn general demon receives only its owner's species allocation.
...
- [x] Read the design gate, demon system map, decisions, species-build allocation transport, data/runtime architecture, ActorHub SSOT, and power-scale SSOT in this session.
