...
|---|---|
| ~~Player-driven, turn-by-turn interactive skill choice~~ — **corrected 2026-09-13, not a real gap.** `InteractiveIntentSource.cs` (T6) already exists: a real `IIntentSource` implementation that asks a caller-supplied delegate for a `PlayerChoice`, falls back to the AI `IIntentSource` on timeout, and records every decision (including the timeout) into a `DecisionTrace` for byte-identical replay. Its own doc comment: *"`IIntentSource` is already documented as the AI-policy seam the auto-resolved modes need, **and the player-input seam an interactive mode needs**... an interactive turn is a slower `TryDeclare`, not a new state machine."* What is missing is the **wiring**, not the mechanism: a real `Func<string,long,PlayerChoice> ask` implementation backed by a live SignalR prompt + countdown, and a Server/FE surface to call it from — matching `the-loops.md`'s own *"interactive battles you play yourself **WIP**"* status. Re-tag: **wiring gap**, not real gap | `InteractiveIntentSource.cs:14-51` |
