# Ideal: seedsmith's **second feature** — creatures
...
**Program: `seedsmith`. Feature: `creatures`.** This is not a new program and not a creature-program
...
creature-program module called `creature-forge` that would *consume* seedsmith's machinery. That was
wrong, and the owner corrected it: the request was *"creature seed **in the seedsmith**"* from the
...
`tools/seedsmith/seedsmith/adapters/creatures/`, beside `adapters/items/` and `adapters/_stub/`. Two
...
| `codex` | Already the **player-facing discovery UI** — [creature-system-map.md:25](creature-system-map.md) *"Codex = almanac FE with discovery states"*, shipped by `creature-domain-fe` with a `rpg_creature_codex` table carrying `seen`/`discovered` |
...
depend on it representing a concept. Nothing in this document asks PvZ to know what a creature is.
...
> *"build new pipeline that agents will read almanac make see for creature, we will generate item
> specific, action specific, aspect specific, commander specific and something like environment
> specific for each creature"*

Read plainly: agents read the almanac, produce a per-creature record, and five generators consume that
record to produce content that is **specific to that creature** — items, actions, aspects, commander
...
[`CreatureSpeciesGenerator.cs`](../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs)
...
- **FNV-1a over `(typeId, salt)`** is the randomness source —
  [`CreatureSpeciesGenerator.cs:173-189`](../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs),
...
- **The output is committed C#** — `EmitCSharp` at
  [`:192`](../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs) writes
  `CreatureSpeciesCatalog.Generated.cs`, header *"Do not hand-edit — rebalance via the generator, then
  re-emit."* This is what makes gameless-first hold.
- **Derived today:** element primary/secondary, rarity from observed-HP rank
  ([`:75-82`](../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs)), deploy mode,
  variants, trait pool, acquisition flags.
- **24 species shipped** in `CreatureSpeciesCatalog.Generated.cs`.
...
program's consumer: *"This is the generator the Creature program's species catalog already commits to
needing."*
...
`spawn_stats` sample before coverage runs. Any design that assumes rich text for every creature is
...
**What `recipes` is genuinely good for:** fusion lineage — which the shipped `creature-fusion` module
...
and an open-loop review queue. **This is not a neighbouring tool to borrow from — it is the same program.** The creatures feature adds an adapter; it adds no planner, no briefkit, no pipeline.

### WIRING GAP

**W1 — The generator is specced to read lore and portraits, and reads neither.**
[spec-creature-core.md:17](creatures/spec-creature-core.md) commits the tool to read *"`types` catalog for
...
[`CapturedTypeSeed(Side, TypeId, TypeName, DisplayName, HpBase)`](../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs)
— **no lore text, no icon, no attack, no armor, no cost.** `Name` is the raw captured display string
([`:53`](../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs)).
...
[spec-aspect-scope.md](creatures/spec-aspect-scope.md) — *"Status: proposed 2026-08-26, awaiting owner
...
**R1 — There is no creative kernel anywhere, for anything.** Nothing in the repo carries a creature's
*concept*, its motifs, its tone, or what it is explicitly **not**. `CreatureSpeciesDef` is entirely
...
split exists for the item corpus; there is no per-creature equivalent saying *this creature gets 2 items in
...
exactly this class of thing — but no creature→environment relationship is designed anywhere.
...
`legal_combinations()`, `registries()`, `channels()`. A creatures adapter implements those five and
...
|---|---|---|
| item-specific | **the items corpus, as a creature *theme*** | `set` already *requires* `themeKey`; 31 sets are already themed — audit **A3** |
| action-specific | the action corpus, referencing the creature | same reasoning as items |
| aspect-specific | a `KindSpec` in the creatures adapter | genuinely creature-shaped |
| commander-specific | a `KindSpec` in the creatures adapter | genuinely creature-shaped |
| environment-specific | a `KindSpec` in the creatures adapter | genuinely creature-shaped (⚠️ audit **A7** — no consumer in v1) |

⛔ **Corrected by audit A3.** The first version of this table put all five in the creatures adapter.
`Corpus.load` is **single-root**, so creature "items" would have been a *different thing* from real
items — unequippable, and outside the item corpus's own rules. **The creature is a theme**; items and
actions stay in their own corpora and reference it. What remains in the creatures adapter is what is
genuinely creature-shaped, and everything below still applies to those kinds.
...
- **"What each generator is entitled to produce" is already built.** That is `declare`/`fulfil`
  (P5) — a creature's needs become `NeedSpec`s, and reuse-before-generate is the default.
...
- **`_stub` proves the seam continuously.** Its docstring: *"If the core ever reaches into item
  ...
  A creatures adapter is the first real test of that claim.
...
sword, the skill, the aura and the biome sound like **one creature** — *without any generator knowing
...
The mirror of it matters as much: **anti-motifs** — what this creature is explicitly not. Prior art and
...
**The governing rule: the record adds what `CreatureSpeciesDef` lacks; it never restates it.** Element,
...
hashes, so reordering silently moves content that was already generated. Most creature vocabularies want
...
|---|---|---|---|
| `speciesId` | append-only | `CreatureSpeciesCatalog` | join key for everything |
| `gameTypeId`, `side` | derived | `types` | links the creature to the body it wears |
| `displayName` | derived | `almanac_seed` | creature catalog, briefs |
...
|---|---|---|---|
| `families[]` | append-only vocabulary, **multi-valued** | **LLM-extracted from name + description** (R4, §3.5) — not derived, not captured. A creature may belong to several | **partition key** (a creature appears in several partitions), motif inheritance, coverage metric |
| `motifs` (3–5) | append-only **vocabulary**, per-creature selection | vocabulary grows; existing entries must never renumber | all five generators — §3.1's coherence mechanism |
| `antiMotifs` | same vocabulary | what this creature is **not** | drift prevention |
...
1. **It is the natural partition key** — ⛔ *but see audit A5 (§6): with `families[]` multi-valued,
   family coverage no longer implies per-creature coverage, and both metrics are needed.* `Coverage/EmptyPartition` and the stratified sampler then
   work on creatures exactly as they do on items — and the sampler's guarantee ("every non-empty
...
2. **It is a motif inheritance channel.** Everything in the wall-nut family shares *defensive,
   shell, endurance* without each creature restating it — which is the difference between a corpus that
...
B3's measurement makes one thing unavoidable: **most creatures will have thin or absent lore.** A design
that requires rich flavor per creature fails for 87% of plant types. The record must therefore carry
...
from the corpus; stage B assigns each creature into it. Collapsing them into one call invites a model to
invent a fresh family per creature, which produces a taxonomy with no sharing — the exact failure §3.1
...
| `blocked` | neither was sufficient — the creature has no family yet |

`blocked` is not a failure (G1's rule): a creature with no usable text **must** be able to have no
...
**3. It is closed-loop, but only per-item.** "Is this creature in the wall-nut family" has a right
...
`Coverage/EmptyPartition` and the stratified sampler start working on creatures for free — including
...
enrichment lands. A corpus that cannot answer "which creatures were classified from a name only, under
...
The creatures adapter ships with all five `KindSpec`s — item, action, **aspect**, commander,
environment. This is an approval of another program's spec
([creatures/spec-aspect-scope.md](creatures/spec-aspect-scope.md), previously *"proposed, not authorized to
build"*) and it unblocks that module in the creature program independently of this feature. It carries a
byte-identical migration path, so today's trait pools are reproducible after the move.

### ✅ Q6a — family: **PURE LLM EXTRACTION, and a creature may have MORE THAN ONE.** *(owner)*

> Owner: *"Pure llm extraction, base on name and description, a creature maybe have some family, need
...
still earns its place — extract candidate labels per creature, **consolidate into a vocabulary**, then
assign — because collapsing extraction and assignment into one call is what produces a fresh family
per creature and a taxonomy with zero sharing (§3.1, Q7). Consolidation is the step that makes families
mean something; it is not the same step as extraction.

**2. `familyId` becomes `families[]` — multi-valued.** A creature belongs to zero, one or several
families. This is a real change to §3.2's field table and it propagates:

- **As a partition key**, a creature now appears in *several* partitions. That is fine for coverage and
...
- **As a motif inheritance channel**, a creature inherits from every family it belongs to. Q7's
  creatures-per-motif metric becomes more important, not less: multi-membership is exactly how a
...
A name-classified creature counts as **unfinished** in coverage — the same discipline
...
emitted JSON under `data/seed/creatures/`. Q7 motif budget = a metric (creatures-per-motif), not a cap.
...
sounds like one creature. Give five generators `shell, endurance, patience` and they produce *Shell of
...
derive from **the same string — the name**. Every "-nut" creature then gets nut-ish motifs *and* the nut
...
twice. **And every metric reports success** — creatures-per-motif looks high (real sharing!), families
...
from `name` (Q6b). Extend it: where a creature's motifs **and** families are both `basis = name`, that
creature's family-motif agreement is excluded from the sharing metric rather than counted as evidence
for it. Enrichment (Q6b) is what eventually breaks the tie — another reason it is a named pipeline
rather than a hope.

### ⛔ A3 — "Item specific" does not belong in the creatures corpus. **Changes the architecture, for the better.**

**The attack.** §3.0 listed five kinds including `item`, all in the creatures adapter. But
`Corpus.load(root)` is **single-root** — one corpus, one directory — and items live in
`data/seed/items/`. So either creature items are a *different thing* from real items (a player cannot
equip them; the feature is cosmetic), or the creatures feature writes into another adapter's corpus (and
...
**So a creature is a theme.** Items stay in the items corpus and reference the creature; the creatures feature
...
- A creature's signature gear is naturally a **set** — already theme-required, already has members and
...
### ✅ A4 — `numerics` does not apply to creatures, and that is load-bearing.

`adapter.channels()` is consumed by exactly one subsystem — `numerics`
([`numerics/model.py:68`](../../tools/seedsmith/seedsmith/numerics/model.py)). A creatures adapter has no
...
buys "no family goes ungenerated". If a creature belongs to three families, **a handful of multi-family
creatures can satisfy every partition** while most of the roster gets no content — and coverage reports
green.

**The fix.** Family coverage answers *"is every family represented"* — a real question, but a
different one from *"does every creature have content"*. Both are needed, and the second is the one a
...
deliberately, exactly as `CreatureSpeciesCatalog.Generated.cs` already is. Either is fine. Silently
...
what a creature is. No new power-shaped scale (A4). No SQL outside `FusionRpg.Data` (Q5 keeps the corpus
as emitted files).
...
[x] I identified the subsystem(s) this touches.
    — creatures, almanac, data/SQL, power/scaling, tunables, the atom layer, seedsmith.
[x] I read every doc in the §1 row(s) for those subsystems, this session.
    — creature-system-map.md, creatures/spec-creature-core.md, creatures/spec-aspect-scope.md (§1),
...
[x] I verified claims against CODE, not comments.
    — CapturedTypeSeed's field list, the FNV-1a salt discipline, the rarity-from-HP-rank rule and
      EmitCSharp were read in CreatureSpeciesGenerator.cs, not inferred from the spec.
...
[x] I tested (not assumed) any constraint I am reporting.
    — the two name collisions were grepped, not assumed: `almanac_seed` is a real specced table and
      `codex` is a real shipped table + FE module. `creature-forge` was grep-checked for collision.
...
[ ] Corrections are propagated to prose, Structure, Testing, Boundaries, map, and tasks.
    — **Not yet, deliberately.** This is an idea doc; nothing is approved. `creature-system-map.md`
      gains no module row until the owner accepts a program here. **W1 is the exception worth
      flagging now**: the gap between spec-creature-core.md:17's four named sources and
      CapturedTypeSeed's actual fields is a real inconsistency in a shipped module, and it is worth
      recording against `creature-core` whether or not this program ever exists.
```
