...
| `content-completeness-creatures` | Adopt for creature species — `_provenance` already real and written; this module adds the missing-field metric and wires automatic backfill to it | `content-completeness-core` |
...
Build order: `content-completeness-core` → `{items, actions, creatures, dungeon, passive-tree}` (parallel) → `passive-tree-identity-content`.
...
- **Each domain gets its own module** because each is independently testable (a domain's own
  ...
  genuinely different starting point (items: mostly there; actions: nothing; creatures: provenance
