...
The owner asked where the creature species catalog is generated. There are two distinct patterns, and the
item program needs a third that borrows from both.

**Shape A — offline emitter, checked-in output.**
`tools/CreatureCatalogGen/Program.cs` reads captured types through the DAL (no SQL of its own), calls
`CreatureSpeciesGenerator.Generate(seeds)`, runs `CreatureSpeciesCatalog.Validate(species)`, and writes
`src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.Generated.cs` — 24 species, with an `<auto-generated>`
...
`CreatureRecipeCatalog.All = Build()` (`src/FusionRpg.Core/Creatures/Fusion/CreatureRecipeCatalog.cs:47`) derives
every fusion recipe from `CreatureSpeciesCatalog.All` at first touch — one recipe per summonable rare+
...
1. **The roster-scale gear economy** ([item-ideal.md](../item-ideal.md) §8). **Nine of the thirteen
   ...
   ≈ 10 days to gear). If the answer is "all twenty creatures", **drop volume is 4× too low, every number
...
[x] I verified claims against CODE, not comments, wherever a lane's cadence depended on it:
    ...
    field at all), CreatureSpeciesCatalog.Generated.cs, tools/CreatureCatalogGen/Program.cs,
    CreatureRecipeCatalog.cs:47. The finding that t4/t5 are unreachable is read from code and
