...
| `stat.derived` using `Replace`/`Flag` ops in Sim | ⚠️ **Amended 2026-09-06 (mechanism-wiring E5):** no longer a blanket block — `AtomKindRegistry.cs:561`'s SIM slot moved `None → RuntimeState.Partial` once `ActorDerivedLookup`'s contribution fold gave it a real consumer. `Flat`/`Increased` now compose correctly (a plain sum matches `DerivedComposer`'s own answer for both); `Replace`/`Flag` still silently miscompose as if `Flat` — `BoundDerivedAtom` carries no `Priority` field, so `Replace`'s ordering cannot be honoured. Proven empirically by `EffectOfflineKitTests.The_four_derived_ops_decide_Full_versus_Partial`. A squad-harness sweep can now score a `stat.derived` node using `Flat`/`Increased` in Sim; one using `Replace`/`Flag` still reads a silently wrong number, not a rejection |
