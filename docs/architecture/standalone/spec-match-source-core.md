...
Success looks like: given a creature squad and a wave definition, the server resolves a deterministic battle, the pipeline records it indistinguishably from any other run (source-tagged `web`, game `webrpg-1`), and every existing projection (runs list, facts, XP, Souls) just works — provable entirely in CI.
...
- **Inputs:** squad = creature specimen snapshots (level, element typing, traits, equipment) composed into derived stats via the existing **ActorHub** compose path (server-side profiles, as the status prove-packs already do); wave = enemy roster from a code-authored `WaveCatalog` (species + counts + level scaling).
...
- **Output `BattleReport`:** ordered event list in the **existing kind vocabulary — lean profile**: `board.start`, `zombie.spawn`/`plant.spawn` analogues, `zombie.die`/`plant.die`, `match.result`, **`board.end` (mandatory — runs never close or compact without it)**. A die record may carry an authoritative `killerPtr` when the BattleEngine resolved the lethal interaction with an explicit attacker; attackerless/status/environment/sweep deaths omit it. This is additive metadata on the existing lifecycle event, not a second die event and not a new event kind. **No per-attack damage events**; per-actor tallies live in the report and persist via `snapshot_json`. ~40 events per 20-actor battle. Actor ids use the synthetic scheme `web:{matchKey}:{n}` — load-bearing for `entities` uniqueness, Activity fact dedupe, XP-ledger dedupe, and non-collision with unique-actor `last_ptr` matching. The engine emits no wall-clock timestamps; `WebMatchService` stamps strictly monotonic `t` values at ingest.

### Death attribution (server-owned battles only)

`BattleEventRec.KillerActorKey` is assigned at the point where the resolver already knows both the attacker and the victim. `BattleReportEmitter` maps that key through the stable `web:{matchKey}:{n}` actor map and writes `killerPtr` only when the key resolves. It never infers a killer from report order, a last-attacker cache, or server arrival time. Live PvZ events remain an observed Unity stream; this module does not shadow-simulate them or emit replacement `plant.die`/`zombie.die` records. See [battle-death-attribution-events](../battle/spec-death-attribution-events.md).
...
6. Suppress almanac type-XP awards (`ZombieSpawned`/zombie-kind) for `webrpg-1` runs, and keep creature species ids in a disjoint id space (≥10000) — web battles must not level PvZ almanac progression. (Related pre-existing fix, filed separately: the XP ledger dedupe is run-unscoped; defeat XP dedupes on the literal `"run"`.)
...
- **Determinism:** same setup+seed twice ⇒ identical reports, including killer keys and die-event order (golden files for 3 canonical battles: stomp-win, close-win, wipe).
...
- **Pipeline e2e (SIM):** run a web match → assert run row (game `webrpg-1`), Activity facts, and XP ledger all appear with correct dedupe (Souls join this assertion once the soul-economy module exists — they are a projection this pipeline *enables*, not one that exists today); replayed correlation adds nothing; a lethal attacker produces the matching `killerPtr`; attackerless deaths omit it; a live-PvZ-concurrent web match leaves the PvZ grant session untouched.
...
- **Always:** engine purity (no I/O/clock); reuse existing event kinds; assign killer identity only from explicit battle-owned cause context; seed recorded on every run; correlation idempotency.
...
1. Three golden battles locked and deterministic, including death attribution. 2. Subsystem-integration tests prove shared math (element/status/trait effects visible in outcomes). 3. SIM e2e: a web match produces run + facts + XP + Souls with zero injector involvement and preserves attackerless deaths as unattributed. 4. Guards green; all existing suites green. 5. `runs` list in the FE shows web matches alongside PvZ runs with their profile id.
