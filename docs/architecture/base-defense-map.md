...
| `battle-stage` | **Decision 44** — `#/battle` as **playback of a resolved `BattleReport`** on the generic layer. Retires the dead stage id **by using it**; proves `board-render` generic with a genuinely different second consumer. **Thin is a constraint**: if a requirement is not derivable from a report that already exists, it is out of scope. ⛔ **PAUSED 2026-09-06 before any task started** — same Phaser-refactor block as `siege-stage`; this module leans on `board-render`'s own generic layer, which the same audit found has zero real consumers today | `board-render` |
