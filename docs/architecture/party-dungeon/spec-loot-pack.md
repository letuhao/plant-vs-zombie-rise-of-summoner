...
| `PackDto` (no `PartyIndex` in a label — the stage names parties; each cell carries `movable` so an autopilot party's grid is disabled at the model, never by a client-side rule — `spec-delve-stage.md` §18 ask 5) and the two decision payloads | **`delve-stage`** (wave 5): the band-2 pack layer (`decisions.md:113`; `design/information-architecture.md:113-118`) owns the grid UI, this module the model. Today the web renders no armoury: `StoragePage.tsx:3-12` is run/archive storage, `/api/items/armoury` (`ItemSurfaceEndpoints.cs:72-106`) has no web caller and its `ArmouryRowDto` hard-codes `Assigned: false` (`:83`) |
