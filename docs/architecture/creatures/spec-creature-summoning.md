# Spec: creature-summoning (V1 wave 4 — the V1 ship gate)

Module id `creature-summoning` in the [creature system map](../creature-system-map.md). Depends on `creature-core` + `soul-economy`. V1 ships when this module and its minimal FE are playable end-to-end.

## Objective

Souls-funded summoning that mints real creature specimens into the roster. Per the adopted design rules: gacha is a **parallel** acquisition path (capture arrives later), rarity represents *potential and uniqueness* rather than raw power, and every pull mints an **individual** — "duplicates" are distinct specimens whose value deepens when fusion lands.

Success looks like: the player earns Souls by playing, spends them at the Summon panel, watches results reveal with rarity/variant/element/traits, and finds the new creatures in the roster and Codex — fully offline-provable in SIM.
...
- `standard-rift` — **100** Souls/pull, 10-pull **900**. Pool = every `summonable` species (acquisition flag from creature-core), weighted common 74% · rare 20% · epic 5% · legendary 1%.
...
```
POST /api/creatures/summon { playerId, bannerId, count(1|10), correlationId }
  ...
  → after commit: SignalR CreaturesUpdated + SoulsUpdated → return results
```
...
- **Lock + nickname at reveal**: optional rename in the reveal flow; `locked` sorts to top and protects from future fusion consumption. These two are the cheapest possible implementation of "creatures are individuals" and belong at V1, not later.
- Reserve banner text: "Reserved creatures become fusion and ritual material in a future update" — sets the hoard expectation explicitly.
...
### FE (minimal V1 slice of `creature-domain-fe`)

`#/creatures` page: Souls balance header · Summon panel (×1 / ×10, disabled below cost, **pity counters displayed**, results reveal ordered common→legendary with nickname/lock controls) · Active/Reserve roster (portrait from linked type icons, rarity frame, element badges, traits) · Codex tab (species grid: undiscovered = silhouette + `???`; discovery rewards from the soul-economy faucet surface here). Uses the existing bus layer (`lib/bus`) — no direct fetch.

### V1 playable floor (resolved 2026-08-21)

**V1 is an internal gate, not an announced release** — the owner declined the Patron-creature bridge, so the first player-facing announced ship is **expeditions**. V1 still includes the near-free floor items: visible pity counters, nickname/lock at reveal, and codex discovery rewards. No injector work in this module.
...
```
src/FusionRpg.Core/Creatures/        → SummonBannerCatalog.cs, SummonRoller.cs (pure, seeded-RNG injectable)
src/FusionRpg.Contracts/          → summon DTOs in CreatureDtos.cs
src/FusionRpg.Data/Sqlite/        → summon log + atomic pull in RpgStore.Creatures.cs / RpgStore.Souls.cs
src/FusionRpg.Server/             → summon endpoint in CreatureEndpoints.cs
web/fusion-rpg-web/src/features/creatures/ → SummonPanel, CreatureRoster, CodexGrid + bus queries/mutations
...
Reveal animation polish and a dedicated `#/domain` shell are deferred to the fuller `creature-domain-fe` module.
