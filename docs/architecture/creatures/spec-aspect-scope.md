...
**Module id:** `aspect-scope` · **Program:** [creature-system-map.md](../creature-system-map.md) ·
**Status: APPROVED by the owner 2026-08-31. Authorized to build.**

> Approved while resolving the open questions of
> [seedsmith-creatures-ideal.md](../seedsmith-creatures-ideal.md) §5 Q2: that feature ships an `aspect`
...
**Depends on:** `creature-core` · **Blocks:** `point-economy` (its third scope) in the class-system program

> **⛔ Ownership moved 2026-08-26, by owner decision.** This spec was written as class-system module 4
> and now belongs to the **creature program**, because **every file it edits is that program's**:
> `CreatureSpeciesCatalog`, `CreatureSpeciesGenerator`, and the checked-in generated catalog. The creature map's
> `creature-core` already owns *"species link, rarity, variants, trait slots, **element typing**"* — this
...
> tests are supplied here so the creature program schedules a specified change rather than a request.
>
...
**The one real migration.** [CreatureSpeciesCatalog.cs:16-23](../../../src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.cs)
...
**Users:** `point-economy` (scope 3); the creature roster; whoever later builds `element_mastery`, which
...
```csharp
// CreatureSpeciesGenerator.cs:65 today
...
[CreatureSpeciesGenerator.cs:125-140](../../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs)
...
([decisions.md](../decisions.md) *Creature program*). So 20 species × 6 elements is **120 generated
...
| `variant` | **Taken and means something else.** `CreatureSpeciesCatalog.KnownVariants` ([CreatureSpeciesCatalog.cs:31-34](../../../src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.cs)) is a shipped closed list — `normal · ancient · mutated · corrupted · blessed · cursed · shiny` — i.e. **cosmetic-rarity finishes**. Adopting it creates exactly the collision `race` was avoided for |
...
| **`aspect`** | **Chosen.** Free everywhere in `src/`, reads correctly (*"the fire aspect of Peashooter"*), implies more than a damage type, and carries no biological framing — which matters because zombies and creatures take element typings too |
...
| Trait pools, generated per species | `CreatureSpeciesGenerator.TraitsFor(rarity, typeId)` | **Gains an argument** |
| `ElementPrimary` / `ElementSecondary` / `TraitPool` | on `CreatureSpeciesDef` | **Down one tier — this is the migration** |
...
So either (a) `CreatureSpeciesDef`'s element fields do not reach a hashed path, in which case the
...
git diff --stat src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.Generated.cs
...
```text
src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.cs             ElementPrimary/Secondary/TraitPool leave the def
src/FusionRpg.Core/Creatures/AspectCatalog.cs                   new: (speciesId, element) -> traits, starting skills
src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs TraitsFor gains `element`; generates aspects
src/FusionRpg.Core/Creatures/CreatureSpeciesCatalog.Generated.cs    regenerated, checked in
tests/FusionRpg.Core.Tests/Creatures/AspectCatalogTests.cs
```
...
Match `CreatureSpeciesCatalog`'s shipped shape exactly — a `sealed record` of `init` properties, a
...
1. `ElementPrimary`, `ElementSecondary` and `TraitPool` no longer sit on `CreatureSpeciesDef`.
...
```
[x] Subsystems identified: creatures/species, elements, stats, battle goldens, power scale.
[x] Read this session: DESIGN-GATE.md, decisions.md (Creature program, Element Hub SSOT, Shield layer,
...
[x] Verified against CODE: CreatureSpeciesCatalog.cs:16-23 (the three fields on the def),
    CreatureSpeciesCatalog.cs:31-34 (KnownVariants, the seven cosmetic finishes),
    CreatureSpeciesGenerator.cs:65 and :125-140 (TraitsFor and its hash salts),
...
[x] Corrections propagated - §3 corrects the earlier "this tier has no shipped home" claim in place.
    OWNERSHIP moved to the creature program 2026-08-26: creature-system-map.md gains its module row,
...
- [decisions.md](../decisions.md) — *Creature program*, *Element Hub SSOT*, *Golden ordering across streams*
