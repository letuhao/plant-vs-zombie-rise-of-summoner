# Spec: creature-fusion

Status: **shipped 2026-08-21** (all eight owner locks implemented; star merges, promotion, recipe fusion, and the recipe book live in SIM/FE at `#/fusion`; the E2E legendary chain proves commons → legendary purely via fusion). Module id `creature-fusion` in the [creature system map](../creature-system-map.md). Depends on `creature-core`, `soul-economy`, and the expedition material inventory. Consumed later by `patron-creature` (star scaling) and `creature-contracts`.

## Objective

Give duplicates and low-rarity creatures lasting value (design rule 2) and open the build-crafting midgame (rule 3): every spare creature is either star-fuel for an individual you keep, or an ingredient in a discoverable recipe. Fusion is the anti-gacha — you always know the species you'll get.
...
8. **Patron creature un-parked:** builds immediately after this module (separate spec).
...
- `CreatureRecipeCatalog` (Core): **amended 2026-09-06 (`creature-seed` modules 17-18, `fusion-recipe-generator`/`-runtime`, T8.3-T8.5)** — recipes are no longer computed live at startup. A deterministic pass (same same-rung pairing rule as originally shipped) covers the overwhelming majority of outputs; for the rungs that mathematically cannot support a unique pair per output (the real corpus's own Almanac/Sunwoven shortfall — 20 eligible outputs, only 4 rungs-below candidates), a bounded, per-member-voted LLM proposal fills the gap, validated and reconciled by a deterministic reconciler that is the SOLE write authority. The result is committed as a seed (`data/generated/creatures/_fusion-recipes.json`, `Configure`d at startup like the species catalog) — never recomputed in the running game. **Best-effort, not exactly-one-per-species:** most summonable rare-and-above species get a recipe; an output whose gap-fill vote never converges ships with NO recipe, named as `unresolved` in the generator's own run report — never fabricated. Inputs: two species from the band below (same rung) whose elements relate to the output, OR — only for a flagged `crossRungGapFill` recipe — from the nearest 2-3 populated rungs below when the same rung cannot supply a unique pair. Capture-only species get no recipe and appear in no recipe (owner lock 6, unconditional, enforced by both the deterministic pass and the reconciler). Validated at import (unknown ids, capture-only inputs, duplicate pairs reject).
...
- `rpg_creature_profiles`: `star INTEGER DEFAULT 0`, `promoted INTEGER DEFAULT 0` (EnsureColumn).
- `rpg_creature_lineage`: append-only (instance_id, event `star-merge|promotion|recipe-birth|consumed-by`, detail_json, t) — the individuals-with-history rule made durable.
...
- `POST /api/fusion/preview` (pure read: given base/inputs → mode, costs, result species or `recipe.unknown`, pickable traits); `POST /api/fusion/execute` (correlationId ≤64, server seed, hub pushes CreaturesUpdated/SoulsUpdated); `GET /api/fusion/{playerId}/recipes` (discovered + silhouettes). Wire projections never leak seeds.
- FE `#/fusion`: lab with base slot + sacrifice tray (locked/expedition specimens greyed), recipe browser with silhouettes, cost panel (Souls/shards/essences with have/need), pick-one trait selector, result reveal; star pips render on roster cards here and in `#/creatures`/`#/expeditions`.
...
```
src/FusionRpg.Core/Creatures/Fusion/   → CreatureRecipeCatalog.cs, StarPolicy.cs (incl. FusionCostTable), FusionRoller.cs
...
- **Ask first:** cost-table or star-bonus tuning (game balance); making stars affect PvZ deploy before `patron-creature`.
- **Cleared, not silently taken (2026-09-06, `creature-seed` T8.3):** "recipe-graph shape changes" above once meant no recipe could ever draw its two inputs from more than one rung below the output. The owner authorized `crossRungGapFill` — a bounded subset of recipes (the real corpus's own Almanac/Sunwoven shortfall, ≤15 outputs) may draw inputs from the nearest 2-3 populated rungs below when the same rung cannot supply a unique pair. Every other recipe's graph shape (same-rung inputs) is unchanged.
