# Spec: patron-creature

Status: **approved anchors 2026-08-21** (owner decisions below); implementation not started. Module id `patron-creature` in the [creature system map](../creature-system-map.md). Depends on `creature-summoning` (creatures to designate) and `creature-fusion` (stars scale the aura). First creature module with **injector scope** — the build ends at a LIVE gate that needs the owner's eyes, not just SIM.

## Objective

One designated creature — the patron — stands behind the summoner during live PvZ matches: its element becomes a small typed aura on the player's plants, its stars/rarity/level set the strength, and its presence sweetens Soul earns. This is the first bridge from the web RPG back into the game, deliberately small: one grant, one read path, zero new hot-path work.
...
- `rpg_patron` (player_id PK, instance_id, set_utc, revision). Set/switch is one transaction: replay-check → validate specimen (owned, creature profile, not Retired) → first-set-free else `TrySpendSouls(100, "patron", correlation)` → upsert + revision.
...
- FE: "Make patron" action on roster cards (`#/creatures`), patron badge with aura preview (element, ‰, next-switch cost); patron shown on `#/fusion` trays as unconsumable.

### Explicitly not in this module

Trait echo (recorded extension), multiple patrons, patron XP from PvZ, aura effects on hypno-ally creatures, any change to Soul kill-cap/victory math.
...
```
src/FusionRpg.Core/Creatures/Patron/   → PatronPolicy.cs (magnitudes + earn bonus math)
