...
Module id `soul-economy` in the [creature system map](../creature-system-map.md). Depends on `creature-core` (spend targets), feeds `creature-summoning`.

## Objective

Souls are the strategic currency of the creature game: earned from real play, spent on summoning (V1) and later on rituals, fusion, and Domain upgrades. The vision's rule: the player should regularly face meaningful spend choices.
...
- `MatchEnded(victory)` **+100 with repeat decay**: full for the **first 3 victories per UTC day**, 50% thereafter (rested-bonus shape — caps the grind ceiling without punishing normal play; this is the review's sanctioned level-free variant — per-level tracking was judged not worth the ledger schema it needs, revisit if farming one trivial level 3× daily proves abusable).
- **Codex discovery faucet**: first-ever discovery of a species pays by rarity — common 25 · rare 75 · epic 200 · legendary 500; codex milestones pay 500 at ≥50% and 1,500 at **≥90%** discovered (the 90% threshold is the guardrail that keeps the full milestone claimable by web-only players once capture-exclusives exist; both awarded inside the summon transaction, dedupe `codex:half`/`codex:full`). Gives pulls an information axis of value beyond inventory.
...
```
src/FusionRpg.Core/Creatures/        → SoulEarnPolicy.cs
