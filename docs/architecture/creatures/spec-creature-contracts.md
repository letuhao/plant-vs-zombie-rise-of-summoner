# Spec: creature-contracts

Status: **shipped 2026-08-21** (Checkpoint G passed; owner decisions below). Module id `creature-contracts` in the [creature system map](../creature-system-map.md). Depends on `creature-core` (specimens + profiles), `soul-economy` (the ledger this module spends from), and `creature-fusion` (the release valve a capacity cap makes meaningful). **Pure standalone scope** — server + web only, no injector work, no PvZ dependency; every rule here is provable with the game closed.

## Objective

Turn "creatures I own" into "creatures who serve me." A contract binds one creature to the summoner: it occupies a slot, it carries loyalty that rises with use and rots when tribute goes unpaid, and it can refuse to be fielded. Owning stays unlimited; **fielding** becomes finite and earned.

Success looks like: a player with 40 creatures has 12 under contract, sees the daily Soul tribute their army costs, buys a 13th slot for 300 Souls, watches a neglected creature slide from Devoted back to Bound, and gets a flat refusal when they try to send an unbound imp on an expedition.
...
3. **Disobedience is a hard refusal.** Below the deploy floor a creature simply cannot be fielded — no stat penalty, no RNG roll, no new random stream.
4. **Decay floors at the deploy threshold.** Time can strip everything a creature *earned*, never its deployability. Only battle defeats can push a creature under the line.
5. **Capacity = base slots + Soul-priced upgrades**, rising price — the permanent late-game sink the economy lacks.
6. **Upkeep is per contracted creature, rarity-scaled, charged daily**, settled lazily from the balance.
...
Every creature is in exactly one of three conditions, and every refusal in this module names one:
...
Ranks (loyalty → rank, own-channel bonus in per-mille):
  ...
  600–799   Trusted          +35‰
  800–1000  Devoted          +60‰
Gains        battle/expedition won with the creature  +15
             lost                                  −10   (may cross the floor)
             daily gain cap per creature              +60   (losses are uncapped)
...
**Personality** (rolled at mint from the specimen's seeded stream; pre-existing creatures derive one deterministically from `instanceId`) — integer percentages applied as `x * pct / 100`:
...
2. For each elapsed day, in order: `due = Σ upkeep(bound creatures)`. If balance ≥ due → `TrySpendSouls(due, "upkeep", dedupe "upkeep:{playerId}:{yyyy-MM-dd}")`. If not → **no charge, every bound creature decays** that day (floored at `DeployFloor`). All-or-nothing per day: you either paid the tribute or you didn't.
...
Cost ceiling: ≤ 48 bound creatures × ≤ 30 days of integer math, ≤ 30 ledger rows. Nothing here scales with roster size beyond the slot cap.

### Migration — one-shot auto-bind

**Built addition (plan §Wave G decision 1):** a newly minted creature — pull, fusion output, or wild join —
...
every creature acquired after migration would land unfieldable behind a button press with no decision
...
Contracts guard every path that *fields* a creature, and nothing else:
...
| PvZ deploy | `TryBeginDeploy`, **only for specimens that have a creature profile** | `contract.unbound`, `contract.insubordinate` |
| Patron designation | `SetPatron`, before pricing | `patron.unbound`, `patron.insubordinate` |

Unique actors without a creature profile are untouched on the PvZ path — a test pins that, because that path predates creatures entirely.

Releasing is refused for a creature that is on an expedition (`contract.on-expedition`) or is the active patron (`contract.is-patron`) — mirroring the fusion guard. Fusion itself needs no new rule: **unbound creatures are fully consumable, and that is the point** — the capacity cap is what finally gives fusion its pressure. Retirement (fusion consumption) releases the contract inside the same transaction, freeing the slot.

### Data (all DDL in `FusionRpg.Data`, `EnsureColumn` migration style)

| Table | Key | Columns (essence) |
|---|---|---|
| `rpg_creature_contracts` | `instance_id` (FK → `rpg_unique_actors`) | `player_id`, `bound` (0/1), `loyalty`, `personality`, `bound_utc`, `released_utc`, `gain_day` + `gain_today` (the daily gain cap window), `revision` |
...
- `GET /api/contracts/{playerId}` — settles first, then returns capacity (used / total / next slot price), total daily tribute, and per-creature rows (rank, loyalty, personality, upkeep, deployable).
...
```
src/FusionRpg.Core/Creatures/Contracts/  → ContractPolicy.cs, LoyaltyRank.cs, CreaturePersonality.cs
...
web/fusion-rpg-web/src/               → lib/bus/contracts.ts, features/creatures/ contract badge + capacity header
...
- **E2E (SIM):** bind → dispatch expedition (ok) → release → dispatch (`specimen.unbound`) → ritual on an insubordinate creature restores deployability → advance 3 days → tribute charged exactly, balance exact.
...
1. Capacity is server-authoritative — every fielding path refuses an unbound creature, with the PvZ path leaving non-creature actors untouched.
2. Upkeep settles idempotently, is bounded at 30 days, and an insolvent day decays instead of charging.
3. Decay never drops a creature below `DeployFloor`; only defeats do.
...
Whether a creature left insubordinate for a long stretch should eventually break its own contract (auto-release, freeing the slot) — deferred; it interacts with `world-events` and needs no answer to ship this module.
