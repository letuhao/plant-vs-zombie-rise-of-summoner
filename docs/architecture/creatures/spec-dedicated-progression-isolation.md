...
Ensure a unique creature always resolves through its specimen progression and a Commander contribution always resolves through Commander progression. Neither path may receive the empire general-creature species fallback, even if it has the same species or `typeId` as an eligible lawn general creature.
...
This module updates dedicated-source composition and rewards in the web-battle/expedition consumers and establishes the isolation rule the lawn consumer must obey. Lawn-specific unique XP attribution and participation settlement belong to [creature-lawn-deploy/spec-lawn-deploy-progression.md](../creature-lawn-deploy/spec-lawn-deploy-progression.md). It covers:
...
|---|---|---|
| `UniqueSpecimen(playerId, instanceId)` | The owned `rpg_unique_actors` row, specimen level, and the existing `UniqueCreatureAllocation` path when a dedicated allocation plan is available. | `EffectiveSpeciesAllocation`, `SpeciesAllocationSource`, species XP. |
...
Unique ActorHub and web-battle composition now consume Commander plus persisted `UniqueCreature`
...
- `src/FusionRpg.Server/UniqueActorHubCompose.cs:38-56` now loads Commander plus `UniqueCreature` allocation; no species fallback remains.
...
- `src/FusionRpg.Core/Stats/Aptitudes/UniqueCreatureAllocation.cs:29` is the existing dedicated allocation primitive; do not duplicate its budget logic in server or web code.
...
- A unique lawn spawn sharing a `typeId` with a general creature cannot enter the generic species run-completion set.
- Absence of a unique allocation plan results in an explicit empty dedicated allocation, never a general fallback.
- General-creature behavior remains owned by `general-empire-fallback` and is not reimplemented here.
...
Add regression tests proving that a unique and a general creature with the same species have different resolution paths; a unique with no plan stays empty rather than inheriting species points; Commander-only composition has no species contribution; and expedition resolution no longer records species XP for unique results.

## Design-gate checklist

- [x] Read the design gate, creature system map, decisions, unique runtime, data/runtime architecture, ActorHub SSOT, and power-scale SSOT in this session.
