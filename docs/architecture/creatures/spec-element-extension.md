...
Module id `element-extension` in the [creature system map](../creature-system-map.md). Prerequisite for all creature typing. **Touches a design-locked area** — ships with a decisions.md amendment, and nothing else in the creature program starts until its golden tests are green.

## Objective

Extend the ElementHub roster from `{omni, fire, ice, air, earth}` to also carry **`light`** and **`dark`**, so creature species can be typed with the vision's celestial/infernal identity. `void`/`chaos` from the vision are explicitly **not** elements — they become rare traits later.
