# Capability map: `creature-seed`

**Status:** proposed 2026-09-01, after the twenty-six cleared questions in
[creature-seed-ideal.md](creature-seed-ideal.md). **Eighteen modules** (fourteen at first draft;
...
That is [item/seed-contract.md](item/seed-contract.md) §1's law, applied to creatures:
...
almanac_seed · spawn_stats · recipes         SQLite, captured from the game
        ...
        v
data/seed/creatures/_dump/**.json               development SOT · capture stamp · content hash
        |  power-parse · threat-band · classify-pipelines   (seedsmith)
        v
data/seed/creatures/species/**.json             THE ANCHOR = THE SEED — enums only, no numbers
        |  species-generator   (deterministic: Theta, P(Theta), aptitude share, rarity bands)
        v
data/generated/creatures/**.json                CONCRETE — checked in, diffable, reviewable
...
just point it at creatures" is reading it wrong.

**Corrected 2026-09-05 — the middle stage has landed.** `data/generated/creatures/` holds **830 committed
JSON files**, one per concrete species, all tracked (counted this session; first committed 2026-09-04 in
`52713f4`). The generator exists too: `CreatureSpeciesGenerator` / `SpeciesExpander` /
`ConcreteSpeciesSerializer` under `src/FusionRpg.Core/Creatures/Generation/`, with the import side in
...
pipeline pointed at creatures.

---

## 2. Five findings that shaped this map, each verified in code

**① `CreatureSpeciesGenerator` has no production callers.**
[CreatureSpeciesGenerator.cs](../../src/FusionRpg.Core/Creatures/Generation/CreatureSpeciesGenerator.cs) is
referenced only from `tests/FusionRpg.Core.Tests/Creatures/CreatureCatalogTests.cs`. Its *output* is
...
[CreatureCorpusEmit/Program.cs:45-52](../../tools/CreatureCorpusEmit/Program.cs#L45-L52) does
`foreach (var s in CreatureSpeciesCatalog.All) -> store.GetAlmanacSeed(s.Side, s.GameTypeId)`. It walks
...
**③ `ssot-rarity.md` §4.3 does not merely omit creatures — it refuses them.**

> *"**Creatures keep their own ladder.** `CreatureRarity` stays a four-value code enum"* … the band map
...
| 8 | `anchor-emit` | Writes `data/seed/creatures/species/**.json` plus provenance (dump hash, prompt version, basis, vote record). One full re-derivation, append-only after (Q19) | — | 7 |
...
| 10 | `rarity-migration` | `CreatureRarity` 4 -> 10 rungs, its consumers, and the `ssot-rarity.md` §4.3 reversal (Q4, Q24) | — | — |
| 11 | `species-generator` | **Seed -> concrete.** Anchor enums -> every number: `Theta`, `P(Theta)`, aptitude allocation, rarity count band and tier window, tempo/reach as real intervals. Writes `data/generated/creatures/` | — | 8, 10 |
| 12 | `species-import` | Concrete JSON -> SQLite in one all-or-nothing transaction, extending `tools/AtomImporter`'s discipline | — | 11 |
| 13 | `catalog-runtime` | **The risky one.** The nine production readers of `CreatureSpeciesCatalog.All` move to store-backed reads | — | 12 |
| 14 | `roster-metrics` | Distribution guard over element pair x aptitude x threat band x rarity. **The D2-Hammerdin control** | — | 8 |
| 15 | `species-effects` | **The container.** Anchor -> a `species-passive.{speciesId}` seed: fixed core, affix pool, affinity ordinals, eligibility tags. Without it a generated creature has a stat block and no effects | **yes** | 8, 10, **effect-pipeline** |
...
| 17 | `fusion-recipe-generator` | Distribution index over the real concrete roster finds rarity rungs whose input pool cannot support a unique pair per output (found real, 2026-09-05, confirmed by the built `tools/CreatureRecipeDistributionIndex`: 20 eligible `Almanac` outputs vs. 4 `Sunwoven` inputs); an LLM proposes a cross-rung pairing only for those deficits; a deterministic reconciler validates and writes the committed recipe seed | **yes, bounded to shortfall rungs only** | 11 |
| 18 | `fusion-recipe-runtime` | `CreatureRecipeCatalog` moves from live in-process computation to a `Configure`d read of the committed seed — the same seam `catalog-runtime` already proved for the species catalog, applied to its sibling | — | 17 |
...
generator — we just generate creature species without ship atom container for it."*

**He was right.** The first fourteen modules took an almanac entry all the way to a stat block and a
runtime catalog, and **not one of them produced an effect container.** A creature generated by that map had
...
**⛔ `creature-seed` cannot finish on its own.** Module 15 needs an affix library, a slot mechanism and a
...
`CreatureRecipeCatalog.Build()` — `spec-creature-fusion.md`'s own "code-generated from the species catalog"
...
capacity, but silently violated `CreatureRecipeCatalogTests`'s own tested invariant that both of a
...
pipeline for fusion with deterministic engine read the whole creature species seed and make distribution
...
- **17 `fusion-recipe-generator`** — dev-time. Distribution index (deterministic) finds shortfall
  ...
  of all 829 recipes. (Verified live via the built `tools/CreatureRecipeDistributionIndex`, 2026-09-06.)
- **18 `fusion-recipe-runtime`** — the same `Configure`/`UseScoped` seam `catalog-runtime` already
  proved for `CreatureSpeciesCatalog`, applied to `CreatureRecipeCatalog`'s much smaller shape (a single
...
| [item/ssot-rarity.md](item/ssot-rarity.md) §4.1, §4.3 | Creatures adopt the ten-rung ladder; the four-row band map becomes a migration shim with an end date, not a permanent wall | `rarity-migration` | ✅ **Done** — §4.3: *"Reversed 2026-09-01 … Creatures no longer keep a separate four-value ladder"*; the four-row band map is explicitly labeled a migration shim only |
| [item/seed-contract.md](item/seed-contract.md) | Its status line stops saying nothing is authorized, or creature seeds declare their own subtree under the same law | `anchor-emit` | ✅ **Done** — status line: *"authoring authorized as of 2026-09-01 … the prior wording … was found to contradict its own dependents"* |
| [creatures/spec-creature-fusion.md](creatures/spec-creature-fusion.md) §"Recipe fusion" | *"built deterministically from `CreatureSpeciesCatalog` at startup"* -> a deterministic pass PLUS a bounded, LLM-filled gap for rungs the deterministic pass cannot mathematically satisfy, reconciled and committed as a seed, never computed live in the shipped game | `fusion-recipe-generator` | ✅ **Done 2026-09-07** (this sweep — was still the pre-Phase-8 text until now) |
| [creatures/spec-creature-fusion.md](creatures/spec-creature-fusion.md) §"Boundaries" ("Ask first: … recipe-graph shape changes") | Cleared, not silently taken: `crossRungGapFill` lets a bounded subset of recipes draw inputs from more than one rung below the output — a real graph-shape change, distinct from the computation-method amendment on the row above. Owner, 2026-09-05 (quoted in `spec-fusion-recipe-generator.md`'s own Objective): authorized the whole pipeline shape this change is one consequence of | `fusion-recipe-generator` | ✅ **Done 2026-09-07** (this sweep) |
| [creatures/spec-creature-fusion.md](creatures/spec-creature-fusion.md) §"Recipe fusion" ("every summonable rare/epic/legendary species gets exactly one recipe") | Narrowed to best-effort: a deficit whose LLM vote never resolves ships with **no recipe**, named as `unresolved` rather than fabricated. A separate loosening from the two rows above — coverage, not method or graph shape | `fusion-recipe-generator` | ✅ **Done 2026-09-07** (this sweep) |
| [power/ssot-power-scale.md](power/ssot-power-scale.md) §5.3, §10 | A species `Theta` offset joins the composition and the closed inventory | `threat-band` | ⛔ **Still genuinely open — and not actually actionable yet.** Verified: `creature-threat.v1.json`'s `thetaOffset` column (nuisance=0 … calamity=40) has **zero consumers anywhere in `src/`** — `threatBand` itself is discarded during species generation and unreachable from the live `CreatureSpeciesDef` catalog. §10 is a closed inventory of scales that actually exist and run; adding a numbered row for a tuning column nothing reads would misrepresent it as shipped. **Real precondition, not previously stated: this amendment is owed once something actually consumes `thetaOffset` (e.g. a creature-as-encounter `Θ_content` contribution), not before.** Until then this stays a named, tracked gap, not a doc-edit task. |

---

## 6. Related

- Ideal: [creature-seed-ideal.md](creature-seed-ideal.md) — twenty-six cleared questions, nine research passes
