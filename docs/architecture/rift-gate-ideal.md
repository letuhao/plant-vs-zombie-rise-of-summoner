...
| 17 | **Switch the WebView; never fight the PVZ engine** (owner, 2026-09-15). The menu signal patches the **menu screen's own lifecycle** — the owner's reference is [`darkthemer/PvZF_MainMenuFlowers`](https://github.com/darkthemer/PvZF_MainMenuFlowers), which patches `MainMenu.Start` and attaches a real uGUI `Button` into the game's menu hierarchy. | **`UIMgr.BackToMenu` / `EnterMainMenu` are forbidden.** They are a documented live hazard ("forced entry … can destabilize the engine", `DebugActions.cs:1495-1496`) and calling them breaks a live run; `BackToMenu` also lands one menu layer short (`live-test-ssot.md:242-251`). The existing observe-only `UIMgr` patches may be *consumed* but their detect logic has never worked well and must not be depended on. **No rift-gate code may call an engine navigation method to change what the player sees** — the injector may read menu presence and attach a UI affordance to an existing screen, never drive the engine's menu state. This supersedes the first-draft `menu-anchor` mechanism (map gap 10 / Correction 3). |
| 18 | **The menu becomes one rendering system: uGUI** (owner, 2026-09-15). The menu art moves from IMGUI (`OnGUI` + `GUI.DrawTexture`, `RiftMenuOverlay.cs:79`) into the uGUI hierarchy as a `Sprite` on an `Image` under the menu transform — the reference mod's own shape — so the clickable affordance and the art share one system and inherit the game's canvas scaling. | The in-match "RPG" button (decision 15) **stays IMGUI**: it is match HUD chrome, not part of a menu hierarchy. That split is deliberate and the spec must say so. The only genuinely new call is `Texture2D` → `Sprite` (`Sprite.Create`) — nowhere in `src/FusionRpg.Injector` today; the click path (`ControlClick.cs:150-153`) and uGUI references (`UnityEngine.UI` in all three host csprojs) already exist. Version drift is a **verification step, not a risk**: no one redesigns a menu hierarchy between versions. |
| 19 | **MelonLoader is the first target; BepInEx is secondary** (owner, 2026-09-15). | On this machine `BepInEx/` has only `plugins/` (no `interop/`) and `deploy-play.ps1:5` calls it *"the older 3.8.1 install, kept for BepInEx-specific testing"*; MelonLoader is the deployed default (`:39,115`). If the BepInEx interop does not expose `MainMenu`, the tombstone is MelonLoader-only and BepInEx keeps F10 — a scoped difference stated plainly, not a design input. |
