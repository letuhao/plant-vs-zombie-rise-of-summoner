...
**Whole-catalog total, reconciled 2026-09-09 (was 256 before `poise`):** **259 registered channels** —
the resource families are generated by iterating the six `ResourceIds`. Breakdown: 196 `combat.*`
(84 original + H.1's 112) + 24 status constants (8 original + H.2's 16 duration/intensity) + 9
`progression.*` (7 original + H.7's `xpRate`/`breakthroughSuccess`) + 1 healing (H.4) + 18 resource
(H.5) + 1 `move.range` (H.6) + 10 action-category (H.3) = 259 — plus the
...
Three families over six resource ids, plus `move.range`, giving **19 channels** — not 10: `efficiency`
...
4. **Exhaustion debuffs compose through this catalog like any other derived mod** — same compose kinds, same per-channel caps, no new ordering rule. `FiveExhaustionDebuffsStack` runs five simultaneous debuffs (stamina/hunger/spirit/qi/poise) on one actor, including two independent efficiency debuffs stacking past the cap on one pool while the other four are debuffed.
...
**Former debt, now fused:** `BattleStatComposer` is **deleted** — `BattleEngine` and every delve/siege/web/expedition caller compose through `BattleHubCompose` (ActorHub) exclusively. The 2026-09-07 "stays separate" ADR exception was overturned as a SOLID/DRY defect (2026-09-12) and fused the next day (T6): `guard-actor-hub.ps1`'s ChannelMods-producer allowlist was trimmed to three files at T6 landing (`EquipAtomSource`/`TraitAtomSource`/`TreeAtomSource`), then trimmed again the same session (`battle-ops-parity` T7, 2026-09-13) to `TraitAtomSource.cs` alone — `EquipAtomSource.ModsFor` and `Battle.TreeAtomSource` were both deleted as proven-dead ignore-op folds (equip/tree now reach battle exclusively as op-aware `BoundDerivedAtom` via `HubInputs.BoundAtoms`), so only `TraitAtomSource` still legitimately feeds Hub subsystems raw `BattleChannelMod` rows. A compile error is now the guarantee that a `BattleStatComposer.Compose` call site cannot reappear; the guard's former text-match check for it is gone. Ideal: [combat-power-number-ideal.md](combat-power-number-ideal.md). ADR: `decisions.md` ActorHub sole Hot compose gate (fused note).
