...
| Stats | **StatSystem** in Core: Y0 baseline + source-tagged modifier bag + plugin registry. Forward-only `Y = Compose(Y0, bag)`. See [stat-system.md](stat-system.md) |
...
| Combat damage SSOT | Overlay HP changes use **`DamagePacket`** (signed delta: negative = loss, positive = heal — **one pipeline**, no separate heal feature). **TargetSpec** (who) + **DeliverySpec** (when/how) are orthogonal; **`TargetResolver`** is the sole interpreter of multi/random/area/all targeting. Apply: plan packet → resolve ptrs → CombatMath (pass-through stub; DEF/element later, **per ptr**) → Funnel → FA10 Writer Add. **No** vanilla `BoardAction` cherry/chili for overlay HP AOE. Overlay world VFX is `Shader.Find` + a short `ParticleSystem` burst at a lawn cell (IL2CPP cannot compile ShaderLab at runtime); digits stay IMGUI. **`ProcDepthLimit`** from match/policy (default **6**, overridable — never hardcoded in resolver/bag). Area footprint defaults: cherry/chili fallback constants in policy. Spec: [combat-damage-ssot.md](combat-damage-ssot.md). **Partially shipped** (resolver, instant fan-out, LIVE snapshot, Counter, OnTimer DoT, overlay world flash). |
...
| Overlay control loops | **Hot** = Injector `EffectBag` + Funnel mailbox (combat procs; no Server RTT). **Cold** = UniqueActor / Data (equip, loadout push). **Intent** = `pvz.*` extras after Admit. Ban: Server FSM must not sit between `combat.hit` and FA* apply. Spec: [overlay-control-loops.md](overlay-control-loops.md), [effect-funnel.md](effect-funnel.md). |
