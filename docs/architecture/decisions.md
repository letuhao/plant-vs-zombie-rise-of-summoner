...
| Element Hub SSOT | **Element typing** (`fire`/`ice`/`air`/`earth`, max 2 slots; `omni` additive-only), **40 combat derived channels**, and **ring-cycle matchup matrix** (`MatchupShareK=0.25`, per-component hybrid, dual-type product rule). Element Hub owns matrix semantics; Actor Hub registers channels. Status runtime **independent** in v1. Spec: [element-hub-ssot.md](element-hub-ssot.md). **Shipped** — [combat-element-implement-plan.md](combat-element-implement-plan.md) C1. |
