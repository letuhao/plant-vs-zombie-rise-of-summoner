# Spec: lawn-deploy-progression (`creature-lawn-deploy` module 2)

**Status:** approved 2026-09-08; partial implementation landed 2026-09-08.
**Depends on:** `lawn-deploy-core`, [progression-source-contract](../creatures/spec-progression-source-contract.md).  
**Decision:** [decisions.md](../decisions.md) — *Unique lawn XP receipts (2026-09-08)*.

## Objective

Make a lawn-deployed unique creature grow through the thing it uniquely does: fighting as an individual specimen. A Bound specimen earns its own XP for enemy kills it is proven to have made and for active time it survives on the lawn. These rewards use the specimen XP curve and their own tunable award values; they are not a multiplier or a proxy for general-creature species progression.

The player benefits from keeping a unique creature alive and using it effectively, while the general creature system remains the empire-wide fallback for disposable populations.
...
Every lawn XP candidate must carry a parsed `CreatureProgressionSource.UniqueSpecimen(playerId, instanceId)`. A candidate is eligible only when the following all hold:
...
These keys are intentionally separate from `data/tuning/species-progression.v{n}.json` and its placement/run-completion awards. A tuning pass can make unique lawn advancement faster or slower without changing general-creature empire growth.
...
```csharp
if (source is not CreatureProgressionSource.UniqueSpecimen unique)
...
- [x] Read the design gate, creature-system map, lawn-deploy map/specs, unique runtime, match runtime, event/control-loop, data, stat/ActorHub, tuning, and power-scale documentation in this session.
...
- [ ] Full transaction, binding-id receipt, attribution, Injector, and guard conformance tests remain open in
  `tasks/creature-lawn-deploy-todo.md` Phase 4.
