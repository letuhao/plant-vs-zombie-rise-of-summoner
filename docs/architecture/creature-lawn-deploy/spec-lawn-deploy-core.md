# Spec: lawn-deploy-core (`creature-lawn-deploy` module 1)

**Status: partial implementation landed 2026-09-06/2026-09-08; follow-up hardening remains.**

## Objective

Let a real, owned unique creature specimen (a `rpg_unique_actors` row with a creature profile attached — not
...
`instanceId` that is already a creature specimen and not Commander/Patron-designated,
...
- A creature specimen already lives in `rpg_unique_actors` (`RpgStore.cs:402-416`) — same table, same
  `phase`/`match_key`/`last_ptr`/`deploy_correlation_id` columns, same FSM (`Roster → Deploying →
  ActiveBound`) equipment-bound unique actors use. Confirmed field-for-field against a real creature actor
  row read live this session.
- `UniqueActorService.DeployAsync` (`UniqueActorService.cs:108-188`) reads the actor generically via
  `_store.GetUniqueActor(instanceId)` — no equipment-specific guard exists. It would admit a creature
...
- The trait-binding gap is now closed by `ReconcileCreatureTraitBindingsUnlocked` on every deploy;
...
- **The wrong fix, ruled out before writing this spec**: appending creature-derived grants onto whatever
...
- **The right fix, matching that precedent**: bind a creature specimen's traits to `effect_binding` rows
  ...
  fight) picks them up with zero new wire format. A creature's own traits already map to real
...
1. **Patron must be refused, not just "assumed already designated" — DONE 2026-09-06.** The map's own
   first draft said this program deploys "a unique creature or Commander" — directly contradicting
   `creature-system-map.md`'s Axis 2 ("neither one ever fights"). No code guard existed: `IsPatronUnlocked`
   ...
   **Implemented**: `RpgStore.UniqueActors.cs`'s `TryBeginUniqueDeploy` now calls the same
   ...
   not just built**: `tests/FusionRpg.Data.Tests/CreatureLawnDeployCommanderRefusalTests.cs` (3 cases: the
   ...
   **Commander is a separate finding, not fixed the same way — it can't be, yet.** Investigated directly
   ...
   always returns exactly `[Dave]` for any player — there is no creature-`instanceId` binding anywhere in
   this mechanism. `creature-system-map.md`'s own Axis 2 prose ("designate ONE creature" as Commander)
   ...
   against — the risk the strengthen pass named (a Commander-designated creature double-dipping aura +
...
2. **Binding must be a reconciled diff, run on every deploy — never a one-time mint-time snapshot.**
   ...
   `rpg_creature_profiles.traits_json` in place, keeping existing traits and appending new ones
...
3. **The `DeployMode`-driven side override needs an explicit decision, not a silent one.**
   ...
   `RpgStore.Creatures.cs:49-54`) and never updated afterward — but `GET /api/actors/{id}/derived`
...
4. **Species-magnitude delivery is implemented in the T1.5 binding slice.** The remaining verification
   is the overflow/rehydration regression coverage tracked in `creature-lawn-deploy-todo.md`.
...
- New tests: `tests/FusionRpg.Data.Tests/CreatureLawnDeployTests.cs` (binding creation AND reconciliation —
  explicitly including a promotion-then-redeploy case per Correction 2, not just a first-deploy case), a
  new case in `tests/FusionRpg.E2E.Tests/StorageE2ETests.cs`-style live E2E asserting a creature deploy
  reaches `ActiveBound` with its trait bindings present (mirroring the existing equipment E2E exactly),
  and a `tests/FusionRpg.Data.Tests/CreatureLawnDeployCommanderRefusalTests.cs`-style case proving the
...
- Integration: a real creature specimen (imported through the real species/trait pipeline this session
...
- **Always do**: reuse `AtomPushService`/`effect_binding` — never invent a second content-delivery wire
  format for creatures specifically. Always reconcile bindings at deploy time, never trust a stale snapshot.
...
2. ~~`OwnerKind` for a creature specimen~~ — **resolved while writing this spec**: `OwnerKind.UniqueActor`
   is real, already built (T6.1, `OwnerScope.cs`), and already used for equipment slot bindings via
   `RpgStore.UniqueActors.cs`'s `ReconcileUniqueEquipmentAtomBindingsUnlocked`. A creature specimen is a
...
5. **New finding from the same investigation: `HypnoAlly` deploy is a named refusal, not built.** Making
   ...
   `TryBeginUniqueDeploy` refuses `HypnoAlly`-mode creatures with `"deploy.hypno-ally-not-implemented"`.
   `PlantAvatar`-mode creatures (the common case — "most creatures deploy as plant-side avatars") deploy
...
6. **A real regression this same change caused, found and fixed the same day**: `CreatureLawnDeployCommanderRefusalTests.cs`/`CreatureLawnDeployTests.cs` (T1.1/T1.2) picked their test
...
7. **Species-magnitude delivery (Correction 4) — resolved 2026-09-07, built as T1.5.** A 6-question
   ...
   - `ConcreteSpecies.Magnitudes` (`long`-typed, `PTheta`-derived, already channel-shaped matching
     ...
     `ConcreteSpeciesSeedReader.ToCreatureSpeciesDef` seam** — the same seam `TraitPool` already needed
     curation at. `CreatureSpeciesDef` (the LIVE catalog type `CreatureSpeciesCatalog.Get` serves) had no
   ...
   **Design chosen**: magnitudes are pre-computed (already folded PTheta at species-generation time),
   ...
   — never a dynamically `PowerLadder`-scaled op. `CreatureSpeciesDef.Magnitudes` now carries the value
   forward (added to the ONE shared mapper seam). `RpgStore.UniqueActors.cs`'s new
   `ReconcileCreatureMagnitudeBindingsUnlocked` mirrors `ReconcileCreatureTraitBindingsUnlocked`'s exact
