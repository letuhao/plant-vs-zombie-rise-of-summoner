# Spec: zomboss-deploy-ai (`creature-lawn-deploy` module 3)
...
Zomboss deploys a unique creature, and which one, without a human player to ask. **Never a Commander** —
`creature-lawn-deploy-map.md`'s own Correction (strengthen pass, 2026-09-06) rules this out for both sides,
...
creature or Commander" in its own Objective; that was a real error, not a simplification. Scope is strictly
...
creature to deploy should read only what a player-observed board state would show, and difficulty should be
...
4. **Zomboss's own creature roster is implemented for the current slice.** Data provisions an idempotent
...
1. Zomboss "owns" a roster of unique creatures the same shape a player does (mint/summon path unspecified —
...
- Unit, with a named scenario table (strengthen pass finding — the first draft's "given synthetic board
  ...
  covering: no eligible creature available (policy declines), exactly one eligible creature (policy picks it),
...
1. ~~Where does Zomboss's own unique-creature roster come from?~~ **Resolved for this slice:** the
...
3. Does this module ever need to coordinate with the world-map `ai-commander` (e.g., a legion's own
   composition influencing which creatures Zomboss can field in a lawn encounter tied to that legion), or
