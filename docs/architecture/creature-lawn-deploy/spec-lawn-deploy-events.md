# Spec: lawn-deploy-events (`creature-lawn-deploy` module 2)
...
Decide, during a live lawn run, *when* a unique creature deploy becomes available — for the plant
(player-facing) side. **Never a Commander** — `creature-lawn-deploy-map.md`'s own Correction (strengthen
pass, 2026-09-06) rules that out entirely; a prior draft of this spec asked whether the deployed creature
...
2. **Ownership checks need a Hot/Cold-safe snapshot, not a live Cold-plane read mid-tick.** Assumption 1
   below gates availability on "a deploy the player already owns the creature for" — but a roster is
   ...
   "no server round-trip on the hit path" violation `creature-system-map.md:36` already forbids. **Fix**:
...
1. A "case" is a run-scoped condition evaluated against live match state (wave number, sun/soul economy,
   ...
   player already owns the creature for, checked against the Correction-2 snapshot.
2. The trigger check runs Hot (during the match, off `MatchRuntime`'s own tick or event stream) — never a
   server round-trip on the hit path, matching the hard constraint `creature-system-map.md:36` already
   states for all in-run creature behavior.
3. The plant-side UI surface (a prompt, a HUD affordance) is a **new** FE concern, not an extension of
   ...
   **mid-run** trigger), so it is not reusable here as-is, though its FE conventions (how a berth picker
   presents creature choices) may be worth matching for visual consistency — a design nicety, not a
   dependency.
...
3. ~~Does the player choose *which* owned creature deploys, or is it pre-selected (e.g., always the
   roster's Commander)?~~ **Resolved by Correction 1**: never a Commander. Still open: does the player
   choose among their eligible roster, or is a specific creature pre-selected some other way?
