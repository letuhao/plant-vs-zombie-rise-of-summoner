...
| [architecture/effect-atom-map.md](architecture/effect-atom-map.md) | **Capability map** — atom effects as the Secondary SSOT: 26 modules in 6 waves, dependency order, checkpoints, four cross-program hazards. **All 26 rows built, Checkpoints A–F reached, no golden re-blessed. `EffectSeedCatalog` deleted from production (E23) — the layer fully reaches the running game** |
...
| [architecture/gui-lego-map.md](architecture/gui-lego-map.md) | **Capability map (design-only)** — game menu Lego kit: pieces, themes, recipes; first surface Derived combat console. Ideal [gui-lego-ideal.md](architecture/gui-lego-ideal.md); menu idea procedure [idea-ui-phase.md](architecture/idea-ui-phase.md) (`/idea-ui`); drafts [design/gui-lego/](design/gui-lego/) |
...
| [architecture/class-system-ideal.md](architecture/class-system-ideal.md) | **Ideal capture (not a spec)** — three postures, 12 aptitudes, 6 resources; 2 defence + 2 break per posture. **Free build: the player has no class** (2026-08-25) — classes are Zomboss AI patterns, the class price is withdrawn, and §7b measures what free build requires of the distribution instead |
| [architecture/class-system-map.md](architecture/class-system-map.md) | Class system program: capability map, 7 modules in 3 layers (deterministic core / tuning config / residual fit), build order (**proposed, no module authorized**) |
| [architecture/class-system/spec-aptitude-tuning.md](architecture/class-system/spec-aptitude-tuning.md) | **Spec (proposed)** — the class system balance surface as one versioned config: point economy, the two read functions (PS-3) with a tunable share exponent, the per-family `unitClass` decision, and the channel←aptitude distribution |
| [research/class-analytic-balance-2026-08-25.md](research/class-analytic-balance-2026-08-25.md) | **Proof record** — win rate in closed form (0.4% residual against the simulator), win rate proved **exactly invariant in `Θ`** by homogeneity, and a balanced RPS cycle *solved* in 2.3s rather than searched |
| [research/class-rps-balance-2026-08-25.md](research/class-rps-balance-2026-08-25.md) | **Measurement record** — the simulated search that came first: nine structural rules found by measurement, the `unitClass` answer, and the process failures that cost more than the wrong guesses |
