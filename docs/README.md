...
| [architecture/effect-atom-map.md](architecture/effect-atom-map.md) | **Capability map** — atom effects as the Secondary SSOT: 20 modules in 5 waves, dependency order, checkpoints, four cross-program hazards. **12 built** through E14a (Checkpoint C reached); E11 next |
...
| [architecture/empire-economy-ideal.md](architecture/empire-economy-ideal.md) | **Ideal capture (not a spec)** — what a territory produces and where it lands: the two-tier stock/treasury seam that keeps world replay deterministic, the `resource`/`shard` naming collisions, and the soul-mine question |
