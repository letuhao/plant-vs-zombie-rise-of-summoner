...
| **The atom / Secondary effect layer** | [architecture/effect-atom/definitions.md](architecture/effect-atom/definitions.md) **(wins over any spec)** · [architecture/effect-atom/atom-catalog-ssot.md](architecture/effect-atom/atom-catalog-ssot.md) · [architecture/effect-atom-map.md](architecture/effect-atom-map.md) | The vocabulary is **closed**: **8 attach points, 17 kinds, 13 triggers** (`AtomKindRegistry.AttachPointCount` / `.KindCount` / `.TriggerCount` — verified by counting, 2026-09-06; `decisions.md:113` carries the "Atom attach points" row, not `:112`, which is now a different, unrelated decision). **This row has now gone stale three times.** It said 5/12/7 until 2026-09-03 (trigger count only, corrected to 8, while kinds/attach points lagged behind E34/E35/E36/E37/E41); it said 7/16/13 until 2026-09-06, when base-defense's `siege-construction` added the `Siege` attach point and `structure.place` (decisions.md's "Atom attach points" row) and again nobody propagated it here. Because this file **wins over any spec**, a stale count here outranks every correct one downstream, so a module that widens the vocabulary is not finished until this line moves with it. Adding one is a reviewed change, not a convenience. **`passive-tree/spec-element-conversion.md` (D56, 2026-09-06) proposes a 9th/18th** — not yet built, so not counted here yet; this line moves again the day it lands |
...
| **Anything a player sees (UI)** | [architecture/game-gui-principles.md](architecture/game-gui-principles.md) · [design/information-architecture.md](design/information-architecture.md) · [design/README.md](design/README.md) · [architecture/fe-game-foundation.md](architecture/fe-game-foundation.md) · [architecture/actor-sheet-ideal.md](architecture/actor-sheet-ideal.md) Runtime catalog (when touching ActorSheet / Band B HUD labels) · [architecture/actor-hud-ideal.md](architecture/actor-hud-ideal.md) §4.1 (HUD tokens) | **A game is a stage with layers, not a document with pages.** Proposing another top-level route, another sidebar entry, or a screen the player must *navigate to* — GG-1 says menus open over where the player already is. Also: engine vocabulary (`typeId`, `Intent`, `UniqueActor`) on a player surface, and mixing developer surfaces into game navigation. **Player names and roster membership for sheet/HUD rows load from `data/tuning/*-catalog.v{n}.json`** — do not hardcode FE unions or put copy in balance-number files. Band B glyphs use catalog `hudToken`/`color` — never `StatusInitials(id)` or hashed RGB (actor-hud H1–H3) |
