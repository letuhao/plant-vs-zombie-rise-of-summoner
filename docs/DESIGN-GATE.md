...
| **Where logic may live (server vs injector)** | [architecture/overlay-control-loops.md](architecture/overlay-control-loops.md) · [architecture/pvz-middle-layer.md](architecture/pvz-middle-layer.md) | **The RPG never reads PvZ's current state and never guesses it.** Two async systems. The RPG observes *past* events and contributes a **signed delta** later. **It does not go to the Server to compute damage at the moment of the hit** — `overlay-control-loops.md:22` bans a **Server round-trip** in hot evaluation, not in-process computation. **Corrected 2026-09-13:** this line read *"It does not compute damage at the moment of the hit"* and, as written, forbade the shipped Hot loop — `EffectBag.cs:567,637` already calls `CombatDamageDispatcher.DispatchInstant` in-process at hit time. Injector-local hit-time math is the design; a Server call on the hit path is the defect |
...
| **Stats** | [architecture/stat-system.md](architecture/stat-system.md) · [architecture/actor-hub-ssot.md](architecture/actor-hub-ssot.md) (§8.1 SourceId grammar) · [design/spec-derived-stat-sheet.md](design/spec-derived-stat-sheet.md) · [design/spec-magnitude-and-units.md](design/spec-magnitude-and-units.md) · [architecture/combat-power-number-ideal.md](architecture/combat-power-number-ideal.md) (one-compose SSOT) · `decisions.md` **ActorHub sole Hot compose gate** | One writer. Combat writes go through `EntityStatWriter`, never ad-hoc Unity patches. **`ActorHub` is the sole Hot compose gate everywhere** (lawn, sheet, battle, delve, siege, sim) for actor derived / AppliedCombat — modules contribute via `IActorStatSubsystem` with GG-49 SourceIds, or consume Hub output only; private folds are out of order. **Dual compose (Hub vs `BattleStatComposer`) is an ADR defect** overturned 2026-09-12 — grandfathered debt until fused, **never** a template; citing the old exception as permission for a **new** design = fail this gate. Guard: `scripts/guard-actor-hub.ps1`. **Two classifications of a channel already exist and are verified against consumers** — the nine-class `UnitClass` ledger and the sheet's six render states. **Inventing a third is the failure this row was widened to prevent** (2026-08-24: a spec set proposed a parallel `magnitude`/`bounded-ratio` scheme, and separately concluded "no UI exists", because the two `design/` docs were not in this index) |
...
| **Any numeric magnitude (types, widths, overflow)** | [../CLAUDE.md](../CLAUDE.md) "Numeric types — overflow is RANGE, precision is a separate question" · [architecture/power/ssot-power-scale.md](architecture/power/ssot-power-scale.md) §9.4 | **The ladder is quadratic, so old type choices are now wrong.** Overflow is **range**: `int` per-mille exceeds its range at `Θ`=3,213 and `int` whole units at 103,557. Choosing `int` because "the numbers are small today" is the defect; they are small only at the calibration point. **Floating-point is allowed** (owner ruling 2026-09-15) — a float/double not being integer-exact past 2^24 / 2^53 is precision, not overflow |
...
| **Creature species generation / seedsmith's creature pipelines** | [architecture/creature-seed-map.md](architecture/creature-seed-map.md) · [architecture/creature-seed/](architecture/creature-seed/) module specs | **Seed → concrete → per-player is binding for every generator, creatures included** (`tasks/seed-to-concrete-plan.md`). Species *stats* are deterministic and shared; only *effects* roll, per player, at runtime — never assume a species table is finished content once generated |
| **General creature vs. unique creature; Commander vs. Patron vs. a Delve party member** | [architecture/creature-system-map.md](architecture/creature-system-map.md) "Vocabulary" section (added 2026-09-06) | These four get guessed wrong. **General creature** = PvZ-engine-spawned, species-stats-only, no persistent instance — the troop-stack-shaped representation for army-scale populations (siege garrisons, world-map legions; nobody tracks a legion as a million `UniqueActor` rows). **Unique creature** = a real `UniqueActor` specimen, player-summoned/fused/captured, individually equipped and built. **Commander** and **Patron** are the SAME shape (designate one unique creature, get one continuous passive side-wide aura for the run) and neither ever fights — they differ only in *what* the aura is (leadership + aptitude routing vs. a specific elemental combat bonus) and *which loop* (lawn run vs. match). An ACTIVE combatant creature is a third, unrelated concept — a Delve **party member** (`party-dungeon-ideal.md`), never a Commander or a Patron |
| **Economy / currencies / yields** | [architecture/empire-economy-ssot.md](architecture/empire-economy-ssot.md) · [architecture/economy-principles.md](architecture/economy-principles.md) · [architecture/creatures/spec-soul-economy.md](architecture/creatures/spec-soul-economy.md) | **Every faucet names its sink in the same change.** Territorial income needs territorial upkeep — tuning cannot fix a growth-rate mismatch. A second stock earns its name only if some cost is a `min(x,y)` bottleneck. The repo has already paid for this once: uncapped +2/kill hit ~20-25 pulls/hour against a ~5-8 target. **`empire-economy-ideal.md` is superseded** — reasoning trail only, and it carries four retraction layers |
...
| **Elements** | [architecture/element-hub-ssot.md](architecture/element-hub-ssot.md) | **Two matrices, not one — and the asymmetry is in the *contract*, not the content.** All 36 pairs agree today **by construction** (`ElementTable.Shipped()` hands the shield matrix a copy of the ring's own rows; `ShieldElementMatrix.cs:17-19` records the same). What differs: `ElementRingMatrix` returns `ElementMatchupRelation` with **`Same` distinct from `Neutral`** and bakes K in via `RelationShare` (`ElementRingMatrix.cs:34-40`), while `ShieldElementMatrix` returns a bare `int ∈ {−1,0,+1}`, collapses same-element to `0`, and applies its own decoupled K **once downstream** in `ShieldMath.cs:80`. So one component cannot silently serve both, and the tables are independently editable — divergence is an Ask-first balance decision. **Corrected 2026-09-13:** this row said the matrices are asymmetric in content; `design/spec-shield-and-elements.md` §0 has carried the correction since 2026-08-23 |
...
| **Battle / turns** | [architecture/battle-timeline-map.md](architecture/battle-timeline-map.md) · [architecture/battle-turn-ideal.md](architecture/battle-turn-ideal.md) · **the rule below lives in code, not in either doc:** [`BattleEffects.cs:225`](../src/FusionRpg.Core/Battle/BattleEffects.cs) | **Battle's plan-item executor consumes four actions and nothing else** — `ApplyResourceDelta` (FA10), `ApplyStatus` (FA2), `ModifyStat` (FA1), `PlaceStructure` (`AttachPoint.Siege`); every other action is inert there (`BattleEffects.cs:206,213,221,225`). **Corrected 2026-09-13:** this row said *"consumes FA10 only; it never grants and never calls `OnEvent`"* and was wrong three ways — FA2/FA1/`structure.place` were added after it was written, battle **does** grant (`BattleRunState.cs:654,686`), and battle **does** call `Bag.OnEvent` (`Battle/Siege/ConstructionActions.cs:272`). It also cited two docs that never carried the rule. Widening the allowlist is a reviewed change — add to it, never a parallel mechanism |
...
| **Proving a feature works live / any `/api/debug/*` or Injector `debug.*` call used as evidence** | [contributing/live-probe-standard.md](contributing/live-probe-standard.md) · [architecture/live-probe/spec-debug-scope-guard.md](architecture/live-probe/spec-debug-scope-guard.md) | **Game Injector Debug** (`debug.*` commands, most of `DebugEndpoints.cs`) can fabricate engine-side state and proves only that the engine reflects it. **RPG Server Debug** (`reforge-world`, `derived-audit-actor`, the real `/api/aptitudes/*` endpoints) must run the real domain/persistence path on a real record — never accept fabricated stats/state. A response body is not proof; read the changed row back through the normal query path. 2026-09-13: a live probe fed a fabricated loadout straight into the Injector and read it back from the SAME injector telemetry — passed the wrong half, and the real defect (Hub-bonus grants never reaching Unity) went unnoticed until the owner checked the actual game screen. **`scripts/guard-debug-scope.ps1`** (wired into `deploy-play.ps1`) now mechanically classifies every `DebugEndpoints.cs` route as one scope or the other and checks its scope-banner comment against that computed classification — no longer a review-only discipline |
...
[ ] If this feature produces or consumes an actor combat/derived magnitude: it either
    ...
    derived fold; BattleStatComposer is debt, not a template. Citing the old dual-
    compose exception as permission for a new design = fail this checklist.
[ ] Does not invent or extend a SOLID-violating parallel path (§2.15). If the change
    would deepen known debt (e.g. BattleStatComposer), a remediation plan is named
