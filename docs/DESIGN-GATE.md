...
| **Proving a feature works live / any `/api/debug/*` or Injector `debug.*` call used as evidence** | [contributing/live-probe-standard.md](contributing/live-probe-standard.md) | **Game Injector Debug** (`debug.*` commands, most of `DebugEndpoints.cs`) can fabricate engine-side state and proves only that the engine reflects it. **RPG Server Debug** (`reforge-world`, `derived-audit-actor`, the real `/api/aptitudes/*` endpoints) must run the real domain/persistence path on a real record — never accept fabricated stats/state. A response body is not proof; read the changed row back through the normal query path. 2026-09-13: a live probe fed a fabricated loadout straight into the Injector and read it back from the SAME injector telemetry — passed the wrong half, and the real defect (Hub-bonus grants never reaching Unity) went unnoticed until the owner checked the actual game screen |
