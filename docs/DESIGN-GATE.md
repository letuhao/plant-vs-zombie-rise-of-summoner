...
| **Anything a player sees (UI)** | [architecture/game-gui-principles.md](architecture/game-gui-principles.md) · [design/information-architecture.md](design/information-architecture.md) · [design/README.md](design/README.md) · [architecture/fe-game-foundation.md](architecture/fe-game-foundation.md) · [architecture/actor-sheet-ideal.md](architecture/actor-sheet-ideal.md) Runtime catalog (when touching ActorSheet / Band B HUD labels) · [architecture/actor-hud-ideal.md](architecture/actor-hud-ideal.md) §4.1 (HUD tokens) · **when porting an approved `docs/design/*.html` draft:** [architecture/html-design-implementation.md](architecture/html-design-implementation.md) | **A game is a stage with layers, not a document with pages.** Proposing another top-level route, another sidebar entry, or a screen the player must *navigate to* — GG-1 says menus open over where the player already is. Also: engine vocabulary (`typeId`, `Intent`, `UniqueActor`) on a player surface, and mixing developer surfaces into game navigation. **Player names and roster membership for sheet/HUD rows load from `data/tuning/*-catalog.v{n}.json`** — do not hardcode FE unions or put copy in balance-number files. Band B glyphs use catalog `hudToken`/`color` — never `StatusInitials(id)` or hashed RGB (actor-hud H1–H3). **HTML draft ports:** structure breakdown + component contract before code; side-by-side screenshots + current wwwroot/vite; do not Tailwind-mood-board a locked SSOT |
