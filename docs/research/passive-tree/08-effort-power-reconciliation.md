...
| **6** | **Migrate `rpg_xp_ledger.xp_before` / `xp_after` to `INTEGER`** using the `ReadXp` tolerance shim the snapshot migration already established, and widen the four `double` DTO fields | A persisted `double` XP path — the exact rule CLAUDE.md names *(superseded 2026-09-15: floating-point allowed)*. Harmless until XP gains a multiplier, which is what fix 3 is about | Two columns, one reader, four DTO fields; the migration pattern is already written | **real defect** |
