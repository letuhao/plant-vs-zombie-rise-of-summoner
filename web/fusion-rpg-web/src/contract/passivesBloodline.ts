 ...
 * sheet (the pin, never a browse) and read from the Demon Codex, read-only (§3 point 2, GG-9 --
 ...
 *
 * `DemonCodexEntryDto` (`lib/bus/demons.ts`) is never imported here or by any caller in `ui/` --
 ...
 * `DemonsPage.tsx:369-370`'s own `codexBySpecies.get(...)` already does inline.
 */

/** `DemonCodexEntryDto.state` is `"seen" | "discovered"`; a species with no codex entry at all (the
 * common case for anything never encountered) reads as `"undiscovered"` -- there is no explicit wire
 * value for it, the same `undefined`-means-unknown reading `DemonsPage.tsx`'s own codex map uses. */
...
/** §3 / §9's silhouette rule: `DemonsPage.tsx:370`'s own `known` reading -- "discovered" and "seen"
