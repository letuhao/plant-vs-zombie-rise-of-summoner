...
/// <summary>
/// One demon specimen the plant-side deploy-event UI may offer — enough to identify it and let the
/// client resolve display info from its own already-loaded species catalog (`GET /api/demons/catalog`),
/// not a duplicate of the full roster DTO.
/// </summary>
public sealed record LawnDeployRosterEntry(string InstanceId, string SpeciesId);

/// <summary>
/// demon-lawn-deploy T2.1 (spec-lawn-deploy-events.md Correction 2) — frozen, deploy-eligible roster
...
/// `CommanderId` (`Core/Commanders/CommanderId.cs`) has no demon-instance binding today.</para>
///
