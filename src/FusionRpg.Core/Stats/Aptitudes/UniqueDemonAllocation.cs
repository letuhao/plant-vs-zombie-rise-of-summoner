...
/// <summary>
/// passive-tree G7 (spec-species-tree.md §8.1 point 2) — makes <see cref="AllocationScope.UniqueDemon"/>
/// real, the exact `UniqueDemon`-scope mirror of <see cref="SpeciesAllocation"/>'s own `DemonType`
/// binding (<c>SpeciesAllocation.cs:35,62</c>). Before this type, nothing in `src/` passed
/// <see cref="AllocationScope.UniqueDemon"/> to <see cref="PointBudget.PointsFor"/> or
/// <see cref="PointBudget.CheckScope"/> — every real call site read `Commander` or `DemonType`
...
/// of it. `UniqueDemon` is keyed by `instanceId` — one specimen. Different identity grammar
/// (`spec-point-economy.md` §2: "unique demon | keyed by instanceId | specimen level"), so this is a
...
/// </summary>
public static class UniqueDemonAllocation
{
    /// <summary>
    ...
    /// scaled by the UniqueDemon budget at this SPECIMEN's own level. Zero shares →
    ...
    /// (`specimenLevel &lt;= 1` ⇒ <see cref="PointBudget.UniqueDemonSourceFromLevel"/> = 0 ⇒ budget = 0),
    ...
    /// discipline to the `DemonType` twin) so the shares' points sum to exactly the budget rather than
        ...
        IReadOnlyDictionary<string, long> planSharePermille, long specimenLevel, AptitudeTuning tuning)
    {
        ...
        var source = PointBudget.UniqueDemonSourceFromLevel(specimenLevel);
        var budget = PointBudget.PointsFor(AllocationScope.UniqueDemon, source, tuning);
                     ...
                     .ThenBy(id => id, StringComparer.Ordinal))
        {
            ...
            if (points > 0)
                allocation += AptitudeAllocation.Single(AllocationScope.UniqueDemon, aptId, points);
        }
        return allocation;
    }
}
