...
public static class DemonEndpoints
{
    public static void MapDemons(this WebApplication app)
    {
        ...
        // species on a live roster (e.g. proving a dual-typed demon's live elementPayload) without
        // gambling the player's real souls on it. Reuses the same MintDemon atomic path every other
        ...
        g.MapPost("/debug/grant", async (DebugGrantRequest body, RpgStore store, IHubContext<RpgHub> hub) =>
        {
            ...
            var (specimen, newlyDiscovered) = store.MintDemon(pid, new DemonMintSpec
            {
        ...
        // Debug-only: a BARE UniqueActor (no demon profile) at a given side+gameTypeId, deployed with
        ...
        // live store. No demon profile means TryBeginUniqueDeploy skips the demon-contracts gate
        // entirely (that gate only fires when ReadDemonProfileUnlocked finds a row), so this never
        // touches the real player's contract-slot/loyalty state. OwnerElements (AtomPushService.cs)
        // only reads Side/TypeId via LawnElementIndex, never the demon profile, so this is enough to
