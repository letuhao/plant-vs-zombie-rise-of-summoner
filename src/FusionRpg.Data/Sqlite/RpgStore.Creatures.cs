...
public sealed partial class RpgStore
{
    ...
    public (CreatureSpecimenDto Specimen, bool NewlyDiscovered) MintCreature(long playerId, CreatureMintSpec spec)
    {
        lock (_gate)
        {
            ...
            var specimen = MintCreatureUnlocked(db, playerId, spec, DateTime.UtcNow.ToString("o"), out var newlyDiscovered);
            tx.Commit();
            return (specimen, newlyDiscovered);
        }
    }

    /// <summary>Mint inside an open transaction — used by MintCreature and the summon pull.</summary>
